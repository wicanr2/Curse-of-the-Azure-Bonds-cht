...
更新日期：2026-08-23（第 701 輪：**兩個「卡住」都不是幀數不夠**——（一）巫師塔那四格是 `GEO5/0x33` 的**單向環**：`(4,3)→北→(4,2)→東→(5,2)→南→(5,3)→西→(4,3)`，四面朝外全是牆，而重放端的 `routeDead` 只數「路線說走得通、按下去沒動」，環裡每一步都真的走得動 ⇒ 永遠不會被判死；2,600／6,000／15,000 三個幀數上限**停在同一幀**（第 1,961 幀、245 格）。補上走位的放手條件（`moveSinceProgress`，耐心逐值量過取 2）之後 → 342 格、最深 `0x51`。（二）接著卡在尤拉什 `(4,7)(4,8)(5,7)` 三格 1,700 幀、門選單一次都沒開——出口是 `(4,8)` 東面一道上鎖的門，而 `tryDoors` 是「轉一圈、每一面按一次前進」，第一次前進就沿著**開著**的那一面離開了；改成只撞 `CanMoveDungeon` 為假的方向 ⇒ 撞到門 1 → 29 次。重量預設幀數取 12,000（轉折點）：**572 格／264 句／13 次段轉換，走到第三章尤拉什**（`0x01→0x02→0x03→0x04→0x50→0x31→0x32→0x33→0x32→0x33→0x50→0x51→0x10`）。⚠ 「落回原文」那條斷言移到報表**後面**：排在前面時 `t.Fatalf` 會把段序列與選單清單全部吃掉，只剩兩行英文。**同一輪把第一人稱最後 13 張差異的成因找到**：spec 1006 的深度 2 掃描在 `if w <> 0` **之前**還有一條清除——出界（座標不在 0..15）而且內側沒牆時把「上一段」歸零，那一格就不補收邊；判斷用的是**還沒 clamp 的**座標，clamp 之後 16 變成 0 看起來就在範圍內 ⇒ 這也是為什麼修好 clamp 之後那 13 張還是差。engine 補上之後 13 張**逐張重驗全部 0 格**（389 → 0）。順帶：`cmd/map-spawn-audit` 加掃「`NEWECL` 前面寫的座標」，巫師塔的宣告 `(7,15,6)` 與 `ECL5/0x32:0537` 逐欄相符 ⇒ `mismatch` 4 → 3）
