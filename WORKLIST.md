...
更新日期：2026-08-24（第 704 輪：**攻略當通則寫進 `CLAUDE.md`，並把還開著的工作重新盤一次**。（1）新增 `CLAUDE.md` 的「用公開攻略協助金盒子 remake」四條：先查攻略再決定要不要反組譯；攻略是**語意的交叉核對**不是座標的來源；它最大的用處是**當檢查清單抓「盤點少了一整塊」**；**絕對不要把攻略座標直接抄進 game pack**（宣告會蓋過原始資料，症狀是「測試綠、玩家走不到」）。（2）`cmd/map-atlas` 把 18 張第一人稱地圖逐張畫成「事件在哪一格、從哪幾格離開」，並加一條擋著的不變量：宣告的出口落在牆上就紅（17 個逐個查）。（3）**新發現一個 P1**：主線快照走訪 114 份裡有 **5 份推不動**，同一個成因——`ECL6/0x45:0557h` 的 `TREASURE ... 7F80`，`7F80` 應由 `0x047D ADD 7B2C, 80h → 7F80` 寫成 `≥ 80h`（隨機那一路），remake 讀到 **1** ⇒ 查固定區塊而 `ITEM6.DAX` 沒有區塊 1；形狀是**控制流到得了 `0557h` 卻沒經過 `047Dh`**。閘釘在**相異成因 1 種**（不是份數，份數會隨快照漂）。（4）順手記帳：`key-driven-session` 那一列的 JSON 落後兩輪（還寫著 1,500 幀／2 段），重生成 12,000 幀／5 段；`README` 的 `partial 487`、`decoded 307`、`250 個分派索引` 全部換成量到的 367／299／288。（5）盤點了 engine 與作品的實際切線：內容界線守住了（engine 非測試碼命中 CoAB 字樣 2 處，兩處都是註解），程式碼界線沒有（engine 6,907 行 vs 作品這一側約 22,800 行的共用層）——寫在 `docs/knowledge/engine-title-split.md`，**不建議現在搬家**。（6）按鍵重放依使用者指示 pending，停的位置與跑法記進 `WORKLIST.md`。）
...
|---|---|---|---|
| **P1** | ECL 的 2 個 `partial` opcode | 可達 14,177 條裡 **367**（`PICTURE` 199／`CALL` 168）| `PICTURE` 要 `4FBAh`／`4FBBh` 的不重繪旁路模型；`CALL` 要原版當 oracle 判掉 `ViewMirror.Block` 那三個宣告的 spawn |
| **P1** | **快照走訪 5 份推不動**（本輪新發現）| 114 份裡 109 推得動、**5 份同一個成因** | `ECL6/0x45:0557h` 的 `TREASURE ... 7F80`：`7F80` 應由 `0x047D ADD 7B2C, 80h → 7F80` 寫成 `≥ 80h`（隨機那一路），remake 讀到 **1** ⇒ 查固定區塊而 `ITEM6.DAX` 沒有區塊 1。**形狀是控制流到得了 `0557h` 卻沒經過 `047Dh`**，`7F80` 留著別處的舊值（它是通用工作格）⇒ 下一步是把到 `0557h` 的路徑逐條列出來比對 |
...
| **P2** | Go 漢字 literal 債 | 1,938（`frontend_ui_debt` 24／`runtime_ui_debt` 1,914）| **這是遷移清單不是缺陷數**：那 24 筆逐筆看過全是旗標說明與診斷字串，玩家看不到 |
...
### 暫緩：按鍵重放（使用者 2026-08-24 指示）

**這一項先停在這裡，不再往下追。** 記下停的位置與當時量到的數字，接手的人
不必重跑就知道現況。

- **跑法**（照這條就重現得到）：
  ```bash
  COAB_DECISION_LOG=/src/workplace/campaign-frames/route-clean-new.json \
    ./tools/go.sh test ./internal/game/ -count=1          # 錄路線（整包，24,317 步）
  COAB_ROUTE_JSON=workplace/campaign-frames/route-clean-new.json COAB_KEY_FRAMES=12000 \
    ./tools/go.sh test ./cmd/azure-bonds-game/ \
      -run '^TestKeysDriveARealSessionFromTheTitle$' -count=1 -v
  ```
  ⚠ 路線檔要**跟程式碼同一版**。下水道那條路一改，舊路線的移動步驟就對不上格子，
  而症狀是「走得比較短」不是報錯。
- **停在哪**：12,000 幀走過 137 格、記到 164 句、落回原文 0；走到過的 ECL 段
  `0x01 0x02 0x03 0x04 0x50`（第 752 輪第一次自己走進火刀據點並打到世界地圖）。
  **最後一次有新東西在第 1,437 幀**，之後 **11,271 幀停在世界地圖**（`ModeWilderness`）。
- **下一個瓶頸換位置了**：不是幀數、不是走法、也不是下水道——是**世界地圖那一層
  的選單策略**（`巡查森林｜繼續旅程｜紮營` 這一組繞不出去）。要動之前先照
  `AGENTS.md` §3「調參數之前先確認參數是不是限制」量一次。
- ⚠ **137 格不能拿去跟先前的 390 格比**：兩次用的是不同的路線檔，而路線檔換了
  分母就換了。要比就用同一份路線重跑兩版程式碼。
...
| P1 開著的 | 缺什麼 | 下一步要的東西 |
|---|---|---|
| ECL 的 2 個 `partial` opcode | 可達指令 14,177 條裡 `partial` 還有 **367**（`PICTURE` 199／`CALL` 168）。三個缺口的**大小都量出來了**：`PRINT RETURN` 是 110 個換行段落裡 **10 段會空行、7 段玩家看得到**（`cmd/ecl-print-return-audit`）；`PICTURE` 的 `4FBAh`／`4FBBh` 已解（＝現在／上一次的畫面模式）⇒ 旁路是「前後都還在第一人稱就不重繪」；`CALL` 剩 `2E10h` 髒旗標的**四張宣告值** | `PRINT RETURN` 那七頁全部由 `text_rule` 服務、UI 的 `wrapTextLines` 本來就把 `\n` 當段落切 ⇒ **改的是那七則譯文的版面**，要配截圖驗收（訊息框有 `maxLines` 上限）。`CALL` 的四張宣告值仍要原版畫面當 oracle |
