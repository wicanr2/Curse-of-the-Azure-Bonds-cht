...
更新日期：2026-08-24（第 702 輪：**把「有數字但沒歸零」那九項做完**。分成三類：**（甲）真的是缺陷、修掉了**——中文校對兩處譯法不一致（「那名角色／那名隊員」、「立石／立石群」）；`NextJournalPage` 玩家按不出來（翻頁走前端的顯示頁編號、`State` 記的是第幾則，兩邊不同步，畫面完全正常而 `State.JournalPage` 永遠停在 0）；**全滅只印一句「戰鬥失敗。」就回地圖**——原作的 `2Eh DAMAGE` 收尾在全隊倒下時設 `DS:4FC7h`、兩個主迴圈都以它收尾，與 `PROGRAM 3` 同一個結局，而 corpus 裡唯一的 `PROGRAM 3` 在 demo 裡、正常遊玩走不到 ⇒ 那條路等於沒接。⚠ 接上之後連帶翻出兩個：回標題會把 session 整個丟掉（按「開始」落進沒有 session 的荒野），以及 `visits` 只在地城／事件畫面累加（被事件推回原格的那一格永遠是 0 ⇒ 兩格之間乒乓 5,882 次）。★★ **這個修法也戳破了先前的覆蓋數字**：在那之前隊伍全倒了遊戲照樣走，一場 session 的大半是**一支 HP 全 0 的隊伍**在走 ⇒ 同一份路線 476 格 → **105 格**，105 才是誠實的。下一個瓶頸換成「重放打不贏架」，不是幀數。**（乙）早就判過、只是數字還掛著**——音效兩個「1」（`SOUNDHALT` 是 remake 架構不需要、`CRASHFX` 是平台差異）、每格事件 10 個「沒演出來」（四種盤點限制，逐筆歸類，還沒歸類 0）、走訪可達性那 1 格（幾何上斷開，逐格實測是 `played`）、段交接 6 個入口（一段可以有好幾個入口）。處置一律是**把判定寫進產生報表的程式碼**並拆成「還沒判的／已判定」，不是把數字四捨五入掉。**（丙）不能收的**——`0Eh PICTURE`（缺 `4FBAh`／`4FBBh` 的不重繪旁路模型，玩家看得到）與 `2Dh CALL`（`ViewMirror.Block` 擋板要先判掉三個宣告的 spawn，而那要原版當 oracle）；Go 漢字 literal 債 1,909 是遷移清單不是缺陷數（那 24 筆逐筆看過全是旗標說明與診斷字串，玩家看不到）。**同一輪修了一個儀器的洞**：`ecl-address-refs -all` 傳 nil 起點、跟不到只有生命週期入口才進得去的碼，於是同一個位址在 `-writes` 找得到、在 `-all` 是 0。修完立刻翻出一個先前下錯的結論——`4C2A`（下水道往荒野那道閘門）不是「沒有人寫」，寫它的是 `ECL2/0x04:042F`。`33h PRINT RETURN` 因此收成 `done`（`65A0h`／`65A1h` 是 DOS 資料段的游標，ECL 碰不到，玩家看得到的空行 spec 1147 已接完）⇒ `partial` 487 → **367**、opcode 3 → **2**。**驗證降級**（使用者決定）：第一人稱全量比對不再例行跑，改成 42 張回歸樣本（`docs/reference/original-dos/first-person/regression-sample.tsv`），口徑寫進 `AGENTS.md` §8）
