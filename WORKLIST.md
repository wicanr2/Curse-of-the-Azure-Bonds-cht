...
| **P0** | **戰鬥回合生命週期**（`RE-06` → `ENG-07`）| ✅ **逐項對照收完了**（spec 1135／1136／1137／1138）：先攻算式、選誰動、`20 → 19`、DELAY、GUARD、QUICK、定身、面向與背後攻擊、機會攻擊全部已接且與原作相符；唯一沒接的突襲 −6 **已判定為死碼**（spec 1136）。⚠ 剩兩個小缺口：`+07h`（保留的機會攻擊）沒有投影進 `Fighter`，以及 ECL `24h COMBAT` 的 199 處 `partial` 指的是**遭遇編成的參數**還沒逐處對過（不是生命週期）| 把 `+07h` 投影進 `Fighter`；`24h COMBAT` 的參數依出現次數逐處對 |
