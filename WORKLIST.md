...
| 4 | `RE-07` → `ENG-08` | 怪物 AI。**RE-07 其實大半已解讀**（COMPTACT 16/38，大的全在內）；**移動已接進 remake**（spec 1114）：行為模式 1..6、五個候選方向、半格成本、走到射程才攻擊、20 次上限。剩：原作的目標挑法、用道具（835）、選法術（836）、士氣與恐慌逃走（758／830）、自動換裝（1004）、障礙豁免效果（837）|
| 5 | `ENG-09` | 全法術表：**資料半邊完成**（spec 1111）。100 筆全部匯出成 `gamepack/rules/spell-table.json`，16 個位元組每一個都有出處：職業／環數／兩組持續時間係數／目標模式與半徑／選目標模式／豁免處置與類別／效果編號／紮營限制／施法時間／AI 分數／目標檢查／掃描半徑。分母改成原作那 100 筆：占位 13、只能紮營 8 ⇒ **戰鬥可施放 79 支，已宣告 12 支**（`combat-spell-coverage-ledger.md`）。剩下的是 67 支的**效果**，不是資料 |
| 6 | `RE-05` → `ENG-10` | 存檔（spec 1115）。**角色記錄 422 bytes 逐位元組有狀態**：`decoded` 294／`documented` 99／`unknown` 29，`decoded` 由位元組突變逐段量測驗證，對帳跑在 `go test` 裡。**存檔完整性的閘**：`Fighter` 每個匯出欄位都要能存進讀回（反射掃描），當場抓到「新增 `StatusPartyFled` 後逃離結局的存檔讀不回來」。剩：`.SWG`／`.FX` 的台帳、PC-98 `CHARREC`（多一 byte）、遊戲層快照的同等閘 |
