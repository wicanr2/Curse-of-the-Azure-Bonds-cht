...
下一個最小可重現工作：**照 spec 1160 那六件事一次做完**。九處原作行為的目的地
與十個測試斷言都已經推導好，塔頂的下樓路也查清楚了（`ECL5/0x33:0811h` 的選單選
`CAVES` ⇒ `088Bh` 設 `(6,15)` 再 `NEWECL 32h`，⚠ 那兩句座標寫入後面**沒有**
`CALL 2E10h`，交接靠 `NEWECL`）。

要動的六處：① 拿掉 `projectFreshDungeonCoordinatesBeforeCall` 的 `Spawn`
early return；② `case 0x2E10` 的守衛改成逐條看 `CallRequests[i].BlockID`；
③ `2Dh CALL C01Eh` 改讀腳本當下的 `C04D` 並把走完的座標寫回暫存器；
④ 重畫要消費座標寫入（記「上一次重畫吃到哪個執行序」）；⑤ 宣告的 spawn 改成
後備；⑥ 拿掉 `ECL2/0x02` 的座標位移。**缺一條主線就會停在某個門口。**

測試那一側還要處理 `walkNormalDungeonTo` 的寫死排除清單——`(8,15)`／`(12,10)`
落在巫師塔那張圖上，會把第 15 列的走廊切斷。

驗收：主線通關測試全綠，而且每一段路線的起點都由原作行為推出來，
不是把失敗值抄進期望值。⚠ 查「誰跳到這個位址」一律用 `cmd/ecl-window -into`。
