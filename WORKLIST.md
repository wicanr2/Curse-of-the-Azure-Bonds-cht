...
更新日期：2026-08-18（第 577 輪：第二個 AC `+19Bh` 的算式與資料驗證、AC 刻度不一致盤點；第 573–576 輪：回合開始生命週期、突襲遮罩死碼、面向與背後攻擊、90° 扇形與機會攻擊面向閘；第 572 輪：第一人稱畫面與原版逐格比對 19／20；第 569–571 輪：README 五張圖的圖層對齊、第一人稱牆面第 0 段符號、戰鬥地圖同步；第 568 輪：第 2 批完成、雲格驗證、龍巫妖等級吸取）
...
| 2 | **戰鬥回合生命週期**（`RE-06` → `ENG-07`）| ✅ 回合開始那一段收完了（spec 1135／1136）：先攻算式、選誰動、`20 → 19`、DELAY、GUARD、QUICK、定身、機會攻擊全部已接且與原作相符；**突襲遮罩是死碼**（全 build 一讀一清 0，先攻的 −6 走不到），CoAB 傳 0 是唯一有證據的值。✅ **收完**（spec 1137／1138）：面向、動作計數、累計轉向三個欄位接上，開場面向表、攻擊時轉向、機會攻擊的轉向記帳、回合開始只清後兩格都與原作相同；背後攻擊的三道條件是字面移植。**兩處刻意留白**：`+09h` 的幾何意義（兩個寫入端互相矛盾）與 `+19Bh` 的算法（spec 1000 未解讀）——後者用 `ArmorClassFacingKnown` 擋住，沒標已知就不改用 | ✅ **spec 1010 的 180° 扇形檢查已接**（spec 1002 八個方向逐條讀完，兩平台一致；斜向並沒有「多一道條件」，八段是同一個形狀）。✅ **第二個 AC `+19Bh` 解出來了**（spec 1000 §七：`＝ +19Ah − 敏捷防禦調整 − 盾牌槽 − 2`，81 筆 `MON*CHA` 有 78 筆殘差 0、三筆是 ＋1／＋2 盾牌），已接進 `ArmorClassFacing`（搬差值不搬絕對值）。剩：機會攻擊前面四道未解讀的閘還沒有對應物 |
...
#### ⚠ 新開的工作：AC 與命中的刻度在三個地方不一致

讀 `+19Bh` 時量到的，**不是背後攻擊的問題，比它大**。

原作的命中判定是 `d20 ＋ 攻擊者儲存命中(+199h) >= 目標儲存 AC(+19Ah)`，
兩邊都在 40..70 這個域裡（畫面上的 THAC0 與 AC 都是 `60 − 儲存值`）。
`internal/combat` 的 `ResolveAttack` 寫的正是這個式子（`attackTotal >= AC`，
數字大＝難打），但餵給它的兩個來源都不在那個域：

| 來源 | 現況 | 後果 |
|---|---|---|
| `monster.CombatArmorClass` | AC 反轉成顯示值（`60 − 儲存值`），`AttackBonus` 直接搬儲存值 | 真實怪物 AC 1..7、命中加值 41..53 ⇒ `d20 ＋ 41 >= 1` **永遠命中** |
| `internal/party` 的 `Fighter()` | `armorClass ＝ 10 − (敏捷 − 10) / 2`（AD&D 降序）、`ArmorClass -= 護甲改善` | 升序的判定配降序的值 ⇒ **敏捷越高越好打**、防護法術讓人更好打 |

量到的對照：FIRE KNIFE 儲存命中 41、儲存 AC 59 ⇒ 原作需要 d20 ≥ 18，
與畫面上的 THAC0 19／AC 1 相符；remake 目前是 `d20 ＋ 41 >= 1`。

⚠ 這一項**沒有動**：統一刻度會改到隊員模型、怪物匯入、效果修正與整批戰鬥測試，
是一次獨立的工作，不該夾在背後攻擊裡順手改。背後攻擊那一格因此搬的是
**差值**而不是絕對值（見 spec 1138），刻度統一之後那個關係仍然成立。
