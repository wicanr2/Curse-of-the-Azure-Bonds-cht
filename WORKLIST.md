...
擷取環境做好了，收在 [`tools/dos-oracle.sh`](tools/dos-oracle.sh)：Docker `dosbox-run:latest`
＋ Xvfb ＋ xdotool，`START.EXE STING Wooden` 測試模式繞過翻譯輪（spec 530），
支援連拍。**在圖形模式下按鍵是通的**——按 `d` 可以進示範模式並拍到原版的
第一人稱畫面。

擋住的是**顯示介面卡選不到 EGA**：

- 沒有 `CURSE.CFG` 時，開機先問「顯示介面卡」與「存檔路徑」，兩題都在**文字模式**。
  **實測按鍵在文字模式送不進去**：3 秒與 15 秒都試過，每次送鍵前重新搜尋視窗
  並重設焦點也一樣（那兩題沒有逾時，所以不是來不及按）。
- 有 `CURSE.CFG` 時直接進圖形模式、按鍵正常，但那份設定是**預設的 CGA**。
  遊戲第一次跑會建出一個 0 byte 的 `CURSE.CFG`，之後就當成「已設定」。
- 手寫 `CURSE.CFG`（第一個位元組填 `02h`／`'2'`）沒有用：前者仍是 CGA、後者整個黑掉。
  檔名字串在 `GAME.OVR` 偏移 `114766` 的 `\x05CURSE`，寫檔的常式還沒讀。

⇒ 目前只能拿到 **CGA（4 色）**畫面，而 remake 畫的是 EGA，**逐格美術對照做不了**。

另外量到一件會改寫既有標註的事：**示範模式走的是示範專用地圖**，地名是
`NOWHERE IN THE REALMS`，不是提爾佛頓街道（spec 1130 已記）。所以示範模式
拿不到提爾佛頓的第一人稱畫面，要拿得**實際建隊走進城**。

三條可走的路，代價差很多：

| 解法 | 內容 | 代價 |
|---|---|---|
| **A. 解 `CURSE.CFG` 格式** | 反組譯 `GAME.OVR` 裡寫這個檔的常式，直接手寫一份 EGA 設定 | 一次 RE，之後永久免疫；與本專案既有的 IDA 流程同一套 |
| **B. 解文字模式輸入** | 容器裡起一個真的視窗管理員，或改用其他注入方式 | 環境問題，不確定性高 |
| **C. 只比結構不比美術** | 用現有的 CGA 畫面比「哪些格有畫、輪廓對不對」，不比每一塊磚的圖 | 最便宜，但回答不了「選圖對不對」 |

⚠ 走 A 或 B 之後還要**建一隊、走進提爾佛頓**才拿得到街道的第一人稱畫面
（示範模式的地圖不是它）。那一段是另一段鍵盤序列的迭代。
