...
| ✅ 大半收掉（第 718 輪）| 實跑覆蓋的未蓋段 | 整段 0% 歸零：`0x52` 判定為 **demo**（spec 278，0% 是設計事實、報表已註記）；戰火街區 `0x15`→43.4%、洞窟 `0x35`→54.0%、`0x45`→48.8%（加進 `walkableRouteSegments` BFS 走訪）；酒吧 `0x23`→19.1%（新 `TestRealSideServiceMenusAreWalkable` 選單走訪）。總覆蓋 55.9% → **60.6%** | 剩魔法商店 `ECL4/0x25` 3.9%：BFS 走得進地城但 SHOP 的物品買賣對話（`0x822E` 起 661 條叢集）要商店互動走訪才碰得到 |
