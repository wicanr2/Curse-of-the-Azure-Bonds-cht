...
| **P1** | **存檔剩餘欄位** | 角色記錄 422 bytes 裡 `unknown` 還有 **24**（decoded 299／documented 99，量到有讀 291）；`MOVEPARTY` 跨遊戲轉移未做。✅ **SAVGAM 的 ECL 記憶體已雙向接上**（spec 1163）：前三塊 ＝ 區 0／1／2，⚠ 區 3（ECL 程式碼本身）不寫回，自我修改過的 bytecode 不進原版版面的存檔 | 突變量測逐段收斂 `unknown`；轉移另開 |
