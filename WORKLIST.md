...
更新日期：2026-08-17（第 568 輪：第 2 批完成、盤點剩餘工作、復原術保留的決策）
...
- **逐法術視覺資產**：spec 1126 證明原作只有一段共用演出加少數例外，不存在「七十幾筆待補的視覺資料」。**火球與閃電確實不一樣**——閃電有專屬的電弧圖（COMSPR 區塊 6）沿線逐格播，火球沒有專屬圖，它靠共用投射物飛過去、再對範圍內每個目標依序播魔法傷害命中（區塊 10）。⚠ 那三個槽是 `entry#24` 那條路的全部，**不是整個遊戲的全部**：兩種雲的區域格（區塊 4／2）與弓箭八方向（區塊 0／1／2、128..130）走別的路。

#### 決策：復原術保留，等級吸取不自己加（使用者 2026-08-17）

- **復原術照原作實作保留**。它在 CoAB 一定回「沒有可恢復的等級」，但規則、
  `+0E7h`／`+0E8h` 兩個欄位與存檔往返都已就位——**同系列後續作品有等級吸取時
  直接可用**，共用引擎那一側不必重寫。
- **不自己挑一隻怪物加這個能力**。使用者提到的「蝕心魔」不在 CoAB 的怪物名單裡
  （68 種，見 [`monster-roster.txt`](docs/audit/monster-roster.txt)；最接近的
  中文名是眼魔、羅剎、食人魔）。要加就是**明確偏離原作**，而本專案的最高優先是
  正確性／領域對齊，所以這一步等使用者指定名單裡的哪一隻、以及要不要接受這個
  偏離，再做。真要做的話成本很小：`MON*SPC` 給它一個效果碼，效果 handler 寫
  `+0E7h += 1`／`+0E8h += 失去的 HP`，復原術那一側完全不用動。
