...
下一個最小可重現工作：**解釋 `ECL2/0x04:160Ch` 那一處退格分支在火刃巢穴路線上
什麼時候成立**（spec 1155）。`4BF0`／`4BF1` 的 producer 已經接上，拿掉 `C04D`
條件之後投影的是真座標了，但路線仍在第 4 步分歧——剩下的差異是「哪些 `CALL`
該投影」。那一處的守衛是 `COMBAT` 之後 `COMPARE 7EC7 80h / IF >`，所以要先答出
`7EC7h` 是什麼（戰鬥結果？逃跑？），才知道 remake 在該路線上有沒有讓它成立。
驗收是把 `C04D` 條件拿掉之後那三條火刃巢穴路線測試仍然綠。
