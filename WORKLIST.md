...
| **P1** | **ECL 的 10 個 `partial` opcode** | 1,037 條指令：`CLEARMONSTERS` 206、`PICTURE` 199、`COMBAT` 199、`CALL` 168、`PRINT RETURN` 120、`TREASURE` 63、`DAMAGE` 24、`LOAD PIECES` 23、`LOAD FILES` 22、`PROGRAM` 13 | handler 的位址與條數在 [`docs/audit/ecl-opcode-handlers-dos.md`](docs/audit/ecl-opcode-handlers-dos.md)——**照成本排順序**（✅ `33h PRINT RETURN` 已讀完（spec 1147，缺口在 UI 行模型）；✅ `0Eh PICTURE` 也讀完了（spec 1148，缺口收斂成表現層三項）；✅ `24h COMBAT` 的分派順序也照抄了（spec 1149）；✅ `2Dh CALL` 七支全部讀完（spec 1150，缺口收斂成 `2E10h` 的髒旗標模型）；✅ `27h TREASURE` 也讀完了（spec 1151，修掉隨機表的區間 bug 與清單順序；缺口收斂成「把 spec 1036 的造物品接進隨機戰利品」）；接下來依條數是 `2Eh DAMAGE` 305 條與 `37h LOAD PIECES` 104 條）；每一個都要 producer→state→consumer 三段齊全 |
...
下一個最小可重現工作：**把 `CREATERNDTREASURE`（spec 1036，已 `exact`）接進隨機
戰利品**——`ItemBlock > 80h` 產生的物品目前只有 `{Type, Count}`，加值、名稱三段
索引、重量、價值、卷軸法術與特殊物品範本都是空的。這是接線工作不是逆向工作，
驗收是「隨機戰利品在撿東西畫面上顯示得出加值與名稱修飾」。

之後回到 `partial` 名單照成本排：`2Eh DAMAGE`（305 條）、`37h LOAD PIECES`（104 條）、
`38h PROGRAM`（104 條）。
