...
更新日期：2026-08-18（第 569–571 輪：README 五張圖的圖層對齊、第一人稱牆面符號、戰鬥地圖同步；第 568 輪：第 2 批完成、雲格驗證、龍巫妖等級吸取）
...
| 建議 | 工作 | 為什麼是這個順序 | 粗估 |
|---:|---|---|---|
| 1 | **原版第一人稱 oracle**（進行中，卡在 EGA）| 見下方「原版 oracle 的現況」 | 已花約 25 次擷取；剩餘代價取決於走哪一條解法 |
| 2 | **戰鬥回合生命週期**（`RE-06` → `ENG-07`）| P0 而且是主線戰鬥的地基。ECL `24h COMBAT` 的 199 處目前只接了分派點；initiative／held／delayed／guard／quick 沒接，主線戰鬥就不是原作行為 | 五項各自要 RE ＋ 規則 ＋ 回歸測試，是一個完整批次 |
| 3 | **開場→結局的主線續接** | 唯一真正決定「能不能通關」的工作，但它會**反覆撞上第 2 項**：每接一章就要打一場沒有完整回合生命週期的戰鬥 | 逐章推進，每章一個可重播 checkpoint |
...
#### 原版 oracle 的現況（2026-08-18）

擷取環境收在 [`tools/dos-oracle.sh`](tools/dos-oracle.sh)：Docker `dosbox-run:latest`
＋ Xvfb ＋ xdotool，`START.EXE STING Wooden` 測試模式繞過翻譯輪（spec 530），
支援連拍與**逐鍵軌跡**（`frames=0`，一輪就看得到路徑上每一步的畫面）。

**EGA 已經解掉。** 關鍵不是設定檔也不是時機，是**按鍵要按住再放開**
（`keydown` → `sleep 0.25` → `keyup`）——瞬時的 `xdotool key` 在開機那幾題
文字模式提示上會整批被吃掉，看起來就像「文字模式送不進按鍵」。改掉之後
三道提示（顯示介面卡／音效／存檔路徑）全部答得進去，畫面是 16 色 EGA。

⚠ `CURSE.CFG` **不存這些答案**（實測永遠 0 byte），它只是「已設定過」的旗標；
留著它就會用預設值 CGA 起動。本工具預設每次刪掉它。

⚠ 判斷畫面**不要只看顏色數**：顯示介面卡那題與音效那題的像素統計幾乎一樣，
只看數字會把「已經前進一步」誤判成「按鍵沒進去」，白花好幾輪。

**還沒到終點：選單導航。** 已走通到「建角 → 存檔 → 加入隊伍」的路徑
（逐鍵記在工具的註解裡），但**定時送鍵的序列很脆**：任何一步的載入時間漂移
會讓後面每一個鍵錯位，而錯位的鍵可能剛好按到 `EXIT TO DOS`。約 50 輪擷取
之後結論是——要穩定重現得改成**看畫面決定下一鍵**（底部選單列的像素指紋
就足以當狀態判別），不能再靠加長固定延遲。
...
- 所以提爾佛頓街道的第一人稱畫面**只能靠實際建隊走進城**拿到。

已經走了一段的是「用 remake 產原版存檔」（spec 1133）：

- `cmd/dos-save-export` 會產生 `savgam<槽>.dat` ＋ 角色檔。
- 途中拿到**第一份原版自己寫的存檔樣本**（`BOB.GUY` 422 bytes／`BOB.FX` 27 bytes），
  收進 `docs/reference/original-dos/save-samples/`，並用它驗過 remake 的原版解碼器
  ——spec 102 當年要的「實際 sample bytes」到手了。
- 樣本第一眼就抓到一個 remake 的錯：存檔裡那 41 bytes 的角色檔名欄是
  Turbo Pascal 的 `string[40]`（長度位元組在前），remake 寫的是原始位元組。
  ⚠ **自洽的往返測試對它是盲的**——remake 讀自己的槽根本沒讀那一欄。
- ⚠ 順帶記下一個規格落差：DOS 前綴的 ECL 區塊，spec 181（decompiler 輸出）是
  `1E00h`、spec 1072（PC-98 逐條讀完）是 `1E41h`，差 65 bytes。本輪不裁決。

**還沒成功**：原版仍然拒絕合成的存檔（按下槽的字母就回主選單）。已排除前綴長度、
角色檔來源、檔名欄的長度位元組三個變因；還沒查 `GameState`／`SetBlocks` 的合法值
與存檔裡角色檔的副檔名。

要繼續的話兩條路：把 DOS 側的 `SaveGame` 逐條讀完（最直接），或改走狀態機驅動的
鍵盤導航（繞開存檔這條路）。

⚠ 第 1 項刻意排在最前面**不是**因為它最重要，而是因為它便宜且會**改變後面的判斷**：
沒有原版畫面，「UI fidelity」這一列永遠只能寫「尚未逐張比對」。
