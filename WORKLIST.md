...
下一個最小可重現工作：**把 `ECL2/0x04:02D7h`「被押去見首領」那一類接起來**
（spec 1156）——它是**沒有 `C04D` 的真實移動**，remake 現在整個跳過，玩家看得見
人不會被移過去。判準是**逐處分類 spec 1155 那 23 處**：哪些是真實移動（要接）、
哪些是戰後退回（要等 `7EC7h` 有 producer）、哪些是別的。分類完才知道 `C04D`
條件該換成什麼，而不是整個拿掉——整個拿掉會讓「戰後退回」那幾處在 remake
永遠成立（`7EC7h` 沒有 producer ⇒ 比較永遠不真，但投影還是會發生）。
