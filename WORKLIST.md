...
更新日期：2026-08-24（第 701 輪：**四個「卡住」，沒有一個是幀數不夠**。（一）巫師塔那四格是 `GEO5/0x33` 的**單向環**（`(4,3)→北→(4,2)→東→(5,2)→南→(5,3)→西→(4,3)`，四面朝外全是牆），而重放端的 `routeDead` 只數「路線說走得通、按下去沒動」⇒ 環裡每一步都真的走得動、永遠不會被判死；2,600／6,000／15,000 三個幀數上限**停在同一幀**。補走位放手條件（耐心量過取 2）。（二）`tryDoors` 是「轉一圈、每一面按一次前進」，站在只有一面是門的死角時第一次前進就沿著**開著**的那一面離開了 ⇒ 尤拉什 `(4,7)(4,8)(5,7)` 繞了 1,700 幀、門選單一次都沒開；改成只撞走不通的方向，撞到門 1 → 29 次。（三）`chooseHeading` 算鄰格沒繞回來：`(15,0)` 往北算成 `(15,−1)`，而隊伍真的踏上去是 `(15,15)` ⇒ 兩邊 `visits` 永遠 0、互相看起來都是「去得最少的」⇒ 上下邊界乒乓 3,500 次。（四）**下水道訓練場把隊伍丟回主選單**：`PROGRAM 0` 的判準用「腳下地形碼 `8C`」，而同段訓練場其實**任何一格隱密房間**（地形 `8E`、17 格）第一次踏進去就會問 ⇒ 條件不成立 ⇒ ECL 段變 `0x00`、之後每步空轉。正確判準是 `7EA8h == 7Fh`（全 corpus 四處 `PROGRAM 0` 前面都寫它）。**另補**：game pack 少宣告尤拉什 `(11,0)` 往北的出口（`COMPARE C04B,0Bh → NEWECL 11`）；按鍵測試的 app 沒載六章 `MON*CHA` ⇒ 走到雅麗亞絲噴「has no MON3CHA Player record」，**是測試自己沒載**不是 remake 缺實作；四個沒翻到的選項（`ATTEMPT TO SNEAK PAST`／`MOVE NORMALLY`／`BARGAIN`）。**第一人稱**：第二次全量 1,474 張／1,461 張逐格相同／389 格，13 張的成因是 spec 1006 那條「出界且內側沒牆 ⇒ 上一段歸零」引擎沒抄；⚠ 第一版把「內側」取成掃描自己的方向，全量跑到 913 張時是**變好 7、變壞 29**——而引擎那條測試**左右取反照樣全綠**（測資兩面都是 0）。內側是朝畫面中央那一面；改對之後 13 ＋ 29 共 42 張逐張重驗全部 0 格，測試也改成分得出左右。⚠ 全量比對因時間成本停在 913／1,474，可引用的全量數字仍是第二次那一輪）
