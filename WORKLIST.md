...
| **P1** | **ECL 的 9 個 `partial` opcode** | 1,024 條指令：`CLEARMONSTERS` 206、`PICTURE` 199、`COMBAT` 199、`CALL` 168、`PRINT RETURN` 120、`TREASURE` 63、`DAMAGE` 24、`LOAD PIECES` 23、`LOAD FILES` 22 | handler 的位址與條數在 [`docs/audit/ecl-opcode-handlers-dos.md`](docs/audit/ecl-opcode-handlers-dos.md)——**照成本排順序**（✅ `33h PRINT RETURN` 已讀完（spec 1147，缺口在 UI 行模型）；✅ `0Eh PICTURE` 也讀完了（spec 1148，缺口收斂成表現層三項）；✅ `24h COMBAT` 的分派順序也照抄了（spec 1149）；✅ `2Dh CALL` 七支全部讀完（spec 1150，缺口收斂成 `2E10h` 的髒旗標模型）；✅ `27h TREASURE` 也讀完了（spec 1151，修掉隨機表的區間 bug 與清單順序，造物品常式也接上了；缺口收斂成「多筆 TREASURE 原作覆寫、remake 相加」一項）；✅ `2Eh DAMAGE` 也讀完了（spec 1152，三種目標形式與「連打 N 下」那一支都接進正式路徑；缺口收斂成 corpus 走不到的「單體但隨機挑一名」）；✅ `37h LOAD PIECES` 也收尾了（spec 1153，牆面參數接進存檔；缺口收斂成兩條 corpus 走不到的路）；✅ `38h PROGRAM` 已轉 `done`（spec 1154，結局過場照原作的等鍵位置分五頁播完））；每一個都要 producer→state→consumer 三段齊全 |
