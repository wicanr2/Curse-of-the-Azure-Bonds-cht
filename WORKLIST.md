...
| **P1** | **怪物特殊攻擊五碼**（第 716 輪新開）| 真的缺 **5 碼／8 筆**：吐酸 `56h`×1、區域吐酸 `5Ah`×2、凝視 `7Eh`×2、噴火 `80h`×2、龍息火 `83h`×1（`monster-ai-coverage`）| handler 全部已逐條讀完（spec 720／722／723／725／735；分派歸屬 spec 1202），是轉錄工作不是新 RE：選目標模式 `41h`／`3Dh`／`33h`／`24h`、傷害旗標、次數欄 `arg_2^[3]`；視覺用既有槽 18／19／23 |
| **P1** | **實跑覆蓋的未蓋段**（第 716 輪新開）| 整段 0%：序章 `0x52`、希爾斯法 `0x15`、洞窟 `0x35`、`0x45`；另魔法商店 `0x25` 0.1%、酒吧 `0x23` 17.8% | 補實跑路線（新測試，不是新盤點）；大叢集清單在 `docs/audit/ecl-run-coverage.md`，跑法寫在報表開頭 |
