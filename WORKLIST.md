...
| 2 | `RE-04` | 劇情與全地圖事件的逐格盤點。**副作用的分母已建立**（`docs/audit/ecl-effect-coverage.md`）：可達 14,177 條指令中 `done` 13,097／`partial` 1,057／`consumed` 23。依出現次數排，✅ **`1Ch CLEARMONSTERS`(206) 逐條讀完了**（spec 1145：它連同還沒領走的戰利品一起丟掉，remake 已跟上；`partial` 只剩 `7603h := 8` 的語意）。其餘缺口是、`0Eh PICTURE`(199)、`24h COMBAT`(199→RE-06)、`2Dh CALL`(168→RE-03)、`33h PRINT RETURN`(120)、`27h TREASURE`(63)|
...
| ECL 副作用 | 可達 14,177 條指令，`done` 13,120／`partial` 1,057／`consumed` **0** | `cmd/ecl-effect-coverage` |
