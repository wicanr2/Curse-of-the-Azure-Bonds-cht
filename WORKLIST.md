...
## 2026-08-31 下一輪第一優先（使用者指定）

- [x] 重新擷取 `README.md` 內左上角場景有偏移的 CoAB 遊戲畫面。先由正常遊戲路徑
  重現同一狀態，確認 88×88 scene 在內框中的填滿、裁切與座標，再替換 README 引用；
  不得只後製搬動既有 PNG，也不得把舊截圖的偏移誤稱為原版留黑。
- [x] 替換後逐張檢查 README 實際引用檔、截圖 manifest、原版／手繪 theme 及文字是否
  被框線遮住；以重生指令與新雜湊留下收據。
  完成收據：`docs/audit/readme-scene-recapture-2026-08-31.md`。開場舊 segment 命令的
  GEO 缺口另列，不拿錯誤 checkpoint 覆蓋現行 README 圖。

## 2026-08-29 現行 UI 設定收尾（使用者已確認）

- [x] `F7` 開啟設定；`Esc` 取消、`Enter` 套用、`R` 恢復預設。
- [x] 手繪 theme 提供 A／B／C 三款框線；A 為預設，B／C 必須由可重生正式素材產生，
  不直接採用 AI 比較原型。
- [x] 手繪外框可調 `4..20px`（步進 2，預設 10）；內框可調 `1..12px`
  （步進 1，預設 8）。原版 theme 永遠恢復原版框線；切回手繪 theme 時恢復先前設定。
- [x] 閱讀文字與介面文字的大小、粗體分開設定；大小皆為 `12..36px`、步進 1，
  預設分別 24px／16px。
- [x] 框線與文字的硬性驗收：框線不得遮字；文字先安全換行，狹窄欄位仍放不下時
  才局部縮小顯示，不改寫玩家保存的期望字級。
- [x] 舊 `coab-ui-settings/1` 缺少新增欄位時安全套用預設；同步 Help、README、
  `CONTEXT.md`、spec、四語系字串、正式截圖與抽樣測試。
...
spec 1246 已 `CONFORMED`；本作新建角色現在依原版外層流程「獨立儲存 → Curse
...
| **P2 進行中** | 三平台發行 | `1.0.3-20260901` 以獨立 engine `8cc96f8650ef` 重生 patch／full-local 各 **4 件**：Linux AppImage、Windows ZIP、macOS x86_64／arm64 ZIP；八件雜湊、公開邊界與 full-local 遊戲資料均已驗證，GitHub Release 五個附件已由 API 讀回對帳 | Linux AppImage 與 Windows EXE（Wine）已啟動與繁中截圖驗收。Windows／macOS 真機、macOS 簽署／公證與對外影片音樂權利尚待收尾 |
...
換曲點 13／13、前端沒有路可以到的 `State` 進入點 0、
...
| **P1** | **音樂／音效的 cue 綁定** | ✅ **播放生命週期接完了**（spec 1192）：三格狀態的 17 處寫入收斂成 **4 種動作，4 種都發得出來、4 種玩家都碰得到**。「停」不是劇情資料而是**按鍵**（原作派曲常式查不到就 `ret`，唯一會停的是玩家關音樂）⇒ 補的是 **Ctrl+S 音效／Ctrl+O 音樂**兩個全域開關，不是 pack 裡的 binding。✅ **換曲點對回事件 13／13**：7 個查 `CURRENTECL` 派曲表（`ecl_blocks` binding）＋ 6 個事件驅動（**開戰**、開戰的 **`47h` 分岔**、**開場**、**角色建立**、**全滅**、**結局**，各用 `context` binding／cue，因為原作那幾處**不看場景**）。分母釘在 `TestEveryOriginalMusicChangePointHasARemakeCounterpart` 的表裡，不是敘述。✅ **DOS 側結案：DOS 版沒有 BGM**（94 個成員逐一列舉，沒有 `MSCDRV.EXE` 也沒有音樂資料檔；配正對照） 結局那一首掛在 `beginEndingScene()`（原作 PC-98 `overlay-18:168Dh` 寫在結局文字**正上方**）；全滅那一首掛在 `finishCombat()` 的 `StatusEnemyWon` 分支（原作 POSTCOM `1955h`，判準是 `PARTYDEAD && !ADUEL`，而非全滅那一條在 `18F6h` 直接跳過換曲點） | ✅ **「戰鬥 phase 的音樂同步」查掉了：原作沒有這回事**——位元組直掃戰鬥模組（`overlay-08` COMBAT／`overlay-13` COMSTUFF／`overlay-32` TACMAP）：`MSCPLAY` **0 處**、`SOUNDFX` 12 處，`MSCSTOP` 也只有常駐那一處（玩家關音樂）⇒ 曲子在 `INITCOMBAT`（開打前）選定，整場戰鬥只有音效；★ 那個 0 有正對照（同一次掃描在同幾個模組找到 12 處音效）⇒ 不是掃描的假零。✅ 兩個畫面都接上了：全滅走 `enterPartyKilled`（全滅畫面→回主選單，spec 1152／1045／1095 的 `DS:4FC7h` 兩個來源同一個結局），結局走 `beginEndingScene`（五頁過場→存檔詢問，spec 1154／1082）。✅ 全滅判準第 722 輪對齊（spec 1204）：戰後閘與全滅曲換曲點改用 POSTCOM 的 `PARTYDEAD` 狀態集合（昏迷／瀕死不算全滅，隊伍活下去）；ECL DAMAGE 收尾的「沒有人能行動就全滅」（昏迷也算倒）先前沒接、本輪接上。✅ 傷害放倒的狀態階梯第 723 輪收掉（spec 1205）：`14Ah:0ACh`＝overlay-24 的 `SAVEDAMAGE`（戰鬥與 ECL 共用），戰鬥路徑接上（`DownOverkill`＋`syncCombatDownStatuses`，戰鬥擊倒會標昏迷／瀕死／死亡、瀕死可包紮）|
| ✅ **P1** | **UI 與原版 fidelity（收斂）** | 五張 manifest 圖的圖層對齊已收（spec 1130／1131／1132）。第 726 輪全量重測：`index.tsv` **1,498/1,498 張逐格相同、差 0 格**（17 張圖全等）；簽章覆蓋 `cmd/fp-screen-plan -covered` 實測 **585/585、unresolved 0**。歷史殘差（3,552 格、相鄰色階）是 engine viewport 邊界修正（`f2676e9`..`39d1ac4`）落地前的過期量測；方法與逐輪收斂記錄在 spec 1185 | —（天空色、牆磚選圖、調色盤各自的比對均已在 spec 1185 各節收掉）|
...
| P1 開著的 | 缺什麼 | 下一步要的東西 |
|---|---|---|
| ✅ ECL 的 `partial` opcode（收掉）| `partial` **0**（第 713 輪 `2Dh CALL` 轉 `done`）。`PRINT RETURN` 七處看得見的空行 2026-08-23（`f45a31fb`）補進兩語系譯文，`TestBlankLineMessagesStillFitTheEventBox` 守版面（清單由 `ecl-print-return.json` 掃出、行寬按字寬算、`maxLines` 超框變紅）；`CALL` 四張宣告值第 707 輪拆宣告實驗判掉並刪除、兩個入口落點第 711 輪原版判掉並修掉 | `ViewMirror.Block` 是**文件化的刻意邊界**（spec 1184「明確不宣稱」：三張 `area-default` 載檔補值仍靠它擋更早執行殘留的舊座標），不是待辦 |
...
| UI 與原版 fidelity | ✅ **已擷取範圍收斂完畢**（第 726 輪全量重測）：`index.tsv` 全部 **1,498／1,498 張逐格相同、差 0 格**（17 張圖全數全等，含先前殘差的 `geo5-b32`／`b33`／`e31-b32`）——「737／766、差 3,552」是 engine viewport 邊界修正（`f2676e9`..`39d1ac4`）落地前的過期量測。簽章覆蓋 `cmd/fp-screen-plan -covered` 實測 **585/585、unresolved 0**（「382/585」同樣是過期數字）| —（本列收斂；天空色／牆磚選圖／調色盤各有自己的比對，均已在 spec 1185 各節收掉）|
...
一項要**表現層的模型**，而 UI fidelity 第 726 輪整列收斂（全量 1,498/1,498 逐格相同、簽章 585/585）。
...
| 缺口 | 現況 | 為什麼還沒排 |
|---|---|---|
| ~~第一人稱牆磚的逐格美術對照~~ | ✅ 第 726 輪全量收掉：17 張圖 1,498 張與原版逐格比對**全部相同**（spec 1185）| — |
| **SPRIT 畫布相對於戰場格的錨點** | 戰場圖示改回 CPIC 之後，SPRIT 只在「沒有 CPIC」時才會走到。二十個 checkpoint 的素材普查是**零筆退回** ⇒ 目前跑不到那條路 | 跑不到的路徑先不投資；等真的有怪物沒有 CPIC 再量 |
| ~~第一人稱的天空層~~ | ✅ 已收：天空選色由各段進入碼寫 `4BFDh`／`4BFEh`（`cmd/ecl-sky-colours` 掃成表，載檔閘門判定 23 處／11 段），`geo2-b04` 的紅天花板實測 16／16 全等（spec 1185 天空選色兩節）| — |
...
| 建議 | 工作 | 為什麼是這個順序 | 粗估 |
|---:|---|---|---|
| 1 | **原版第一人稱 oracle**（✅ 完成，spec 1134；後續擴到全圖並於第 726 輪全量收斂：17 張圖 1,498 張逐格全等、簽章 585/585，spec 1185）| 提爾佛頓五格 × 四個朝向逐格比對：19/20 完全相同、1 張差一格（那一格也在後來的全量重測歸零）| 每張地圖約 20 次擷取 ＋ 20 次 remake 截圖，各約一分鐘 |
...
✅ 這一節的「還開著的」後來全部收掉（記錄保留在下方，結論如下）：
其餘 GEO 地圖的逐格比對＝第 726 輪全量 1,498 張全等；`(1,7)` 面西那一格
在全量重測裡差 0；天空色的「誰寫 `1FAh`／`1FCh`」有答案了——**各段自己的
進入碼寫 `4BFDh`／`4BFEh`**（`cmd/ecl-sky-colours` 掃成表，spec 1185），
runtime watchpoint 不必做。以下保留當時的查證過程：
...
2. ~~`INTERPET` 內部函式盤點~~ ✅ 已收：`ecl-handler-operand-audit.md` 現況是
   64 個 opcode 中 62 一致、2 個不一致都是**變長選單**（`15h`／`2Bh`，
   parser 特別處理，報表已判定「不是錯誤」）。「29 筆待讀」是過期數字。
3. ~~external `CALL` registry selector 層~~ ✅ 第 561 輪完成；7 個分支主體第 713 輪全部接上。
4. ~~packed text 長度規則與 bank 1 計算 routine~~ ✅ 已收：`FINDSTR` 兩平台
   讀完（spec 616 PC-98 逐 byte／spec 992 DOS 6-bit 壓縮），bank 1 計算
   routine ＝ `CHECKSPECIALS`（spec 624 PC-98／1040 DOS，含 `7CA5..7CACh`
   範圍、`7ECFh` 換算表、`7D0Dh` 讀未初始化值等全部分支）。
   第 720 輪對 PC-98 三支（FINDSTR／GETSTR／CHECKSPECIALS）獨立重讀一次，
   與既有 spec 逐項相符（正對照，無新差異）。⚠ `ecl-bank1-field-map.md`
   的三筆（`7CACh`／`7ECFh`／`7F3Eh`）是抽取器的簡化投影，完整語意以
   spec 624／1040 為準——表頭已註記。
5. ~~未定義區段判定（`RE-11`）~~ ✅ 第 721 輪機械分類收掉大半
   （`cmd/re-gap-audit`，spec 1203）：DOS 929 段／16,065 bytes 中
   待人工只剩 187 段／5,818 bytes；PC-98 388 段／20,321 bytes 中剩
   95 段／7,326 bytes（其餘＝碎屑／`00h` 填充／文字，含 Shift-JIS）。
   第 724／728／729 輪四刀（函式台帳交叉比對、截斷函式尾、碼內常數
   引用掃描、殘餘 38 段逐段一手判定進 `re-gap-manual.json`）：
   36,386 bytes 全數有機械分類或人工判定，**兩平台「無判定」0 段**。
   台帳在 `docs/audit/re-gap-audit.md`；RE-11 關閉。
