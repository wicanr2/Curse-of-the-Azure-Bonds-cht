...
更新日期：2026-08-24（第 705 輪：**修掉「全滅重開帶著上一局章節」的缺陷，主線快照走訪收到 114／114**。（1）上一輪列的 P1「快照走訪 5 份推不動」查清並修掉，但**成因與上一輪寫的完全不同**：不是 `ECL6/0x45:0557h` 的 `7F80`——那一段靜態上根本沒有繞過 `0x047D` 的路（無直接跳入、六個 `ON GOTO` 表、五個生命週期入口逐一排除）。真正的失敗點在 `ECL2/0x01:03FF` 的商店 TREASURE（第八運算元是**立即值** 0x01）：那五份快照的走訪打輸全滅 → 返回標題 → 重開新局，而 `resetSessionForNewGame` 只重設 session 與隊伍，`Area.GameArea` 留著章 6；全新開局不經過「ENTER CITY」那條會改章的路，於是新隊伍帶著章 6 走進提爾佛頓——GEO 載成「找不到 GEO6 block 0x01」、商店拿章 6 查 ITEM 區塊 1 而整個 `Update()` 回錯。修法：`resetSessionForNewGame` 一併收回 seg001.Init 的開局值（章 2、GEO2/0x01、(7,13) 朝北）並發 GEO 重載請求；回歸測試 `TestReturnToTitleAfterPartyKilledResetsCampaignArea` 釘住，走訪測試的「相異成因上限」從 1 收緊到 0。（2）教訓寫進 AGENTS.md §3：**錯誤訊息裡的位址是「消費失敗的位置」，不是「值寫錯的位置」**——上一輪從報錯的 TREASURE 反推到 `0x45:0557h`，而執行期 trace 顯示那條指令根本沒跑過；定位這類「讀到舊值」要先用 runtime trace 確認失敗那條指令真的是它，再談控制流。）
