...
| **P0** | **戰鬥回合生命週期**（`RE-06` → `ENG-07`）| ✅ **逐項對照收完了**（spec 1135／1136／1137／1138）：先攻算式、選誰動、`20 → 19`、DELAY、GUARD、QUICK、定身、面向與背後攻擊、機會攻擊全部已接且與原作相符；唯一沒接的突襲 −6 **已判定為死碼**（spec 1136）。⚠ 剩下的是 ECL `24h COMBAT` 的 199 處 `partial`——那指的是**遭遇編成的參數**（隊形、圖示、旗標）還沒逐處對過，不是生命週期。`+07h`（保留的機會攻擊）不是缺口：remake 用 `CombatAction.Guarding` 表達，三端都對得上 spec 1136 且存得下去讀得回來 | `24h COMBAT` 的參數依出現次數逐處對 |
