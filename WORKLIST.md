...
下一個最小可重現工作：**重新推導火刃巢穴那三條路線**（spec 1157）。分類已經
做完——那 23 處全部是真實移動，`C04D` 條件擋掉的是玩家看得見的位移，所以條件
該整個拿掉；擋著的是三條 `TestRealNewGame*` 路線測試，而它們是照著**被壓掉
之後**的行為寫出來的（路線註解自己說「intentionally crosses the blade barrier
and frozen-room cells」，而原作在那些格子會把隊伍推回去）。

驗收：拿掉 `C04D` 條件、三條路線重新走得通，而且**每一步的預期座標都要由原作
行為推出來**（不是把失敗值抄進期望值）。⚠ 這一項會動到主線通關測試，先確認
`ECL2/0x04:160Ch` 那六個入口各自的觸發條件，否則會用試誤法湊路線。
