...
更新日期：2026-08-17（第 568 輪：`ENG-09` 分母歸零後，使用者指定第 2 批：等級吸取來源、visual RE、sound 覆核）
...
| # | 項目 | 為什麼要做 | 完成的樣子 |
|---:|---|---|---|
| 2-1 | **等級吸取的來源** | spec 1125 解出 `+0E7h`／`+0E8h` 並接上復原術，但**沒有任何東西寫這兩格**——規則對了，來源還沒接。攻略／手冊裡記載有怪物會讓等級下降 | 找出是哪個效果碼／怪物能力在吸等級，接上寫入端；復原術因此有東西可還 |
| 2-2 | **visual RE** | 覆蓋台帳的 `visual` 欄 52 支 `partial`、16 支 `missing`。runtime 有排視覺事件，但 pack 只宣告 13 筆視覺資產 ⇒ 缺的是**原作的逐法術視覺資料**，不是接線 | 找出法術 → 視覺資產的對應表，逐支進 `combat_visuals`；台帳的 `visual` 欄收斂 |
| 2-3 | **確認 sound 真的修上去** | 上一輪把稽核從「只讀 `combat_state.go` ＋ 靠函式名猜 behavior」改成掃整包＋跟分派表，sound 由 26／73 變成 73／73。**數字自己會動不等於修對了**，要有一條獨立的證據 | 不靠同一支稽核工具，另外驗證每一支宣告過的法術施放時真的發得出聲音 |
