...
| 建議 | 工作 | 為什麼是這個順序 | 粗估 |
|---:|---|---|---|
| 1 | **原版第一人稱 oracle** | 最便宜的收尾：它同時解掉上表三個小缺口裡的兩個（牆磚逐格、天空層），而且**環境已經有了**——`dosbox-run:latest` 映像在，spec 530 已證明 `START STING Wooden` 測試模式可以繞過翻譯輪進到遊戲內 | 每次擷取約 30–60 秒；成本在把鍵盤序列走到指定格的迭代，約 10–20 次 |
