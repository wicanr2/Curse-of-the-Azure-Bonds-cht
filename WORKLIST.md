...
5. ~~未定義區段判定（`RE-11`）~~ ✅ 第 721 輪機械分類收掉大半
   ...
   第 724／728 輪補上三刀機械判定（函式台帳交叉比對、截斷函式尾、
   碼內常數引用掃描）：36,386 bytes 收到**真正待人工 38 段／914 B**
   （DOS 21 段／148 B 全是 ≤34 B 碼碎片；PC-98 17 段／766 B，最大是
   overlay-11 的 96 B byte 表）。`overlay-22 48D4h` 判定為邊界碎片
   函式的截斷尾（法術 handler 收尾），不是漏碼。逐段台帳在
   `docs/audit/re-gap-audit.md`。
