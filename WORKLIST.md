...
下一個最小可重現工作：**把 GEO 目錄交給 `State`**（spec 1161）。原作的
`2E10h` 重畫會自己去地圖讀目前這一格的牆面／地形寫進 `C04E`／`C04F`，而每格
事件靠 `C04F` 分派；remake 的 `State` 讀不到 GEO，所以腳本傳送之後那兩格是舊的。

驗收：`projectFreshDungeonCoordinatesBeforeCall` 投影完座標之後能照原作重算
`C04E`／`C04F`，而且不必由呼叫端傳 `grid`；補一條兩面測試（傳送到另一格之後
`C04F` 要換成新格子的地形，沒傳送就不動）。

做完之後再套 spec 1160 那六件事（① `Spawn` early return、② `2E10h` 守衛逐條看
`CallRequests[i].BlockID`、③ `C01Eh` 讀腳本的 `C04D` 並寫回、④ 重畫消費座標
寫入、⑤ spawn 改後備、⑥ 拿掉 `ECL2/0x02` 的座標位移），九處原作行為的斷言與
巫師塔、眼魔洞穴的走法都已經推導好（spec 1160／1161）。走訪器那三條規則也要
一起加。⚠ 查「誰跳到這個位址」一律用 `cmd/ecl-window -into`。
