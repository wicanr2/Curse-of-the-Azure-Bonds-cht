...
| P1 開著的 | 缺什麼 | 下一步要的東西 |
|---|---|---|
| ECL 的 3 個 `partial` opcode | 可達指令 14,177 條裡 `partial` 還有 **487**（`PICTURE` 199／`CALL` 168／`PRINT RETURN` 120）。三個缺口的**大小都量出來了**：`PRINT RETURN` 是 110 個換行段落裡 **10 段會空行、7 段玩家看得到**（`cmd/ecl-print-return-audit`）；`PICTURE` 的 `4FBAh`／`4FBBh` 已解（＝現在／上一次的畫面模式）⇒ 旁路是「前後都還在第一人稱就不重繪」；`CALL` 剩 `2E10h` 髒旗標的**四張宣告值** | `PRINT RETURN` 那七頁全部由 `text_rule` 服務、UI 的 `wrapTextLines` 本來就把 `\n` 當段落切 ⇒ **改的是那七則譯文的版面**，要配截圖驗收（訊息框有 `maxLines` 上限）。`CALL` 的四張宣告值仍要原版畫面當 oracle |
