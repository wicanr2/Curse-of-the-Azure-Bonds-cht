...
更新日期：2026-08-25（第 711 輪：**第一人稱 fidelity 覆蓋收滿**：585 種牆面配置全部有原版畫面可比（最後 6 種補拍 `geo5-b35`／`geo6-b40` 各三格 × 4 朝向共 24 張，`tools/fp-capture-plan-710.sh`），24 張與 remake 逐格比對**全部相同（差 0 格）**；`fp-screen-plan.md` 重生為 585／585、貪婪清單空。第 710 輪：**按鍵重放的世界地圖死環拆掉**（spec 1201）。「卡住換下一項」夾在最後一項會被紮營「修改」×「改名」兩個選單互踢鎖死（12,000 幀有 11,000 幀在環裡）；改成環繞後同路線同幀數 137 → **314 格**、5 → **8 段**（重返巫師塔），無路線預設 94 → **309 格**，落回原文 0。下一個瓶頸換位置：`ECL2/0x03` 爬牆事件是盜賊技能的閘，六個預設戰士永遠失敗。第 709 輪：戰鬥佈陣演算法解讀完並轉錄（spec 1200，接線待做）。第 708 輪：段內支線接進主線，段 subtest 23 → 25。）
...
|---|---|---|---|
| **P1** | ECL 的 1 個 `partial` opcode | 可達 14,177 條裡 **168**（`CALL`；`PICTURE` 第 706 輪收掉）| 四張宣告 spawn **第 707 輪判掉並刪除**（拆宣告整套測試不變綠 ⇒ 宣告值是腳本執行期落點的抄寫，spec 1184 增補）。`CALL` 剩 `ViewMirror.Block` 這一格：兩個入口第 711 輪用原版**判掉了**——原版落腳本寫的第 0 列（下水道 (0,0)、火刀據點 (8,0)），remake 的 (0,1)／(8,1) 是偏差（spec 1184）⇒ 下一步是修落點偏差＋重評 Block 拆除（`area-default` 三張仍靠它），並重驗主線 25 段 |
