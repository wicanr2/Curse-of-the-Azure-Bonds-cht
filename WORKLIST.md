...
更新日期：2026-08-25（第 709 輪：**戰鬥佈陣的原作演算法解讀完並轉錄成模組**（spec 1200）。PC-98 COMPREP（overlay-10）`17EEh`／`1364h`／`1220h` 逐指令讀完：佈署模板是 DSEG `702h` 的「每列欄範圍」表（5 種形狀×6 列，形狀依隊的方向組選、組 1 一律形狀 4）；兩隊起點＝我方 (0,0)、敵方 遭遇距離×方向 delta（`player^[582h]`，SETUP MONSTER 寫的）；occupancy `78AAh` 是 2 隊×4 組×6×11，放好寫 0；找空位 `1364h` 是理想格為中心的狀態機掃描（種子表 `6F2h`/`6FAh`、軸向 `6EEh`、失敗位移、地城牆面調整、四組升級、放不下就把戰鬥員從戰鬥移除）；座標 x＝teamY×5＋teamX×6＋欄＋22、y＝teamY×5＋列＋10，與 round-88 的 reference 公式逐項相符。`internal/combat.Deployment` 忠實轉錄（含手算落點的釘位測試）。**接線留待下一輪**：把 `StartEncounter` 的 fallback 換成它，要接遭遇距離／地圖朝向／地面檢查／地城牆面，並重驗主線 25 段的戰鬥。第 708 輪：段內支線接進主線（盜墓者、下水道口），段 subtest 23 → 25。）
