...
| 3 | `RE-06` → `ENG-07` | 戰鬥回合生命週期。**回合開始那一段的逐項對照已完成**（spec 1135）。**逃跑已接**（spec 1112／1113）：走出邊界即脫離，比「我的移動率」與「敵方最快移動率」，平手擲 1d2；新增 `StatusPartyFled`。**突襲量到「沒有人設遮罩」**：36 個 overlay 裡對 `+596h` 只有先攻讀它與一處清 0，常駐段那一側還沒掃（正對照失敗，見 spec 1113）。**剩兩項**：突襲遮罩沒有人寫，以及回合開始那三個歸零（動作計數／累計轉向／保留的機會攻擊）在 remake 沒有對應欄位 |
...
| 2 | **戰鬥回合生命週期**（`RE-06` → `ENG-07`）| 逐項對照做完了（spec 1135）：先攻算式、選誰動、`20 → 19`、DELAY、GUARD、QUICK、定身**七項都已接且與原作相符**；修掉一個「玩家控制的隊員套不到 `CHECKFX(07h)`」的缺口。**剩兩項**：突襲遮罩沒有人寫（spec 1113），以及回合開始那三個歸零（動作計數／累計轉向／保留的機會攻擊）沒有對應欄位 | 突襲要先找到遮罩的寫入者；三個歸零要先決定機會攻擊要不要做 |
