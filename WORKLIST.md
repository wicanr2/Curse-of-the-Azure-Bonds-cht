...
更新日期：2026-08-24（第 703 輪：**下水道往火刀據點的入口宣告在一格走不出去的牆上，而測試綠了七十輪**。`gamepack` 的 `external_exit` 被 `CanMoveDungeon` 當成「這一格一定通」——**直接回 true，不看 GEO** ⇒ 宣告在 `(8,15,S)` 的交接把牆蓋掉了，正常玩家永遠走不到。真正的來源格是 `(10,15)`：腳本去程 `X := X − 2`、回程 `X := X + 2`，而 spec 537 自己就記著回程落在 `(10,15)`，**減二套錯了方向**。⇒ 據點入口 `(6,1)` → **`(8,1)`**。★★ 連帶翻出更大的一件事：`GEO2/0x03` **不是連通的**——以走路可達性算是三塊互不相通的區域（X 0..4／5..9／10..15），區域之間唯一的通道是走出圖外讓 `ECL2/0x03` 把隊伍傳送回同一張圖的另一個 X。少宣告那幾格圖緣，後兩塊（160 格）在 remake 裡是死區。⇒ 逐格宣告十一格圖緣（`confidence: exact`，spec 1199）。★ 同一輪修掉第二個儀器的洞：每格事件分派器只往**位移小**的方向找遮罩，而火刀據點（`ECL2/0x04`）的 `AND 3Fh, C04F → 4C05` 住在 `GOSUB 970Dh` 進去的子程式裡、位移比 `ON GOTO` 大 5,656 ⇒ 整個 block 落空，而落空跟「這個 block 沒有每格事件」在報表上一模一樣。改成照**執行順序**掃（`scanOrderBefore`）之後，火刀據點 33 個索引回到表裡：有地形分派的 block 16 → **17**、逐格實測 259 → **288**、演出來是中文 249 → **276**、落回原文仍是 **0**。走訪可達性同步重生：聯集 214 → **287**／288，未達成 44 → **1**（那一格逐格實測是 `played`）。★ 按鍵重放走到過的 ECL 段從 `0x01 0x02 0x03` 變成 **`0x01 0x02 0x03 0x04 0x50`**——第一次自己走進火刀據點並打到世界地圖；新瓶頸換成世界地圖（11,271／12,000 幀停在那裡）。⚠ 順手修一個玩家看得到的洞：改名選單把角色的**內部 ID**（`creation.human.fighter-3`）印在畫面上，改成隊列位置。）
