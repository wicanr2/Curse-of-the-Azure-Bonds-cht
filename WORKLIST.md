...
下一個最小可重現工作：**拿掉「宣告了 spawn 的地圖整張跳過」那道閘門**
（spec 1159）。它擋掉的是提爾弗頓整座城、贊提爾暗黑神殿與巫師塔的座標投影，
本輪已經量過拿掉之後會浮出哪六個原作行為，逐處的目的地也都由 ECL 推導出來了。

這一項要**三件事一起做**，缺一條主線就會停在公會門口：

1. 拿掉 `projectFreshDungeonCoordinatesBeforeCall` 開頭的 `Spawn` early return。
2. `2Dh CALL C01Eh`（`MOVEFORWARD`）改成讀腳本當下的 `C04D`，不要用
   `s.DungeonDirection`——公會抓捕動畫在 `ECL2/0x02:0D90h` 先
   `SAVE 7F7B C04D` 指定方向，走四步；朝向沒同步就會走錯方向。
   判準是 [spec 292](docs/spec/292-tilverton-carriage-guild-transition.md)
   量到的到站暫存器 `(1,12,0)`。
3. 跨 block 的執行（`SessionStartBlockID != SessionEndBlockID`）現在整批不投影，
   盜賊公會那兩處就在裡面；要改成逐條 `CallRequest` 看自己的 `BlockID`。

驗收：提爾弗頓那六個行為各補一條**兩面測試**（像
`TestFireKnifeFrozenRoomRetreatReturnsToPreviousCell` 那樣，搬與不搬兩邊都釘
——只釘一邊的話「所有重畫都不搬座標」也會過），主線通關測試的路線要跟著重新
推導（起點會因為退格而位移一格）。⚠ 查「誰跳到這個位址」一律用
`cmd/ecl-window -into`，`-operand` 看不到 `ON GOTO` 的目的地。
