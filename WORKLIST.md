...
| **P1** | **ECL 的 10 個 `partial` opcode** | 1,037 條指令：`CLEARMONSTERS` 206、`PICTURE` 199、`COMBAT` 199、`CALL` 168、`PRINT RETURN` 120、`TREASURE` 63、`DAMAGE` 24、`LOAD PIECES` 23、`LOAD FILES` 22、`PROGRAM` 13 | handler 的位址與條數在 [`docs/audit/ecl-opcode-handlers-dos.md`](docs/audit/ecl-opcode-handlers-dos.md)——**照成本排順序**（✅ `33h PRINT RETURN` 已讀完（spec 1147，缺口在 UI 行模型）；✅ `0Eh PICTURE` 也讀完了（spec 1148，缺口收斂成表現層三項）；✅ `24h COMBAT` 的分派順序也照抄了（spec 1149）；✅ `2Dh CALL` 七支全部讀完（spec 1150，缺口收斂成 `2E10h` 的髒旗標模型）；✅ `27h TREASURE` 也讀完了（spec 1151，修掉隨機表的區間 bug 與清單順序，造物品常式也接上了；缺口收斂成「多筆 TREASURE 原作覆寫、remake 相加」一項）；接下來依條數是 `2Eh DAMAGE` 305 條與 `37h LOAD PIECES` 104 條）；每一個都要 producer→state→consumer 三段齊全 |
...
下一個最小可重現工作：**讀完 `2Eh DAMAGE`**（305 條，`partial` 名單裡最貴的一支）。
remake 目前只結算「明確指定全隊」的封包（旗標 `0xC0`），其餘要選定角色的形式仍留在
pending——驗收是把選定角色那幾種形式的判準從 handler 讀出來，並用兩個方向釘住
（只驗全隊那一支的話，把選角那幾支整個刪掉也會過）。

之後照成本排：`37h LOAD PIECES`（104 條）、`38h PROGRAM`（104 條）。
