...
下一個最小可重現工作：**把那 8 處「明確移動」逐處接起來**（spec 1157 的分類、
spec 1156 的 `ECL2/0x04:02D7h`「被押去見首領」）。`C04D` 條件已經拿掉、15 處
退格全部生效（spec 1158），剩下的是那 8 處（`SAVE <常數> C04B/C04C` 傳送、
`ADD`／`SUBTRACT 1` 走一步）在玩家路徑上到底走不走得到——目前只有被四條路線
測試踩到的那幾處有實跑證據。

驗收：逐處給出「哪一段劇情、哪一格、走過去會被搬到哪」，踩得到的補一條**兩面
測試**（像 `TestFireKnifeFrozenRoomRetreatReturnsToPreviousCell` 那樣，搬與不搬
兩邊都釘——只釘一邊的話「所有重畫都不搬座標」也會過），踩不到的要說明被什麼
守衛擋著。⚠ 查「誰跳到這個位址」一律用 `cmd/ecl-window -into`，`-operand`
看不到 `ON GOTO` 的目的地。
