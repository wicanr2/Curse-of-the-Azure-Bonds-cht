...
下一個最小可重現工作：**答出 ECL 格 `4BF0`／`4BF1` 的值從哪來**（spec 1155）。
它擋著 `2Dh` 的最後一項，也擋著 spec 515／516 從第 515 輪就留到現在的未決項。
全 ECL corpus 只有兩處常數寫入（`ECL4/0x20` 寫 `(7,2)`、`ECL5/0x32` 寫 `(0Fh,5)`），
其餘 18 處都是把它們還原進 `C04B`／`C04C`——所以值來自 ECL 以外。三個候選：
原生側（spec 516 點名 DOS `overlay-22` 的 `[di+4BF0h]`）、跨 block 殘留、
`21h LOAD FILES` 載進來的初值。驗收是三條各自排除或證實，並說明第二章
（提爾佛頓）那幾處在第四章寫入之前拿到的是什麼值。
