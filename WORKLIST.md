...
下一個最小可重現工作：`partial` 名單只剩 9 個，照 handler 條數排最貴的是
**`2Dh CALL` 的 `2E10h` 髒旗標模型**（spec 1150 收斂出來的那一項）：原作是
`STOREVALUE` 當場寫 `720Fh`／`7210h`／`7211h` 並立五個髒旗標之一，`CALL` 只負責
「髒了才重畫」，而 remake 改用 `CALL` 當下回頭掃 `SaveWrites` 的啟發式。
⚠ 那個判準是第 402／403 輪的正常玩家路徑回歸逼出來的，換掉之前要先有能重現
「該重畫卻沒重畫」的測試，否則會用一個看起來更忠實的模型換掉一個真的有效的。
