...
5. ~~未定義區段判定（`RE-11`）~~ ✅ 第 721 輪機械分類收掉大半
   ...
   待人工逐段台帳（附 hex 前綴、依大小排）在
   `docs/audit/re-gap-audit.md`；最強「漏掉的程式碼」候選是兩平台
   同座標的 `overlay-14 0044h`（258 B）。逐段語意判定轉為平行細項，
   不再是無分母的總數。
