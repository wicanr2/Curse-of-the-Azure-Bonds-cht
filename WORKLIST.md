...
| **P1** | **存檔剩餘欄位** | ✅ **角色記錄的 `unknown` 歸零**（spec 1164）：422 bytes ＝ decoded 299／documented 123，PC-98 除錯符號展得開 `CHARREC` 的 79 欄。✅ **SAVGAM 的 ECL 記憶體已雙向接上**（spec 1163）：前三塊 ＝ 區 0／1／2。⚠ 剩下的是三處讀法待對（`+0E6h`／`+11Ch`／`+192h`）、DOS 比 PC-98 少的那一格確切位置、區 3（ECL 程式碼本身）不寫回、`MOVEPARTY` 跨遊戲轉移未做 | 對那三處讀法；轉移另開 |
