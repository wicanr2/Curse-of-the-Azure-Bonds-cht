...
| **P2 進行中** | 三平台發行 | `1.0.2-20260831` 以獨立 engine `0819c64e451d` 重生 patch／full-local 各 **4 件**：Linux AppImage、Windows ZIP、macOS x86_64／arm64 ZIP；八件雜湊、patch deny-list 與 full-local 遊戲資料均已驗證 | Linux AppImage 與 Windows EXE（Wine）已啟動與繁中截圖驗收。Windows／macOS 真機、macOS 簽署／公證與對外影片音樂權利尚待收尾 |
