...
| 解法 | 內容 | 代價 |
|---|---|---|
| **狀態機驅動**（建議）| 每按一鍵就擷取、用底部選單列的像素指紋判斷目前在哪個選單，再決定下一鍵 | 一次性把迴圈做穩，之後原版取證都受益 |
| **用 remake 產生原版格式存檔** | remake 已經有 `.SAV`／`.SWG`／`.FX` 的寫入路徑；產一份讓原版 `LOAD SAVED GAME` 直接進世界 | 省掉整段建角導航，但要先確認原版吃得下 remake 寫的檔 |
