...
5. ~~未定義區段判定（`RE-11`）~~ ✅ 第 721 輪機械分類收掉大半
   ...
   第 724 輪補上**函式台帳交叉比對**：待人工段大半落在已解讀函式的
   中段（IDA 匯出的 undefined ≠ 沒被讀過），真正函式外的只剩
   **DOS 64 段／2,276 B、PC-98 73 段／4,133 B**（大宗是夾佔位碼的訊息
   模板；函式外未定義碼最強候選＝PC-98 `overlay-22 48D4h`）。
   逐段台帳（函式外優先、附台帳欄與 hex 前綴）在
   `docs/audit/re-gap-audit.md`。逐段語意判定轉為平行細項，
   不再是無分母的總數。
