...
var OpcodeEffects = map[byte]OpcodeEffect{
	...
	0x1A: {EffectDone, "同上"},
	0x1B: {EffectDone, "同上"},
	0x1C: {EffectPartial, "ECL 看得到的部分已完整（怪物鏈與已放置數；原作另清的 47E6h／8B69h／7603h／6F8Ch 都不是 ECL 變數，見 ecl-shared-cells）。要收尾的是 remake 這側的怪物／圖示模型還有沒有該一起清的狀態"},
	...
	0x24: {EffectPartial, "COMBAT 是三選一的服務分派點（spec 1095）。回合生命週期已收（spec 1135～1138）。199 處分成 153 處真的要打（前面擺過怪）與 46 處走服務分派（商店／營地／神殿，見 docs/audit/ecl-combat-sites.md）；`partial` 指的是那 46 處在 remake 走的是別的機制，不是 24h 的請求旗標"},
