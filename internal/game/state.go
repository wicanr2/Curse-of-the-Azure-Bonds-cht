...
type State struct {
	...
	CreationOuterStep        CreationOuterStep
	CreationLibrary          party.Roster
	CreationLibraryCursor    int
	CreationSourceCursor     int
	creationLibraryPath      string
	...
	GuidedActive bool
	GuidedCursor int
	// SETACTIVEICON（spec 1037／1247）在姓名後編輯 READY／ACTION 共用的
	// 頭、武器與體型。Backup 三格讓 Exit 真的還原，而不是只離開畫面。
	GuidedIconCursor       int
	GuidedIconBackupHead   uint8
	GuidedIconBackupWeapon uint8
	GuidedIconBackupSize   uint8
	GuidedIconBackupColors [6]uint8
	...
	combatItemIndex int
	// combatScrollSpellIndex 是卷軸上三支法術裡目前選中的第幾支（spec 1171）。
	combatScrollSpellIndex  int
