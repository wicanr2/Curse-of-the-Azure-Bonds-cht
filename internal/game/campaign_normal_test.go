...
//
// ⚠ 份數不能無上限：走訪時間是「份數 × 選單策略數」。
//
// ★ **6／6 與 12／3 量過**：12／3 產 204 份（6／6 是 114 份），走出來的
// (block, 地形碼) 組合 329 → 333（**沒有少掉任何一個**），但可達性的**聯集
// 完全沒動**（214／258），只有「冷走走得到」從 205 變 206。代價是走訪時間
// 20 分鐘 → 27 分鐘。⇒ 維持 6／6；要再提高覆蓋率不能靠多存快照（spec 1193）。
const (
