...
def main():
    ...
    print()
    print("⚠ 抽取器只認「單純欄位讀取」的形狀，抽出來的是**簡化投影**：")
    print("`7CACh` 實際是 `7CA5h..7CACh` 範圍（`+0E9h ＋ (addr − 0A4h)`）、")
    print("`7ECFh` 過一張十二段換算表、`7F3Eh` 的基底是 bank 1 視窗不是角色")
    print("記錄。完整分支語意以 spec 624（PC-98）／1040（DOS）為準。")
    print()
