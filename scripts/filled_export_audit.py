...
- 新增的指令若**全部落在原本最後一條 `ret` 之後**，那是尾巴的資料被誤解，
  對函式本體的判讀沒有影響。
- 只要**有任何一條新增指令落在最後一條 `ret` 之前**，就要逐支看。

第二類**不等於**「一定被截斷」。函式中段本來就可能夾著資料——VROOMM 的
`int 3Fh` stub 陣列、6-byte real 常數表、`nop` 對齊填充——那些位元組被
`create_insn` 解成 `daa`／`verr`／`aas` 之類的東西，同樣落在最後一條 `ret`
之前。第 761 輪的 9 支裡有 5 支屬於這種假陽性。**這個工具給的是候選清單，
不是判決**：每一支都要打開來看新增的是真程式碼還是資料。
