...
- [x] 依 reference `CheckAffectsEffect(Death)` 接入 affect_63 recovery、Bleeding、troll_fire_or_acid damage-flag gate／TrollRegen `0x66`，explicit dragon-slayer `0x4B` target context，以及 HP=0 時的 combat position removal、renderer-neutral `DeathOverlay` signal、`COMSPR 0x8B`／`0x19` skull sprites 與 `CombatAction` cleanup；完整 9-cycle timing 與其他 Death routine 仍待接入。
