...
- [x] 依 reference `CheckAffectsEffect(Death)` 接入 affect_63 recovery、Bleeding、troll_fire_or_acid damage-flag gate／TrollRegen `0x66`，explicit dragon-slayer `0x4B` target context，以及 HP=0 時的 combat position removal、renderer-neutral `DeathOverlay` signal、team `DownedCorpse`／`Tile_DownPlayer=0x1F`、`COMSPR 0x8B`／`0x19` skull sprites、可治療倒下隊員的 Cure Light Wounds、explicit `CombatHealAllowed`→`RestoreCombatant` stand-up、`CombatAction` cleanup 與 NewBattle 初始倒下正規化；完整 9-cycle timing 與其他 Death routine 仍待接入。
