...
本檔是**最初的計畫與逐輪驗收紀錄**。目前狀態不看這裡：

| 想知道什麼 | 看哪裡 |
|---|---|
| 現況與已被推翻的斷言 | [`CONTEXT.md`](CONTEXT.md) |
| 接下來做哪一個、為什麼是這個順序 | [`WORKLIST.md`](WORKLIST.md) |
| 對外的狀態摘要與數字 | [`README.md`](README.md) |
| 工作規則（完成定義、證據標準、Git 紀律） | [`AGENTS.md`](AGENTS.md) |

下面的「分階段」表會跟著更新；再往下的逐輪驗收段落只作追溯，
**裡面的「目前」「下一步」都是當時的狀態**。
...
每一格的狀態都要能回指到一個可重生的數字，不寫「大致完成」。

| 階段 | 產出 | 狀態 |
|---|---|---|
| 0. 基線與素材盤點 | 本計畫、素材清單、GitHub 基線 | 完成 |
| 1. DAX／EXE／ECL 格式分析 | `docs/spec/` 格式規格與樣本工具 | 進行中：兩平台 2,874 個函式全進覆蓋台帳，**待解讀 0**；1,115 份規格 |
| 2. ECL 直譯器 | 可讀取並追蹤原作 block | 進行中：控制流可達 1,022 頁**未接上 0**、14,177 條指令**只讀掉運算元 0**；11 個 opcode 仍 partial（1,057 條） |
| 3. 遊戲狀態與 AD&D 規則 | 核心模型與相容性測試 | 進行中：戰鬥法術可宣告 **73／73**；回合生命週期的 initiative／held／delayed／guard／quick 未接 |
| 4. 渲染、輸入、音效 | Go/Ebiten 可執行 prototype | 進行中：第一人稱五格 × 四朝向與原版逐格 **19／20 相同**；其餘畫面未逐張比對，音效綁定未完 |
| 5. 中文化與手札 | 字串資源、字型、繁中內容 | 進行中：pack `zh-TW` 1,363 條、UI 917 條、譯名一致性 0 不一致；手札 45 則有中英文，只有 15 則接上 ECL 觸發 |
| 6. 整合與遊玩驗證 | DOS 對照測試與 release build | 進行中：**同一 session 已可從開場打到結局**（23 個段 subtest）；段內支線與三平台發行包未做 |
...
- [x] 依兩張 DOS oracle 還原固定 320×184 combat frame raster：
  8px 外框／divider、1px EGA bevel、點狀內緣與固定裂紋，再以 nearest-neighbour 2×。
