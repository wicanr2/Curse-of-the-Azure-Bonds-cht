...
更新：2026-09-03。這是 compact／接手後的第一份狀態摘要；規則仍以
...
- 發行：公開版為 `v1.0.5-20260904`（engine `1d271d58716c`），四個平台包可重生，
  五個 Release 附件已由 GitHub API 讀回對帳；Linux 與 Wine smoke 已做，兩張開場
  截圖與 1.0.3 起就逐位元組相同；Windows／macOS 真機、macOS 簽署／公證尚待。
  1.0.5 只修一件事：角色 VIEW 與建隊選單那三行文字先前畫在建角框的框帶上，
  被框蓋住只剩上半截（spec 1252）。
...
- 戰鬥彈藥：spec 1249 已回填原版 `49h` Arrow／`1Ch` Quarrel 指標；玩家射擊上限與
  AI／QUICK 換裝不再把 ITEMS 槽 11／12 的卷軸誤當彈藥。
- 營地選單：原版是**畫面最下面一行的橫排指令列**（spec 1251，三層原版擷圖在
  `docs/reference/original-dos/camp/`）。先前的縱排在世界地圖畫面只放得下三項，
  營地選單有七項——後四項畫到畫布外面，玩家看不到也點不到。營地的**畫面本身**
  仍是世界地圖；原版紮營會換成夜營圖、隊伍 NAME／AC／HP 欄與 `CAMPING` 狀態。
