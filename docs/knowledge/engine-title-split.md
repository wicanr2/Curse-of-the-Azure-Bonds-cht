...
盤點日期：2026-08-24。這是一份**帶日期的量測快照**，數字不可當今日現況；重跑
方式寫在最後。跨作品的現行短契約見 [`gold-box-series-reuse.md`](gold-box-series-reuse.md)。
...
⚠ 這份盤點不直接要求搬家。是否抽取只由第二款作品的實際 adapter 與 consumer
證據驅動；`WORKLIST.md` 已封存，不能再用其中的 P0／P1 狀態決定今日順序。
