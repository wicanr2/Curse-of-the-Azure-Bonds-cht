...
3. **`RE-12` 資料段表格取出**——多份規格卡在同一個原因，取出後 `ENG-04`／`ENG-05`／`ENG-06` 一起解鎖。
   可與第 2 項平行。
4. **`CHT-01`／`CHT-02`／`CHT-09`**——雙位元組處理與譯名表是地基，愈晚改代價愈高。可與 2–3 平行。
5. **`ENG-02`／`ENG-03`**——game pack 分檔與 stable ID 定案。必須在大量產出內容之前，否則後期分檔會動到每一條資料。
6. **`RE-04` ＋ `ENG-01` ＋ `CHT-06`**——**主要工作量**。事件盤點、寫成 game pack、同步兩語系是同一輪的三個動作，不分階段。
   依矩陣區域表的順序推進：艾森布拉 → 希爾斯法 → 尤拉什／摩安德之坑 → Myth Drannor 正常入口 → 各區補完。
7. **`RE-06`／`RE-07` ＋ `ENG-07`／`ENG-08`**——戰鬥系統。可與第 6 項平行（不同人／不同輪）。
8. **`RE-05` ＋ `ENG-10` ＋ `VER-07`**——存檔。
9. **`UI-01`／`UI-02`／`AUD-01`**——表現層，需要原版 runtime 當 oracle。
10. **`RE-11`（未定義區段判定）／`RE-13`／`ENG-12`**——盤點與整理的收尾，不阻塞玩家路徑。
11. **`VER-04`／`VER-05`／`VER-06`**——最終驗收：零捷徑通關 ＋ 對 DOSBox 原版實測。
