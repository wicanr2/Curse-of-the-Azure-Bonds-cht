...
| ├ 事件 | 4 個 `events`、113 條 `option_rules`、**413 條 `text_rules`** | `json` |
| └ 語系 | `en` **656** 條、`zh-TW` **656** 條，**一一對齊、沒有漏譯**；譯名一致性由 `internal/glossary` fail-closed 擋住（79 詞條、0 不一致） | `json` |
...
game pack 內 `en` 與 `zh-TW` 各 656 條，一一對齊沒有漏。

所以剩下的**不是重構程式碼，是產出資料**：把原作 25 個 block／4,222 個可達
instruction 的事件，逐條寫成 game pack JSON。目前 413 條 `text_rules` 的區域分佈
顯示缺口集中在哪裡：
...
目前 `assets/locale/zh-TW.json` 870 條；game pack 內建雙語 `en` 656 ＝ `zh-TW` 656，key 完全對齊。
...
| `CHT-06` | **翻譯隨內容同步** | 目前 game pack 的 `en`／`zh-TW` 各 656 條**一一對齊、沒有漏譯**。翻譯不是獨立階段，是 `ENG-01` 每寫一條事件就同時寫兩個語系。把「兩語系條數與 key 完全一致」設成 CI gate（`cmd/locale-drift-audit` 已有基礎），漏一條就紅 |
...
| `CHT-07` | **Journal 整合進遊戲** | 遊戲內閱讀器已可用（`ModeJournal`：開啟／左右翻頁／Esc 返回，依字型 advance 分頁，spec 556）。**59 則校訂稿在 `docs/manual/journal/`，game pack 有 36 則**（`journal.*` 53 個訊息鍵），36 則**全部都有 producer**、只在劇情命中時解鎖。剩 23 則（1、2、6、8、10、13、14、16、24、27、28、35、36、37、39、40、42、43、45、47、49、55、58）：它們的觸發文字是共用子程式「AND YOU RECORD IT IN JOURNAL ENTRY」＋ `81h` 運算元帶進來的編號，要先解出字串記憶體那一格是誰寫的，才知道每一則掛在哪個事件。另缺手札原圖（第 4、52、59 條是地圖）與不提前劇透的驗收 |
