...
| `ENG-09` | 全法術表 | spec 1111 ✅（資料）／1117 ✅（資料驅動施法）| 原作 100 筆全部在 `gamepack/rules/spell-table.json`，逐欄有出處。**實作不再是一支法術一段程式碼**：效果碼（`+0Ah`）、持續時間係數、豁免類別直接讀表，一次接上 11 支效果碼 remake 解讀得了的法術（定身、加速、緩慢、隱形…），宣告數 12 → **23**／79。剩：`+0Ah = 0` 的 21 支傷害類、其餘 35 支的效果碼語意（spec 1005）、新接 11 支的 visual／sound |
