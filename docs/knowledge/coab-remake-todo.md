...
| └ 語系 | `en` **672** 條、`zh-TW` **672** 條，**一一對齊、沒有漏譯**；譯名一致性由 `internal/glossary` fail-closed 擋住（89 詞條、0 不一致） | `json` |
...
game pack 內 `en` 與 `zh-TW` 各 672 條，一一對齊沒有漏。
...
目前 `assets/locale/zh-TW.json` 870 條；game pack 內建雙語 `en` 672 ＝ `zh-TW` 672，key 完全對齊。
...
| `CHT-06` | **翻譯隨內容同步** | 目前 game pack 的 `en`／`zh-TW` 各 672 條**一一對齊、沒有漏譯**。翻譯不是獨立階段，是 `ENG-01` 每寫一條事件就同時寫兩個語系。把「兩語系條數與 key 完全一致」設成 CI gate（`cmd/locale-drift-audit` 已有基礎），漏一條就紅 |
...
| `CHT-07` | **Journal 整合進遊戲** | ✅ **可讀的部分已完成**（spec 1108）：遊戲內閱讀器可用（`ModeJournal`，依字型 advance 分頁，spec 556）；全 corpus 窮舉普查出 **59 則裡有 44 則會被引用**，這 44 則**全部收錄且全部有 producer**，只在劇情命中時解鎖，`TestEveryJournalEntryHasAProducer` 擋住死條目。剩下 15 則 corpus 沒有任何引用：第 1 則是開場導言、第 8／39 則只有插圖、九則講菲蘭與匕首瀑布等別部作品的地點、三則（14／37／49）是本作風味但查無引用。缺：第 1 則導言要讓玩家開場就讀得到、手札的圖（第 4／8／39／52／59 條）、逐則驗解鎖時機 |
