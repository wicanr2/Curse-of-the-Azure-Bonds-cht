...
**段內的每格內容已經全遊戲量過一輪**（`cmd/cell-sweep` →
[`docs/audit/cell-sweep.md`](../audit/cell-sweep.md)，報告
[`seg-23-cell-sweep-report.md`](seg-23-cell-sweep-report.md)）：13 個 block、
**194 個分派索引**逐個站上去跑生命週期，**185 格演出中文、落回原文 0 格**，
9 格沒演出來且每一格都印得出處理常式的守衛。⚠ 那一份量的是**內容與語系**，
不是可達性——它把隊伍撐起來以免入口戰鬥擋住盤點，所以「演得出來」不等於
「正常隊伍走得到」。走得到走不到仍由 `SEG-10` 那 12 段的段測試負責。
...
| ID | 不變量 |
|---|---|
| `SEG-30` | ✅ **完成**：22 個段界快照逐份存下去、讀回來，比對 block／模式／章節／地城／隊伍人數（`SEG-10` 報告 §三）|
| `SEG-31` | ✅ **完成**：同一個角色的經驗值不准倒退、隊伍變動位置是宣告好的，**裝備／記憶法術／效果存下去讀得回來且段界之間沒有變動**（報告 `seg-31-33-invariants-report.md`）|
| `SEG-32` | ✅ **完成**：整條主線 233 句話**落回原文 0 句**；第一次跑抓到 12 句英文並補譯（報告 `seg-32-locale-report.md`）|
| `SEG-33` | ✅ **完成**：23 段結束時播的就是那個 block 在 PC-98 原作會選的 selector（spec 355），game pack 的宣告也逐格對回那張表；整條主線播出 4 首不同的曲子 |
