...
| Esc | 取消當前操作或關閉最上層畫面 |
| J | 隨時開啟冒險手札（戰鬥中停用） |
| F1 | 開啟／關閉 Help 與全域按鍵說明 |
| F2 | 在原版忠實 theme 與 A6 現代 theme 之間即時切換 |
| F3 | 開啟／關閉目前地圖攻略；`V` 切換探索／完整攻略 |
| F4 | 依序切換 640×480、1024×768、1280×960 |
| F5 | 儲存已建立的隊伍至 `party.json` |
| F9 | 從 `party.json` 載入隊伍 |
| F10 | 自動儲存後離開；儲存失敗時留在遊戲並顯示原因 |
...
theme、解析度、攻略探索格與劇透警告確認另存在使用者設定目錄的
`ui-settings.json`，不會混入隊伍或劇情存檔。視窗可任意拉伸並填滿；若拉成
非 4:3，畫面會依使用者選擇非等比變形。高解析度下 A6 框線與 overlay 依當前
邏輯畫布繪製；原版圖像使用最近鄰縮放，不捏造不存在的細節。
...
戰鬥中按 `M` 再按方向鍵，可讓當前角色移動一格；Esc 取消移動模式。移動會消耗該角色回合；若離開敵人鄰接範圍，敵人會進行免費反擊。
