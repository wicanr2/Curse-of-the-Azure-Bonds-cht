...
第二次反向稽核再找到一個同型缺口：spec 1000 已 exact 解出 DOS
`overlay-24:00C28h` 以物品類別 `49h`／`1Ch` 建立兩個彈藥指標，但 spec 1120／
1186 漏掉這份較早證據，正式程式因而把卷軸槽 11／12 當成彈藥。spec 1249 已
回填、實作並以玩家與 AI／QUICK 正反測試閉合；兩份舊 spec 均加入 backlink ledger。
