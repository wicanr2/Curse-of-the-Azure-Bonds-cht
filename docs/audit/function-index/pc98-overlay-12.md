...
| `06F9` | sub_6F9 | — | 53 | 20 | 0 | 2 | ✓ | 已解讀 | exact | docs/spec/633-alignment-encoding-and-effect-callbacks.md<br>if arg_0 = 0 且 <sub_1547>(@var_4,18h,DS:9594h) = 0 then DS:0A035h := 1、DS:0A039h -= 4 | audit/function-index/dos-overlay-12.md<br>spec/633-alignment-encoding-and-effect-callbacks.md<br>spec/639-overlay12-poison-and-race-gates.md |
...
| `133F` | sub_133F | — | 87 | 32 | 0 | 2 | ✓ | 已解讀 | exact | docs/spec/639-overlay12-poison-and-race-gates.md<br>p := <sub_FCC>(DS:9594h);p = nil 或 p^[5Ah] = 0 則 DS:0A02Eh := 0;否則 p^[5Ah] < 3 時 DS:0A02Eh := DS:0A02Eh div 2(有號)。第二個條件又檢查了一次 p <> nil,編譯出來是兩次檢查 | spec/639-overlay12-poison-and-race-gates.md |
| `1396` | sub_1396 | — | 91 | 36 | 0 | 2 | ✓ | 待解讀 | — | — | — |
| `13F1` | sub_13F1 | — | 20 | 9 | 0 | 0 | ✓ | 邊界碎片 | — | docs/spec/569-small-function-batch-reading.md<br>邊界碎片：body 內沒有 `ret` 也沒有尾跳躍，最後一條是 `push ax`；這是 IDA 建錯的函式邊界，真正的函式體要以位址範圍重讀（body 共 20 bytes，已逐條讀完） | audit/embedded-strings.md |
...
| `16C2` | sub_16C2 | — | 59 | 21 | 0 | 2 | ✓ | 已解讀 | exact | docs/spec/639-overlay12-poison-and-race-gates.md<br>if DS:9594h^[11Ah](RACETYPE) = 13h then { if <sub_1547>(@var_4,18h,DS:9594h) = 0 then DS:0A035h := 1; DS:0A039h -= 4 }。⚠ 減法那行在內層條件之外(jnz 跳到減法那行不是跳過),與 06F9h 兩行都在條件內不同——差別只有一個跳躍目標 | spec/639-overlay12-poison-and-race-gates.md |
...
| `176D` | sub_176D | — | 60 | 28 | 0 | 3 | ✓ | 已解讀 | exact | docs/spec/639-overlay12-poison-and-race-gates.md<br>if DS:0A02Fh and 20h <> 0 then <sub_1B>(0) 並備妥「は影響を受けなかった。」呼叫 <far 014A:0024>(arg_6,arg_8,訊息,1,0Ah) | spec/639-overlay12-poison-and-race-gates.md |
...
| `17B5` | sub_17B5 | — | 87 | 32 | 0 | 1 | ✓ | 已解讀 | exact | docs/spec/639-overlay12-poison-and-race-gates.md<br>DS:0A03Dh := arg_6^[18Eh]^[0Ah];if 其 ^[11Ah](RACETYPE) = 3 then x := <far 0176:0506>(arg_6)(cbw 有號),DS:0A02Eh := byte(ROLLDICE(1,0Ch)×3 + 4 + x)(1d12×3+4 是 7..40,加 x 後只存低 byte),DS:0A039h += 2 | spec/639-overlay12-poison-and-race-gates.md |
| `180C` | sub_180C | — | 52 | 16 | 0 | 1 | ✓ | 已解讀 | exact | docs/spec/639-overlay12-poison-and-race-gates.md<br>DS:0A03Dh := arg_6^[18Eh]^[0Ah];if 其 ^[11Ah](RACETYPE) = 8 then DS:0A039h += 3、DS:0A02Eh += 3 | spec/639-overlay12-poison-and-race-gates.md |
