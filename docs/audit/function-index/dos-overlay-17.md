...
| `1994` | sub_1994 | — | 10 | 4 | 3 | 0 |  | 邊界碎片 | — | docs/spec/569-small-function-batch-reading.md<br>邊界碎片：body 內沒有 `ret` 也沒有尾跳躍，最後一條是 `mul dx`；這是 IDA 建錯的函式邊界，真正的函式體要以位址範圍重讀（body 共 10 bytes，已逐條讀完） | spec/872-remove-from-party.md |
...
| `3A56` | sub_3A56 | — | 323 | 104 | 2 | 3 | ✓ | 已解讀 | exact | 872<br>把 遠指標(DS:6506h) 那個角色移出隊伍並釋放(retf 4，(不減人數, 要釋放圖像))：分鏈頭/非鏈頭兩條路徑從 DS:650Ah 鏈上摘掉，叫 overlay-17:0003Eh(spec 849 解構子)整筆釋放，再把 DS:6506h 移到前一個/下一個、都不行就退回鏈頭(所以呼叫端拿到的選中者永遠有效)；不減人數=0 時 遠指標(DS:4F9Dh)^[67Ch] 減 1。要釋放圖像≠0 時先叫 1994h(角色^[143h]) | spec/872-remove-from-party.md |
