...
| `05F9` | sub_5F9 | — | 86 | 30 | 1 | 1 | ✓ | 已解讀 | exact | docs/spec/756-map-fill-confirms-grid-and-assorted-routines.md<br>數隊伍(retf，無參數)：走 DS:650Ah 的隊伍鏈，數出 +0F7h = 0 或 +0F7h = 0B3h 的成員數，回 byte。+0F7h 是既知「四個處理常式存回不對稱」的那個欄位 | spec/756-map-fill-confirms-grid-and-assorted-routines.md |
...
| `0F5B` | sub_F5B | — | 84 | 30 | 0 | 1 | ✓ | 已解讀 | exact | docs/spec/756-map-fill-confirms-grid-and-assorted-routines.md<br>兩個旗標的輸出(retf 8，兩個遠指標都是輸出參數)：a^ := DS:6F70h 起 8 個 4-byte 遠指標中前 7 個有任一非 NIL；b^ := 第 8 個(DS:6F8Ch)非 NIL。兩者先都清 0 | spec/756-map-fill-confirms-grid-and-assorted-routines.md |
