...
| `03D4` | sub_3D4 | — | 1041 | 365 | 2 | 3 | ✓ | 待解讀 | — | — | spec/865-list-header-lookup-and-key-prompt.md |
...
| `07FD` | sub_7FD | — | 141 | 52 | 1 | 1 | ✓ | 已解讀 | exact | 865<br>找上一個標題行(retf 8，(上限, 鏈頭, 未用))：走 spec 859 的顯示行節點鏈(+29h 標題行旗標、+2Ah next)，在前『上限』行裡回最後一個 +29h≠0 的節點；找不到且鏈頭自己是標題行就回鏈頭(只在上限≤0 時生效)。第三個參數本體沒讀 | spec/865-list-header-lookup-and-key-prompt.md |
...
| `112A` | sub_112A | — | 111 | 52 | 0 | 2 | ✓ | 已解讀 | exact | 865<br>問到合法按鍵為止(retf 0Ah)：把提示字串指派進緩衝(0A54h:64Eh，上限 28h)、DS:7294h:=2，迴圈內組字後叫 03D4h 收鍵，再用 0A54h:8D4h 對合法鍵表以 ZF 檢查，不合法就重問 | audit/function-strings.md<br>spec/865-list-header-lookup-and-key-prompt.md |
