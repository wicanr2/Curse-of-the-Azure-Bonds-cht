...
| `1199` | sub_1199 | — | 168 | 59 | 0 | 1 | ✓ | 已解讀 | exact | 843<br>顯示行清單的建構子(retf 6，(鏈頭遠指標, 筆數))：GetMem 第一個節點 2Eh bytes、清 next/屬性/文字，之後 for i := 2 to 筆數 逐一 GetMem 掛在 +2Ah 並同樣清空。筆數 < 2 時只建一個。與 spec 843 的『加一行』是同一種節點 | — |
