...
| `0011` | sub_11 | — | 99 | 38 | 5 | 1 | ✓ | 已解讀 | exact | docs/spec/812-coin-values-and-class-name-table.md<br>取鏈上第 N 個節點：retf 6，宣告順序 (索引, 串列頭)，回傳 dx:ax。n := 0；while (p <> NIL) and (n <> 索引) do p := p^[2Ah]、inc(n)；n = 索引 才回 p，否則回 NIL。串列比索引短時也回 NIL，呼叫端分不出兩種情形。spec 812 | audit/embedded-strings.md<br>audit/function-index/dos-overlay-02.md<br>audit/function-index/dos-overlay-04.md<br>audit/function-index/dos-overlay-15.md<br>audit/function-index/dos-overlay-17.md<br>audit/function-index/dos-overlay-22.md |
| `0074` | sub_74 | — | 65 | 25 | 1 | 1 | ✓ | 已解讀 | exact | docs/spec/755-per-character-slots-sound-driver-and-text-io.md<br>數鏈長(retf 4)：走遠指標鏈計數，next 在 +2Ah(＝物品節點的 far next)，回傳 word | audit/embedded-strings.md<br>audit/function-index/dos-overlay-22.md<br>audit/function-index/dos-overlay-24.md<br>audit/function-index/dos-overlay-25.md<br>audit/function-index/pc98-overlay-04.md<br>audit/function-index/pc98-overlay-24.md |
| `00D5` | sub_D5 | — | 240 | 92 | 1 | 1 | ✓ | 待解讀 | — | — | — |
| `01E5` | sub_1E5 | — | 431 | 180 | 1 | 1 | ✓ | 待解讀 | — | — | — |
| `03D4` | sub_3D4 | — | 1041 | 365 | 2 | 3 | ✓ | 待解讀 | — | — | audit/function-index/dos-overlay-26.md<br>spec/865-list-header-lookup-and-key-prompt.md |
...
| `07FD` | sub_7FD | — | 141 | 52 | 1 | 1 | ✓ | 已解讀 | exact | 865<br>找上一個標題行(retf 8，(上限, 鏈頭, 未用))：走 spec 859 的顯示行節點鏈(+29h 標題行旗標、+2Ah next)，在前『上限』行裡回最後一個 +29h≠0 的節點；找不到且鏈頭自己是標題行就回鏈頭(只在上限≤0 時生效)。第三個參數本體沒讀 | audit/function-index/pc98-overlay-26.md<br>spec/865-list-header-lookup-and-key-prompt.md |
| `088A` | sub_88A | — | 314 | 121 | 2 | 3 | ✓ | 已解讀 | exact | 848<br>捲動清單的重繪（巢狀程序，retf 4，(靜態鏈, 頂端行號)）：先用外層 frame 的 +18h/+1Ah/+1Ch/+1Eh 設視窗，sub_11h 取第 n 個節點，逐行畫到 +18h 為止。節點屬性 +29h ≠ 0 用顏色 +24h、否則 +22h（與 spec 848 的『屬性 0 就停』同一格）。文字比欄寬 外^[−54h] 短時再用空白補滿右側 | audit/function-index/pc98-overlay-26.md<br>spec/848-scroll-list-widget.md |
...
| `112A` | sub_112A | — | 111 | 52 | 0 | 2 | ✓ | 已解讀 | exact | 865<br>問到合法按鍵為止(retf 0Ah)：把提示字串指派進緩衝(0A54h:64Eh，上限 28h)、DS:7294h:=2，迴圈內組字後叫 03D4h 收鍵，再用 0A54h:8D4h 對合法鍵表以 ZF 檢查，不合法就重問 | audit/function-index/pc98-overlay-26.md<br>audit/function-strings.md<br>spec/865-list-header-lookup-and-key-prompt.md |
