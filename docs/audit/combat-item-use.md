...
充能物品的效果編號就是**法術主表的列**：目標模式（`+6`）、豁免（`+8`）、效果碼（`+0Ah`）都在表裡，骰子在各自的 handler 裡（spec 1169）。`已接` ＝ remake 的 `internal/combat` 有那一支 handler 的行為讀法；18 件裡還有 0 個效果沒接。
...
| 5 | 48 | Scroll | `3Ch` | 1 | `5Fh` | ✅ |
| 5 | 50 | Scroll | `3Ch` | 1 | `60h` | ✅ |
| 6 | 65 | Scroll | `3Ch` | 1 | `60h` | ✅ |
