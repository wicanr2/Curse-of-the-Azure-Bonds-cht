...
⚠ 分子也有了：`-covered` 把 `docs/reference/original-dos/first-person/index.tsv`
那 20 張換算成簽章，**只蓋掉 12／585**——同一條走廊上五格朝同一方向常常是同一張
畫面。同一個旗標會排出「接下來拍哪一格」的貪婪順序，那就是 oracle session 的
工作清單。
...
由工具產生**的覆蓋率報表併成一張表：每個大類目前量到什麼、數字出自哪一份、
以及**那個數字證明不了什麼**。

★ 它取代的是 `CONTEXT.md`／`WORKLIST.md` 裡手寫的「尚未完成」。那種句子沒有任何
東西會在做完那一刻更新它，所以會同時漏掉已完成的項目、又講不清楚剩下的工作有
多大（第 664 輪抽查兩條就抓到兩條假的）。

⚠ **它不是完成度百分比。** 每個分母的意義都不同，平均起來只會得到一個沒有意義
而且偏樂觀的數字。

⚠ **「沒有數字」的那幾列一定要留著。** 音訊生命週期、逐張 UI fidelity、開場到
結局的同一 session、三平台發行都還沒有分母；省略它們會讓這張表自己變成下一個
假的完成宣告。
