...
上面那張表是**逐頁**判的，而一頁只要曾經被某一條命中的 run 經過就算 `matched`。玩家走的是**一條** run，不是所有 run 的聯集：同一頁在 A 路徑上和兄弟句一起印（規則命中）、在 B 路徑上單獨印（沒有規則），逐頁的算法只看得到 A。

這不是假想的失誤。`wizard-tower.efreet-band` 與 `tilverton.sewers.spoils-and-master-alone` 是實機走出來才發現要補的規則；把它們從 pack 拿掉重跑，逐頁的 `unmatched` **仍然是 0**，而下面這個數字從 93 變成 95，新增的正好是那兩份文字。`TestOrphanRunAccountingCatchesTheAloneSiblingBug` 釘住這個差別。

| 處置 | run 數 | 意思 |
|---|---:|---|
| 控制流走得到的 run | 1375 | 分母（文字相同的只算一次）|
| `matched` | 1199 | 有規則命中這一整條 |
| **`orphan`** | **0** | **走得到、文字完整、沒有任何規則命中——這才是待辦** |
| `variable-insert` | 72 | run 裡有頁印執行期的值，靜態比對不到 |
| `subroutine` | 35 | 整條就是一頁共用子程式片段，實機會被併進呼叫端 |
...
⚠ 這是**候選清單不是缺陷清單**：走訪器把 `IF` 的兩條路都走，其中一條在實機可能永遠不成立。判一條是不是真的缺陷，要看**兄弟句**——若有另一條規則的 `all_contains` 含了這份文字的片段**再加上**別段的片段，那條規則就攔不到單獨印的情況，缺的就是這一條。

| Block | 起始頁 | 頁數 | 這條 run 的文字 |
|---|---|---:|---|
| `ECL1.DAX/0x50` | `0x17E3` | 1 | A YOUNG WOMAN WITH A PURPLE SASH RACES BY ON HORSEBACK. MOMENTS LATER A MAN WITH A HAMMER |
| `ECL1.DAX/0x50` | `0x153D` | 1 | AMIDST THE FIELDS OF DAGGERDALE, YOU FIND HUGE FOOTPRINTS LEADING TOWARD A SMALL FARM. IN THE RAYS OF |
| `ECL1.DAX/0x50` | `0x14C7` | 1 | AS YOU TRAVEL SOUTH, YOU ARE PASSED BY THE |
| `ECL1.DAX/0x50` | `0x030B` | 1 | HE CONTINUES, ' |
| `ECL1.DAX/0x50` | `0x11AE` | 1 | HOWEVER, ALL IS NOT WELL. INDISTINCT SHAPES CIRCLE THE BUILDING. A BATTLE IS |
| `ECL1.DAX/0x50` | `0x16DA` | 1 | PARALLELING YOU THROUGH THE FOREST IS A GROUP WEARING THE LIVERY OF MYTH DRANNOR. |
| `ECL1.DAX/0x50` | `0x16DA` | 1 | PARALLELING YOU THROUGH THE FOREST IS A GROUP WEARING THE LIVERY OF MYTH DRANNOR. WHEN YOU TRY TO CLOSE WITH THEM, THEY MELT INTO THE |
| `ECL1.DAX/0x50` | `0x12E8` | 1 | THE MOUNTAINS RISE INTO AN IMPASSABLE WALL, |
| `ECL1.DAX/0x50` | `0x0963` | 1 | THE PATRONS BECOME SURLY. THE |
| `ECL1.DAX/0x50` | `0x031E` | 1 | THE ROBE COLLAPSES WHEN YOU ATTACK. FROM ABOVE COMES A VOICE, ' |
| `ECL1.DAX/0x50` | `0x189B` | 1 | YOU ARE AMBUSHED BY FIRE KNIVES DISGUISED AS |
| `ECL1.DAX/0x50` | `0x0BAE` | 1 | YOU ARE IN A |
| `ECL1.DAX/0x50` | `0x1161` | 1 | YOU REACH SHADOW GAP. ATOP IT, YOU NOTE A RECENTLY |
| `ECL1.DAX/0x51` | `0x0597` | 1 | 'BEGONE ZHENTRIM SCUM. YOU'LL NOT SOIL MY |
| `ECL1.DAX/0x51` | `0x169A` | 1 | A PIRATE CRAFT APPROACHES UNTIL THE PARTY IS |
| `ECL1.DAX/0x51` | `0x1A26` | 1 | AFTER SEVERAL MILES, IT ENDS IN A HUGE PILE OF |
| `ECL1.DAX/0x51` | `0x17AC` | 1 | ALONG THE COAST, YOU PASS AN ABANDONED ZHENTRIM |
| `ECL1.DAX/0x51` | `0x0072` | 1 | AS YOU DEPART, A HUGE FORCE OF ZHENTIL KEEP |
| `ECL1.DAX/0x51` | `0x00C5` | 1 | AS YOU LEAVE, YOU SEE THAT CIVIL WAR IS ERUPTING. |
| `ECL1.DAX/0x51` | `0x00C5` | 1 | AS YOU LEAVE, YOU SEE THAT CIVIL WAR IS ERUPTING. MANY FACTIONS ARE USING THE HAVOC YOU'VE CAUSED TO EVEN |
| `ECL1.DAX/0x51` | `0x156E` | 1 | BEREFT OF EFFECTIVE LEADERSHIP, THE ARMY OF |
| `ECL1.DAX/0x51` | `0x125E` | 1 | BUGBEARS AND WORGS COME TO THE RESCUE OF THE |
| `ECL1.DAX/0x51` | `0x0E08` | 1 | HALFWAY TO VOONLAR, YOU SPOT THE VANGUARD |
| `ECL1.DAX/0x51` | `0x19DD` | 1 | NEAR YULASH, YOU DISCOVER A GREAT SLASH |
| `ECL1.DAX/0x51` | `0x13ED` | 1 | ON THE LONG ROAD TO DAGGER FALLS, THE MONSTERS |
| `ECL1.DAX/0x51` | `0x13ED` | 1 | ON THE LONG ROAD TO DAGGER FALLS, THE MONSTERS BEGIN TO FIGHT AMONGST THEMSELVES. ONE NIGHT, THEIR |
| `ECL1.DAX/0x51` | `0x0F7C` | 1 | THE ARMY CONTINUES ON PAST YOU. |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. A PIRATE CRAFT APPROACHES UNTIL THE PARTY IS |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. ALONG THE COAST, YOU PASS AN ABANDONED ZHENTRIM |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. AS YOU DEPART, A HUGE FORCE OF ZHENTIL KEEP |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. AS YOU LEAVE, YOU SEE THAT CIVIL WAR IS ERUPTING. |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. AS YOU LEAVE, YOU SEE THAT CIVIL WAR IS ERUPTING. MANY FACTIONS ARE USING THE HAVOC YOU'VE CAUSED TO EVEN |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. HALFWAY TO VOONLAR, YOU SPOT THE VANGUARD |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. NEAR YULASH, YOU DISCOVER A GREAT SLASH |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. THE GUARDS INFORM YOU THAT THE CITY IS SEALED, |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. TRAVELLING THROUGH THE WILDS, YOU COME UPON A |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. YOU ARE AMBUSHED BY FIRE KNIVES DISGUISED AS |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. YOU ARE CONFRONTED BY A PATROL FROM ZHENTIL KEEP. |
| `ECL1.DAX/0x51` | `0x0F7C` | 2 | THE ARMY CONTINUES ON PAST YOU. YOU ARE CONFRONTED BY A PATROL FROM ZHENTIL KEEP. THE OFFICER STEPS FORWARD. 'YOU ARE LISTED IN OUR |
| `ECL1.DAX/0x51` | `0x164D` | 1 | THE CAPTAIN IS IMPRESSED WITH YOUR SKILL |
| `ECL1.DAX/0x51` | `0x0F30` | 1 | THE FIRST OFFICER YOU MEET STATES FLATLY, 'THE |
| `ECL1.DAX/0x51` | `0x032F` | 1 | THE GUARDS INFORM YOU THAT THE CITY IS SEALED, |
| `ECL1.DAX/0x51` | `0x0FB1` | 1 | THE REST OF THE ARMY, CONFUSED BY THE SUDDEN |
| `ECL1.DAX/0x51` | `0x1072` | 1 | THE TRAIL LEADS UP INTO A CRAGGY WILDERNESS, |
| `ECL1.DAX/0x51` | `0x1072` | 1 | THE TRAIL LEADS UP INTO A CRAGGY WILDERNESS, FINALLY ENDING BELOW A GIGANTIC PEAK. A SQUAD OF ZHENTIL |
| `ECL1.DAX/0x51` | `0x1178` | 1 | THE TROOPERS GO TO ALERT TESHWAVE AND LEAVE |
| `ECL1.DAX/0x51` | `0x19A5` | 1 | THEY SAY THEY ARE FOLLOWING THE |
| `ECL1.DAX/0x51` | `0x1024` | 1 | TRAVELLING THROUGH THE WILDS, YOU COME UPON A |
| `ECL1.DAX/0x51` | `0x1B7A` | 1 | YOU ARE AMBUSHED BY FIRE KNIVES DISGUISED AS |
| `ECL1.DAX/0x51` | `0x180C` | 1 | YOU ARE CONFRONTED BY A PATROL FROM ZHENTIL KEEP. |
| `ECL1.DAX/0x51` | `0x180C` | 1 | YOU ARE CONFRONTED BY A PATROL FROM ZHENTIL KEEP. THE OFFICER STEPS FORWARD. 'YOU ARE LISTED IN OUR |
| `ECL1.DAX/0x51` | `0x0EA0` | 1 | YOU ARE TAKEN TO THE OFFICERS WHO STATE THAT THEY |
| `ECL1.DAX/0x51` | `0x0EA0` | 1 | YOU ARE TAKEN TO THE OFFICERS WHO STATE THAT THEY ARE MARCHING ON SHADOWDALE. NOTING THE SIGILS ON YOUR |
| `ECL1.DAX/0x51` | `0x12F2` | 1 | YOU ESCAPE FROM THE MONSTERS AND REACH |
| `ECL1.DAX/0x51` | `0x12F2` | 1 | YOU ESCAPE FROM THE MONSTERS AND REACH DAGGER FALLS BEFORE THEM. THE CITY MOBILIZES AND ROUTS |
| `ECL1.DAX/0x51` | `0x12F2` | 1 | YOU ESCAPE FROM THE MONSTERS AND REACH DAGGER FALLS BEFORE THEM. THE CITY MOBILIZES AND ROUTS THE DISORGANIZED CREATURES. THE ZHENTRIM RESCUE FORCE IS |
| `ECL1.DAX/0x51` | `0x1468` | 1 | YOU SWEEP DOWN UPON THE ZHENTRIM FORCES AS THEY |
| `ECL1.DAX/0x51` | `0x1468` | 1 | YOU SWEEP DOWN UPON THE ZHENTRIM FORCES AS THEY PREPARE TO MARCH ON DAGGER FALLS, AND A VAST MELEE |
| `ECL3.DAX/0x12` | `0x13C9` | 1 | TENDRILS COME UP FROM THE FLOOR AND WRAP THEMSELVES AROUND |
| `ECL3.DAX/0x12` | `0x13C9` | 1 | TENDRILS COME UP FROM THE FLOOR AND WRAP THEMSELVES AROUND ALIAS |
| `ECL3.DAX/0x12` | `0x13C9` | 1 | TENDRILS COME UP FROM THE FLOOR AND WRAP THEMSELVES AROUND ALIAS AND |
| `ECL4.DAX/0x23` | `0x0396` | 1 | YOU ARE BEING ATTACKED BY |
...
| `ECL1.DAX/0x50` | `0x01C4` | `matched` | `standing-stone.arrival` | YOU ARE AT THE STANDING STONES. A GREY ROBED MAN SITS AGAINST A STONE, HIS FACE COVERED IN SHADOW. |
...
| `ECL1.DAX/0x50` | `0x030B` | `matched` | `world.wilderness.voice-continues-seek-black` | HE CONTINUES, ' SEEK GREEN TO THE NORTHWEST.' |
| `ECL1.DAX/0x50` | `0x031E` | `matched` | `world.wilderness.robe-collapses-seek-black` | THE ROBE COLLAPSES WHEN YOU ATTACK. FROM ABOVE COMES A VOICE, ' SEEK GREEN TO THE NORTHWEST.' |
...
| `ECL1.DAX/0x50` | `0x1375` | `matched` | `world.wilderness.black-shapes-in-sky` | SAILING ACROSS THE SKY ARE GREAT BLACK SHAPES. SUDDENLY, THEY SWEEP DOWN, REVEALED AS FEARSOME BLACK DRAGONS. |
...
| `ECL1.DAX/0x51` | `0x081B` | `matched` | `hillsfar.red-plumes-spill-drinks-only` | SOME RED PLUMES COME OVER AND SPILL YOUR DRINKS. THEY ORDER YOU TO CLEAN UP THE MESS. DO YOU? |
...
| `ECL1.DAX/0x51` | `0x14DC` | `matched` | `yulash.battle-mutual-rout` | THE FORCES TURN OUT TO BE CLOSELY MATCHED. BOTH THE TROOPERS AND THE MONSTERS ARE ROUTED. TESHWAVE ITSELF ONLY SUFFERED MINOR DAMAGE. THE MONSTERS FLEE INTO THE… |
...
| `ECL2.DAX/0x03` | `0x034A` | `matched` | `fire-knife.checkpoint-arrival` | YOU ARE AT A CHECKPOINT. THE FIRE KNIVES DEMAND YOUR IMMEDIATE SURRENDER. DO YOU SURRENDER? |
...
| `ECL2.DAX/0x03` | `0x085B` | `matched` | `tilverton.sewers.spoils-and-master-alone` | THEY COLLECT MONEY AND EQUIPMENT AND HEAD OFF TO ENJOY THE SPOILS. THEY LAUGH. 'THE MASTER IS AT THE SOUTH END OF THE SEWERS. TELL HIM WE SENT YOU ON.' |
...
| `ECL2.DAX/0x03` | `0x15CB` | `matched` | `tilverton.sewers.thief-swatch-trapdoor` | THE THIEF RETRIEVES A SWATCH OF CLOTH FROM A SEALED TRAPDOOR. THE DOOR WAS TOO WELL JAMMED TO OPEN. THE SCENT OF A TAVERN WAFTS DOWN THROUGH THE DOOR. YOU RECOG… |
...
| `ECL2.DAX/0x04` | `0x0235` | `matched` | `fire-knife.checkpoint-arrival` | YOU ARE AT A CHECKPOINT. THE FIRE KNIVES DEMAND YOUR IMMEDIATE SURRENDER. DO YOU SURRENDER? |
...
| `ECL3.DAX/0x11` | `0x135A` | `matched` | `pit.lemon-tang-alone` | A SLIGHT TANG OF LEMON IS IN THE AIR. ALIAS SAYS, 'DRAGONBAIT SMELLS FRESH AIR.  THERE MUST BE A WAY OUT AROUND HERE.' |
...
| `ECL3.DAX/0x12` | `0x0B63` | `matched` | `pit.alias-altar-entrance` | ALIAS SAYS, 'THIS IS IT, THE ENTRANCE TO THE ALTAR OF MOGION.' YOU SMELL VIOLETS. |
...
| `ECL4.DAX/0x21` | `0x0080` | `matched` | `zhentil.temple.dragged-to-cell` | YOU ARE DRAGGED THROUGH THE TEMPLE. |
| `ECL4.DAX/0x21` | `0x01A7` | `matched` | `zhentil.temple.dragged-to-cell` | YOU WAKE UP IN A DARK, DREARY CELL. |
...
| `ECL5.DAX/0x32` | `0x0908` | `matched` | `lava-tube.patrol-behind` | YOU HEAR A PATROL APPROACH FROM BEHIND YOU. |
...
| `ECL5.DAX/0x32` | `0x1518` | `matched` | `lava-tube.silk-buys-eggs` | SILK STEPS OUT FROM THE SHADOWS.' CONGRATULATIONS ON YOUR SUCCESSES. I WILL PAY YOU WELL FOR THE DRAGON'S BLACK HEART AND BLACK EGGS , IF YOU ARE INTERESTED. AR… |
...
| `ECL5.DAX/0x33` | `0x1A8C` | `matched` | `wizard-tower.efreet-band` | AN EFREET LEADS A BAND OF DARK ELVES AGAINST YOU. |
...
| `ECL5.DAX/0x35` | `0x0D4B` | `matched` | `dark-elf-caves.girl-escaped-dragons-pursue` | THE GIRL ESCAPED, BUT THE DRAGONS RUSH AFTER HER. |
...
| `ECL6.DAX/0x40` | `0x0E2C` | `matched` | `myth-drannor.red-plume.follow-then-warning` | FOLLOW ME. THANK YOU FOR YOUR MAGNANAMOUS GESTURE. |
...
| `ECL6.DAX/0x40` | `0x0F5C` | `matched` | `myth-drannor.red-plume.trap-warning-alone` | A WHISPERY VOICE CRIES OUT,'BEWARE, A TRAP.' THE RED PLUME SNARLS AND CUTS OFF THE VOICE WITH A GESTURE. |
