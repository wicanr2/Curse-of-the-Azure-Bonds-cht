...
形狀欄位：`entry9`／`entry10` 是**骰數、面數、加值全是立即數**的那一種，數字可以直接用；`computed` 是有一段由程式算出來（火球 `等級d6`、魔法飛彈 `(等級＋1) div 2` 顆、寒冰錐 `等級d4 ＋ 等級`、電擊觸手 `1d8 ＋ 等級`），要人去讀那一支；`flat` 是不擲骰的固定值；`other_finish` 是有擲骰但收尾不是那兩支（豁免即死、反擊、對抗）。
