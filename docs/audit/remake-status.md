...
| 第一人稱 fidelity（幾何） | 18432 張畫面收斂成 585 種看得到的牆面配置（＋15 種牆型值），**已比過 40／585 種** | `fp-screen-plan.md`（`cmd/fp-screen-plan`） | ⚠ **分子只走得到提爾佛頓那一張。** remake 側第 674 輪已用 `-first-person` 打通（下水道畫得出來，與提爾佛頓差 18,244 像素）；卡住的是**原版側**：把存檔跳到別張圖的任一格還沒做出來。⚠ 比完 585 種也不等於第一人稱正確：天空色、牆磚選圖與調色盤跟著地圖宣告走，各自要比。⚠ 分母含全部 4,608 格（其中 187 格四面都是牆）：密室也會被玩家站上去，開新遊戲的起點就是一格密室。 |
