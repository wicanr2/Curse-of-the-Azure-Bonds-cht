...
| 玩家文字資料分離（Go 漢字 literal 債） | 合計 887 次（`localization_debt` 0／`frontend_ui_debt` 15／`runtime_ui_debt` 872） | `go-han-literals-baseline.json`（`cmd/coab-audit`） | 這是**債務**不是進度：數字要往下降。分類只是遷移排序用的 heuristic。 |
...
| 第一人稱 fidelity（幾何） | 18432 張畫面收斂成 585 種看得到的牆面配置（＋15 種牆型值），**已比過 76／585 種** | `fp-screen-plan.md`（`cmd/fp-screen-plan`） | ⚠ **分子仍然只有提爾佛頓那一張圖**（585 種裡它佔 108 種）。第 675 輪把它從 40 拉到 76：用自製存檔把原版直接放到指定格子（`tools/dos-oracle-jump-capture.sh`），不必用走的。129 張擷取逐格比完只差 **1 格**（(1,7) W，既有的那一格）。⚠ **跨圖還是取不到 oracle**：原版的第一人稱地圖由存檔裡的 ECL 狀態決定，改存檔的地圖編號**不會換圖**（實測：寫 `-map-block 3` 之後拍到的畫面與 remake 的**提爾佛頓**同格逐格相同）。第 676 輪修好「載入原版存檔之後牆是空氣」：原版存檔沒有牆磚選圖，原版是重跑 ECL 段拿回來的，remake 改成查表補（19 個段各只發一組常數三元組，量過才做）；同一份存檔餵兩邊，(8,2) N 差 0 格。⚠ 比完 585 種也不等於第一人稱正確：天空色、牆磚選圖與調色盤跟著地圖宣告走，各自要比。⚠ 分母含全部 4,608 格（其中 187 格四面都是牆）：密室也會被玩家站上去，開新遊戲的起點就是一格密室。 |
