...
| 第一人稱 fidelity 的**分母** | 18 張地圖、17684 張畫面收斂成 **585 種**看得到的牆面配置 ＋ 15 種牆型值 | `fp-screen-plan.md`（`cmd/fp-screen-plan`） | ⚠ **這只是分母，沒有分子。** 目前沒有「哪些簽章比過了」的台帳；spec 1134 比過的 20 張也沒記成簽章。而且比完 585 種也不等於第一人稱正確——天空色、牆磚選圖與調色盤跟著地圖宣告走，各自要比（spec 1134 那個黑掉的天花板在 20 張裡張張都錯，逐格比對本身抓不出「兩邊照同一個錯宣告畫」）。 |
...
| UI fidelity 的**分子**（比過幾張） | **沒有數字** | — | 分母有了（見上一列：585 種幾何 ＋ 15 種牆型）。缺的是台帳：spec 1134 比過提爾佛頓 20 張（19 張相同、1 張差一格），但那 20 張沒有記成簽章，所以「比過幾種」目前是零記錄。下一步是那份台帳，不是再建工具。 |
