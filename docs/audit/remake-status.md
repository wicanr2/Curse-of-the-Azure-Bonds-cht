...
| 第一人稱 fidelity（幾何） | 18432 張畫面收斂成 585 種看得到的牆面配置（＋15 種牆型值），**已比過 125／585 種** | `fp-screen-plan.md`（`cmd/fp-screen-plan`） | ⚠ **已拍完兩張圖**（提爾佛頓、下水道），289 張擷取逐格比對只差 **1 格**（(1,7) W）。第 683 輪找到跨圖一直失敗的原因：擷取腳本那一行寫死 `-area 2`、沒帶 `-ecl-block`／`-map-block`（第 678 輪那次字串取代沒命中而我沒斷言）⇒ 檔名寫著別張圖、內容是提爾佛頓。修好之後同一格 0／7744。⚠ **其餘 14 張圖還沒拍**，但用同一支腳本就拍得到。⚠ 比完 585 種也不等於第一人稱正確：天空色、牆磚選圖與調色盤跟著地圖宣告走，各自要比。⚠ 分母含全部 4,608 格（其中 187 格四面都是牆）：密室也會被玩家站上去，開新遊戲的起點就是一格密室。 |
