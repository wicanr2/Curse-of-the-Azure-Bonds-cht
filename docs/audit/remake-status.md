...
| 玩家文字資料分離（Go 漢字 literal 債） | 合計 864 次（`localization_debt` 0／`frontend_ui_debt` 15／`runtime_ui_debt` 849） | `go-han-literals-baseline.json`（`cmd/coab-audit`） | 這是**債務**不是進度：數字要往下降。分類只是遷移排序用的 heuristic。 |
