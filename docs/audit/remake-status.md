...
| 第一人稱 fidelity（幾何） | 18432 張畫面收斂成 585 種看得到的牆面配置（＋15 種牆型值），**已比過 40／585 種** | `fp-screen-plan.md`（`cmd/fp-screen-plan`） | ⚠ **分子只走得到提爾佛頓那一張。** 貪婪清單接下來四格在別張圖上，而兩側的管線都還不通：原版側跳到別張圖的存檔會不會帶對牆組沒驗過；remake 側已驗過**不通**（`-geo-set`／`-geo-block` 單獨用只畫得出 3 種顏色的空畫面，第一人稱要靠 `-tilverton-dungeon` 那條流程準備 piece set，而它只認提爾佛頓）。⚠ 比完 585 種也不等於第一人稱正確：天空色、牆磚選圖與調色盤跟著地圖宣告走，各自要比。⚠ 分母含全部 4,608 格（其中 187 格四面都是牆）：密室也會被玩家站上去，開新遊戲的起點就是一格密室。 |
