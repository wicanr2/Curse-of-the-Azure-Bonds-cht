...
| 第一人稱 fidelity（幾何） | 18432 張畫面收斂成 585 種看得到的牆面配置（＋15 種牆型值），**已比過 31／585 種** | `fp-screen-plan.md`（`cmd/fp-screen-plan`） | ⚠ 比完 585 種也**不等於**第一人稱正確：天空色、牆磚選圖與調色盤跟著地圖宣告走，各自要比——spec 1134 那個黑掉的天花板在已比的 20 張裡張張都錯，逐格比對本身抓不出「兩邊照同一個錯宣告畫」。⚠ 分母含全部 4,608 格（其中 187 格四面都是牆）：密室**也會**被玩家站上去，開新遊戲的起點就是一格密室。哪些格子真的走得到是另一件事，要連通性走訪加腳本傳送，這裡沒做。 |
