...
的原則逐條判讀，**321 個可完全判定並標為 `已解讀`／`exact`**，另有 386 個被判定為 `邊界碎片`：
...
| 單一呼叫包裝 | 39 | 整個 body 只準備參數（推參數／立即數／全域／far pointer）並執行一個 call |
| 尾呼叫 | 26 | 最後一條是無條件跳躍，**且前面每一條都是參數準備**（資料搬移或自我清零） |
| 全域搬移 | 1 | 把一個具名／固定位址的值搬到另一個，沒有其他動作 |
| 呼叫序列 | 35 | body 只有一連串 `call`，沒有任何資料處理（overlay 的 unit 初始化就是這個形狀） |
...
其餘維持 `待解讀`：它們有真正的控制流或資料處理，形狀不在可判定集合內。

## 第四種狀態：`邊界碎片`

判讀過程發現 **364 個「函式」body 內既沒有 `ret` 也沒有尾跳躍**。抽驗證實
它們不是完整函式：

- `START.EXE:11638h` 的反組譯是 `add ch, cl／aas／loope`——IDA 把資料誤判成
  程式碼。
- `START.EXE:197F0h` 以 `pop si／pop dx／pop cx` 開頭，明顯是某個函式的中段。

這些是 IDA 在 raw overlay 上建錯的函式邊界，**既不是已解讀也不是待解讀**。
把它們混進待解讀會讓剩餘量虛胖，標成已解讀更是造假。因此台帳新增
`邊界碎片` 狀態，`cmd/re-ledger` 單獨計數。

第二種碎片形態（第 572 輪補上）：**函式沒有 `sub sp, N` 配置區域變數，卻讀寫
`[bp-N]`**。那個堆疊框架不是它建立的，所以它是別的函式被切開的後半段。這類
在 overlay 內特別多（overlay 是 raw binary，IDA 沒有函式結尾資訊）。

⚠ **這不代表那些位元組不重要**：真正的函式體仍要以位址範圍重讀（第 563／564
輪的 `STOREVALUE` 與 `ON GOTO` 就是這樣讀出來的）。碎片只是說「這一列不對應
一個完整函式」。

## 判準收緊的一次修正

`尾呼叫` 第一版只檢查最後一條是 `jmp`，沒有檢查前面的指令——那違反本輪自訂的
「整個 body 都被解釋掉」原則。抽驗時看到 `mov ah, 3／xor bh, bh／jmp sub_1A0B8`
被判成尾呼叫，但那兩條設定完全沒被解釋。收緊為「前面每一條都必須是資料搬移
或自我清零」之後，數量從 53 降到 26——**降下來的那 27 個才是正確結果**。
