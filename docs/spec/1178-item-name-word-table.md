...
紀錄的 `+00h..+29h` 是一個 Pascal `string[41]`，存的是**未鑑定時的名字**，而且
`sub_467` 每次組名都會把結果寫回去——所以它是快取欄位，不是權威資料。45 件對不上
的來源就是這個：

| 差異 | 例 |
|---|---|
| 數量前綴 | 存 `10 Arrow`，成分只組出 `Arrow` |
| 未鑑定的替身名 | 存 `Deck`／`Beaker`／`Small Raft`，成分是 `Gloves`／`Defoliation`／`Staff` |
| 縮寫 | 存 `Magic User Scroll`，詞表是 `MU Scroll` |
...
- 未鑑定替身名（`Deck`／`Beaker`…）出自哪裡：那是 `ITEM*.DAX` 出廠就有的字串，
  中文版還沒有對應的翻譯管道。
