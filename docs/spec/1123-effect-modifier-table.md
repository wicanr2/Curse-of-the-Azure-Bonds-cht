...
| `DS:6F95h` | `damage_element` | 傷害屬性旗標：**bit 0 火、bit 1 冷、bit 2 電** |
| `DS:6F9Ch` | — | 未定 |

傷害屬性旗標的三個位元各有**兩個獨立證人**：抗性 handler 的守衛遮罩
（抗火看 bit 0、抗寒看 bit 1），以及傷害法術推進 `sub_F06` 的那個值
（火焰打擊 `09h`、冰風暴 `0Ah`、電擊之握 `0Ch`，spec 1124）。
`sub_F06` 的 `mov ds:6F95h, al` 就是那個參數的去處。
...
| `partial` | 有數字，但還有沒解析的指令（動角色記錄、呼叫別支）| 18 |
| `inert` | 只有序幕與 `retf`，什麼都不做 | 12 |
| `unread` | 有內容但沒有可辨識的修正 | 105 |
...
這個形狀收成 `guard_global` ＋ `guard_mask`：修正照樣進表，但**套用端要先確認
那個旗標**。抗寒只減半冷傷害、抗火只減半火傷害——少了這一層，它們會減半所有
傷害，而那個錯在英雄被火燒時看不出來，只在被凍時才顯得「怎麼也減半了」。

同一個全域同時被「設定」與別的運算動到時，整組退回沒解析：那是兩條分支
（`if 不足 then 0 else 折半`），照順序套會先歸零再折半，結果永遠是 0。
...
- 已接的四處：豁免（`RollSavingThrow`）、士氣（`CheckMorale`）、
  移動（`MonsterApproach`）、傷害（`CastDamageDice`，帶傷害屬性旗標）。
- `combat.AffectKindIsInterpreted` 把「表裡有數字」也算進判讀範圍，
  所以法術覆蓋台帳的分母跟著動。

## 明確不宣稱

- 沒有宣稱 `6F9Ch` 是什麼；也沒有宣稱 `6F95h` 的 bit 3 是什麼——
  傷害類法術幾乎每一支都設它。
- 沒有宣稱 `unread` 那 105 個 handler 各自做什麼；它們多半是傷害、狀態、區域
  那一類，不是加減型。
- 沒有宣稱 `partial` 那 18 個「沒解析的部分」不重要——致盲除了三個全域之外還動
  角色記錄的 `+19Ah`／`+19Bh`，那一段沒有進表。
- 沒有宣稱命中那條路上 `modifier` 的正負號怎麼套進公式；已接的四處方向都由
  各自的呼叫端證實，命中那一條還沒接。
