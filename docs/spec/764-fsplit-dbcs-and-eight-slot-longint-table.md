# 764 — PC-98 把 `FSplit` 改成能認 Shift-JIS，以及扣款與累計
...
`DS:6F70h`（PC-98 `DS:0A00Ah`）起是 **8 格 4-byte 的量**，索引與金錢欄位名稱表
（`DS:0F93h`）同一套：0 = Copper、1 = Silver、2 = Electrum、3 = Gold、
4 = Platinum、5 = Gems、6 = Jewelry，第 8 格（`6F8Ch`）另計。三處用法一致：

| 出處 | 用法 |
|---|---|
| spec 756 `overlay-21:0F5Bh` | 測前 7 格有沒有任一非 0、第 8 格單獨測 |
| spec 757 `overlay-21:019Eh` | 清前 5 格，再把 `div 5` 寫進 Platinum、`mod 5` 寫進 Gold |
| 本輪 `overlay-21:0A03h` | 依索引把數量以 32-bit 加上去 |

角色記錄的 `+0FBh` 是同一套索引的 **7 個 word**（spec 772 的金錢顯示逐格對過），
所以這一支是「從角色身上扣掉，累計到全域的那一份」。
