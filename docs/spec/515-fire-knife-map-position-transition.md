...
這不是「已找出 DOS 外部移動函式的 exact 實作」聲明。`4BF0h／4BF1h` 的
DOS writer→ECL consumer 完整橋接仍未閉合；`(13,10)` 是由 raw ECL helper、
PC-98 `MOVEFORWARD` 對照、GEO component 分析與既有事件路徑共同支持的
...
| GEO component | `GEO2.DAX` block 3；16×16 decoded grid | `(1,8)` 所在 component 無法以合法 wall movement 到 `(13,10)`；從 `(13,10)` 也不能 BFS 到 `(8,15)` | `exact`（geometry）；外部 handoff 語意 `strong inference` |
