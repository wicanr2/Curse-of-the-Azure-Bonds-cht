...
⚠ **但配置與釋放的算式不一樣**：spec 934 的 DOS 端 `GetMem(總量 ＋ 17h)`
只算「張數 × 每張位元組」；本支多算了一個 `高 × 寬 × 張數`。
**本規格沒有讀 PC-98 側的配置函式**，所以不宣稱兩邊一致——
有可能是 PC-98 的表面本來就多一塊，也有可能是原版的錯誤。
釋放遮罩的條件也不同（本支多一道 `寬 > 16h`）。
...
- 沒有宣稱 `<sub_1A721h>`（執行期錯誤）、`<sub_187ECh>`、`<sub_1883Eh>`、
  ...
  的內部。
- 沒有宣稱 PC-98 側的表面配置函式（所以不宣稱它與 `13B55h` 的算式一致）。
- 沒有宣稱 `13B55h` 那道 `寬 > 16h` 的門檻為什麼是 `16h`。
