...
實際 `ECL1.DAX` 的三個 block 已能解析出第五個 initial entry point `0x8014`。CLI 的 `-graph` 現在優先從第五個入口開始；若 header 不完整才保留 offset 0 fallback。下一步是建立完整 regression，並確認 code address 與 packed text／選單事件的對齊。
