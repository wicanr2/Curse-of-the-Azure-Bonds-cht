...
## ★★ 法術屬性表 `DS:37DBh` 的 `+5` 是目標模式

```pascal
模式 := byte[37DBh ＋ 法術 × 16 ＋ 5] and 0Fh;
```

表基底與筆距和 spec 1005 的效果碼 63（`byte[DS:6F97h × 16 ＋ 37DBh] < 4`）
是同一張表；PC-98 是 `DS:61B5h`（本支用 `61BAh` ＝ `61B5h ＋ 5`，
與 DOS 的 `37E0h ＝ 37DBh ＋ 5` 一致）。
...
- 沒有宣稱法術屬性表 `DS:37DBh` 其餘 15 個 byte 是什麼。
