...
- `VIEW／MAGIC／SAVE` 已接入目前 remake 的只讀／spell-slot／party-save boundary；`ALTER → ORDER` 已接入 party reorder。`DROP／SPEED／ICON／PICS` 與 `FIX` 仍以明確 placeholder 保留未完成 routine 邊界。
