...
`4000h` ＝ 16384 **bytes** ＝ **16 KB**，正是 EMS 的分頁大小（不是 16384 個段落——
那會是 256 KB）。累加時先塞 `3FFFh` 再除，是**無條件進位**的寫法
（`(總和 + 分頁大小 − 1) div 分頁大小`）。

所以 **`es:[8]` 是位元組數，不是段落數**。`1A34Fh` 用 `(es:[8] + 1) shr 1` 算出
`movsw` 的次數，也只有在它是位元組數時才對得上。
