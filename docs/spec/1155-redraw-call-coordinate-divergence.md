...
- ⚠ **`C04D` 條件還沒拿掉。** 補上 producer 之後再試一次，失敗的座標從
  `(0,1)`（垃圾值）變成 `(5,3)`（真座標）——資料對了，但走的路線仍與
  既有的火刃巢穴路線測試分歧。剩下的差異是「**哪些 `CALL` 該投影**」，
  不再是「投影的值哪裡來」。那要另外一輪，判準是先解釋清楚
  `ECL2/0x04:160Ch` 那一處（`COMBAT` 之後 `COMPARE 7EC7 80h / IF >` 才跳進去
  的退格分支）在該路線上什麼時候會成立。
...
- 沒有宣稱那 23 處在 remake 現況下玩家一定會卡住——只宣稱**投影沒有發生**，
  而原作在同一點座標已經生效。
- 沒有宣稱 `ECL2/0x04:160Ch` 的守衛 `COMPARE 7EC7 80h` 是什麼意思。
