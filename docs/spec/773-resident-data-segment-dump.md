...
——那是**把兩個 word 或起來測非零**，不是載入遠指標測 NIL。`DS:6F70h` 從頭到尾
都是 8 格 32-bit 的量，三支函式（測非零、寫商餘、累加）用法一致。

> ★ **兩種寫法長得幾乎一樣，判別點是有沒有 `les`／`lds`。**
> `les di, ds:X` 之後再 `or` 兩個 word，測的是遠指標是否為 NIL；
> 沒有載入段暫存器、直接 `mov ax, ds:X` ＋ `or ax, ds:X+2`，測的是一個
> 32-bit 量是否為零。只看 `or ... jz` 這個形狀會把後者誤讀成指標，
> 進而替那塊記憶體編出一個不存在的指標陣列型別。
