...
- [第三百三十二輪尤拉什戰區與指揮官等候室](./332-yulash-war-zone-entry.md)（`READY`：限 Hillsfar→Yulash、十二名紅羽衛、ECL3/GEO3 block 16、檢查哨談判與 `(0,3,E)`；★ 護送段是**腳本自己走過去的**——`ECL3/0x10:192Ah`..`1997h` 是查表驅動的走位迴圈（`GETTABLE` 讀方向表進 `C04D` ＋ `CALL C01Eh`），方向序列南南南西，從 `(1,0)` 走四步到 `(0,3)` 再由 `1991h` 轉朝東，**沒有外層 dispatcher 的 teleport**；⚠ 先前「ECL dialogue 不寫 map registers」是**假零**：座標從頭到尾沒當過 operand，只掃「誰寫 `C04B`／`C04C`」看不到查表移動迴圈）
...
- [1166 DOS 的角色記錄少的那一格就是 `MONSTERTYPE`](./1166-dos-charrec-drops-monstertype.md)（`READY`：spec 1164 留的未定項有答案了——角色樣本在 `+14Ch`..`+186h` 全是 0 或指標分不出來，**原版怪物資料分得出來**：`MON*CHA.DAX` 每一筆的 `+14Ch`＝0、`+14Dh`＝小數字、`+14Eh`..`+151h`＝`00 xx 1E 00`，照 DOS 版面讀是「`NUMITEMS`＝0 ＋ 物品鏈指標（位移 `000C`、節區 `1E86`）」——位移小、節區隨資料往前走，**堆積配置的樣子**；照 PC-98 版面讀會得到「一隻怪帶 12 件物品、節區永遠是 `001E`」，兩者都不可能；尾端再驗一次：`CURRENTHP`／`MOVE` 在 `+1A4h`／`+1A5h` 給出人類 12 格、鱷魚 15 格，不挪則 `MOVE` 落在記錄外；★ remake 這一側 `internal/monster` 先前把 `+14Ch` 記成 `RawMonsterType`（PC-98 的位移套在 DOS 記錄上），改名為 `RawItemCount`——該欄在遊戲資料裡恆為 0 且沒有規則讀它，沒有行為變化）
- [1167 驅散不死接進 remake](./1167-turn-undead-in-the-remake.md)（`READY`：規格（spec 834／905）早就在，缺的只是接線——戰鬥選單先前七項少了 `退散`；★★ 矩陣逐 byte 取自資料段 dump，**`0` 與 `−1` 不能統一**：兩者驅離／摧毀行為相同，但額外配額那一條寫的是 `if 有號(v) < 0`，`0` 那幾格摧毀之後不加回配額，測試用「同樣自動摧毀、同樣三個目標、同樣配額 1，欄 8 做得到三個、欄 6 只做得到一個」兩面釘住；★ 選單照 spec 905 改成動態：牧師槽等級 > 0 且 `TRIEDTOTURN` ＝ 0 才接上，用過就消失，⚠ 條件裡**沒有**「場上要有不死生物」——原作照樣讓你按，然後印 `Nothing Happens...`；⚠ 「第一個沒過就整批停」在目前的挑選順序下看不出差別（矩陣每欄不遞減 ＋ 目標由弱到強），照抄不是被釘住；★ 這一作六章裡唯一的不死生物是 DRACOLICH，所以幾乎用不到——但缺了它牧師的選單就與原作不同）
- [1150 增補：`2Dh CALL` 七個選擇子的全量普查，以及那段過期的正文](./1150-ecl-call-dispatch.md)（`READY`：★ 七個選擇子現在有測試守著且**兩個方向都擋**——corpus 用四個（`2E10h` 125／`B200h` 19／`C01Eh` 13／`6803h` 11），`C018h`／`8000h`／`8001h` 三個都是 **0**，而且**沒有第八個**；⚠ 那三個零之所以算數，是因為測試**先做正對照**（四個有在用的必須掃到 125／19／13／11）——少了正對照，掃描壞掉會長成「三個都是死路」這種自洽而錯的結論，本輪已經被這種假零咬過兩次；⇒ **未接的三個目標都走不到**，`2Dh` 的 `partial` 只剩 `ViewMirror.Block`／`Written` 兩個 remake 側補丁；★ 正文那段「remake 沒有那五個髒旗標、改用回頭掃 `SaveWrites` 的啟發式」是**過期的**——spec 1172 早就把五個髒旗標建出來、啟發式整支拿掉了，已改寫；順帶關掉「不宣稱 `GODUEL` 做什麼」：它是**決鬥**，複製目前角色成叫 `ROLF`／`ロルフ` 的 NPC 並設 `8B56h`）
- [1182 每個 overlay 的原始單元名，以及 `24h COMBAT` 的第四支](./1182-overlay-module-names-and-combat-dispatch.md)（`READY`：★★★ **`overlay-04` 是神殿不是營地**——spec 1030 把它讀成「營地主選單」、spec 1095／1149 沿用，還因此說 remake 把 `7EE2h` 標成 `TempleRequested` 是「位址對、語意不符」：**反了**；兩份一手資料都說神殿——PC-98 的 Borland 符號是 `LOADTEMPLE`／`GOTEMPLE`（營地在 `overlay-15` 的 `LOADCAMP`／`DOCAMP`），而 DOS `overlay-04` 自己的字串是 `how can we help you?`／`Heal View Pool Appraise Exit`／`Raise Dead` 與「a priest says…」；spec 1030 讀到的那張 `H`/`V`/`T`/`P`/`S`/`A` 選單也是神殿的，不是營地的 Save／Rest／Magic／Alter；★★ **全部 36 個 overlay 的單元名一次認完**（`cmd/borland-symbols -overlay-names` → `docs/audit/overlay-module-names.md`）：Turbo Pascal 每個 overlay 的第一個 entry 一律是 `LOADxxx`，`xxx` 就是單元名——DOS 版沒有除錯符號，`overlay-NN` 一直只能靠內容推而**推錯不會有徵兆**；編號一致有兩處獨立印證（spec 1156 的 POSTCOM ＝ `overlay-05`、spec 1153 的 THREED ＝ `overlay-30`）；★★★ **`24h COMBAT` 是四選一不是三選一**：先前只知道第四支叫 `sub_184B`、內容沒讀，讀完是 `bank1^[5C4h] = 1 ⇒ 神殿，否則 ⇒ 戰後處理（overlay-05 DOPOSTCOMBAT）`——這正是那 46 處「沒擺過怪的 `24h`」的去向（腳本先用 `27h TREASURE` 堆好戰利品，再用 `24h` 開分配畫面，`overlay-05` 的字串就是 `The party has found Treasure!`／`Each character receives `／`View Take Pool Share`）；★ 收尾重繪的閘門 `bank0^[1CCh]` 就是 spec 1181 從 `21h LOAD FILES` 認出的那一格（ECL 格 `4BE6h`），兩支 handler 互相印證；remake 除了改名，**第四支也接上了**：沒擺過怪的 `24h` 改發 `PostCombatRequested`（原作沒有「零隻怪的戰鬥」），10 個既有測試因此從「零隻怪的戰鬥」變成「戰利品分配」——每一個都是真的撿東西現場（外圍廢墟倉庫、逃亡者暗格、賭場、內城臥房、埋葬谷營地、火刀辦公室、洞穴死精靈皮袋）；★★ 商店與神殿旗標的 producer 是**腳本自己**——spec 1095 說「1,355 條指令裡沒有一條寫過（已全掃確認）」是**假零**，掃全 corpus 是 `7EE2h` 4 處、`7F6Ch` 9 處，慣用法一律是 `CLEARMONSTERS` ＋ `SAVE 01 <旗標>` ＋ `COMBAT`；兩支都走得到而且 remake 本來就接得起來（已用原版資料跑過驗證），釘假零的 `TestCampRequestFlagHasNoProducer` 換成正對照；★★★ **`24h` 轉 `done`**：第一支的第二個條件 `DS:8B56h` ＝ **決鬥旗標**（spec 917 早就解出：`8B56h <> 0` 時戰後印 `You have won the duel.`／`The duelist receives `），整個執行檔只有 `GODUEL`（`overlay-07 entry#25`，分身叫 `ROLF`／`ロルフ`，spec 626）設它，而 `GODUEL` 只有 `2Dh CALL` 的 `8000h`／`8001h` 到得了——**corpus 兩個都沒用到**（spec 1150）⇒ 那一支在 CoAB 走不到，remake 用怪物鏈非空覆蓋了整個可達條件；⚠ 三個「已經解出來但沒被連起來」的斷言散在 917／626／1150 三份規格裡，這一輪只是把它們接上——**要說「解不出來」之前先 grep 自己的 docs**；⚠ 不宣稱 `DS:8B56h` 是什麼、兩個 stub offset 各對 `LOADxxx`／`GOxxx` 哪一個、`overlay-00`／`01` 的單元名）
- [1183 誰寫地圖暫存器：`720Fh`／`7210h`／`7211h` 的全域普查](./1183-map-register-writer-census.md)（`exact`：DOS 執行檔逐位元組普查，`cmd/dseg-writers` ＋ [`docs/audit/dseg-writers-map-registers.md`](../audit/dseg-writers-map-registers.md)；★★★ **寫入者只有五處**——`overlay-07` ECL2 的 `STOREVALUE` 鏡射（`0EA2h`..`0EEFh`）與 `MOVEFORWARD`（`1B53h`..`1BA4h`）、`overlay-11` INIT、`overlay-14` MOVEMENT、`overlay-16` LOADSAVE 的 **5 byte 區塊複製**；⇒ `overlay-02` INTERPET（擁有 `GOECL` 與 block 載入）**一次都沒寫**卻讀了 23 次 ⇒ **原作沒有「換 ECL block 時的引擎進場放置」**，spec 1172 用來正當化 `ViewMirror.Block` 的那句話是錯的；★★ 進新地圖的落點是**腳本自己寫的**，而且不是統一步驟——`ECL2/0x03:00DBh`（下水道）還帶條件 `4C2A == 1`，`ECL3/0x10:006Ch`（猶拉什）則是無條件；⇒ `ViewMirror.Block` 重新分類成「保護 game pack 宣告的 `spawn` 不被腳本蓋掉」的擋板，拿不拿得掉變成**資料問題**（已知一處對不上：下水道腳本說 `(0,0)` 朝南、宣告的 `spawn` 是 `(0,1)`，還沒用原版判過誰對）；★ LOADSAVE 那個 5 byte 區塊獨立確認 SAVGAM 的 map state 就是 `720Fh`..`7213h` 五格（第 660 輪的 `LoadSAVGAMPrefix` 修正）；⚠ **方法**：線性掃描有偽陽性、沒有偽陰性，走控制流的掃描反過來——要下「不存在」的結論兩種都要跑；⚠ 第一版把 `cmpb $0, 0x7211`（`80 3E 11 72 00`，`/7` ＝ CMP）算成寫入，於是 THREED 被列成寫入者——**那正好是錯誤結論需要的證據**，分類已用手造位元組的單測釘住）
- [1184 進場落點是誰決定的：逐張第一人稱地圖的盤點](./1184-map-spawn-sources.md)（`exact`：原版 ECL 控制流走訪 ＋ game pack 宣告值逐張對照，`cmd/map-spawn-audit` ＋ [`docs/audit/map-spawn-sources.md`](../audit/map-spawn-sources.md)；★★★ **沒有任何一個宣告的 `spawn` 等於腳本寫過的座標**——18 個 `first_person` 定義裡 11 個完全靠腳本（沒宣告 `spawn`）、4 個宣告了但與腳本座標**對不上**、3 個沒有 `script_block`（只當同 area 的退回值），`script-agrees` 與 `declared-only` 都是 **0**；⇒ 那四個宣告值不是「把腳本的進場寫入抄下來」，猶拉什的 `(0,3)` 是護送段**走完四步之後**停的格子（spec 332），腳本從來沒把它當立即數寫過 ⇒ 宣告值記的是「玩家最後在哪」不是「腳本把隊伍放在哪」；★★ ⇒ `ViewMirror.Block` 保護的是**四個還沒被判過的宣告值**，不是 18 張地圖——其餘 11 張根本沒宣告，那一格對它們是 no-op；要拿掉它得先用原版把那四個判掉；⚠ 第一版把猶拉什判成 `script-agrees`：`1991h` 那一組**只寫朝向**（走位收尾），而比對只看「有寫的欄位」，於是 X／Y 都是 nil 的那一組無條件相符——**正好是最需要被標成 `mismatch` 的那一張**；判準改成必須同時寫過 X 與 Y，已用單測釘住）
- [1185 第一人稱 fidelity：585 種畫面，已比過 12 種](./1185-first-person-fidelity-denominator.md)（`exact` 計算 ＋ `derived` 可見性模型，`cmd/fp-screen-plan` ＋ [`docs/audit/fp-screen-plan.md`](../audit/fp-screen-plan.md)；★★★ 原版截圖 oracle 第 572 輪就建好了（spec 1134 也比過 20 張），**缺的不是工具是分母**；★★ 兩個軸要分開算——幾何只跟牆的**有無**有關，貼圖是 `LOADWALLSET` ＋ 地圖宣告、與站在哪一格無關；⇒ 18 張地圖、4,421 個站得住的格子、17,684 張畫面**收斂成 585 種**看得到的牆面配置 ＋ **15 種**牆型值 ⇒ 逐張比對是做得完的事，而「新增幾種」那一欄直接就是工作順序（提爾佛頓 108 種全新、`geo5/0x33` 104 種、而 `inner-ruins` 與 `geo6/0x45` 各只新增 1 種）；⚠ **前兩版都算出誠實但沒有用的數字**：第一版把牆型寫進簽章得到 15,693 種（兩個軸混在一起，去重等於沒做），第二版改成二元有無仍有 14,294 種（簽的是**原始鄰域**而不是**看得到的東西**——走廊裡牆後面那幾排根本不會畫）；加上遮蔽才降到 585 ⇒ **分母的定義跟數字一樣需要被檢查**；★ `-covered` 把 `docs/reference/original-dos/first-person/index.tsv` 那 20 張換算成簽章 ⇒ **只蓋掉 12／585**（同一條走廊上五格朝同一方向常常是同一張畫面），並用貪婪法排出「接下來拍哪一格」；⚠⚠ **「站得住」那個過濾器是錯的**：第一版把四面都是牆的格子排除，`-covered` 一跑就有四張擷取對不到任何格子——而那是 `GEO2/0x01 (7,13)`，**開新遊戲的起點**，密室但每個玩家都看得到 ⇒ 分母改收全部 4,608 格（其中 187 格是密室，只記數字不過濾）；工具照實回報「對不到」而不是靜靜丟掉，矛盾才浮得出來——**過濾器也是斷言**；⚠ 明確不宣稱：沒有宣稱比完就等於正確（天空色／牆磚選圖／調色盤各自要比——spec 1134 那個黑天花板在 20 張裡張張都錯，逐格比對抓不出「兩邊照同一個錯宣告畫」）、也沒有宣稱這 585 種玩家都走得到）
- [1181 `21h LOAD FILES` 自己會寫的兩格 ECL 記憶體](./1181-load-files-map-cells.md)（`READY`：`21h` 不只是「記下請求、上層換檔」——載 3D 地圖那一路 handler **自己寫兩格**：`bank0^[18Ah] := o[1]`（格 `4BC5h`）與 `bank1^[592h] := 0`（格 `7EC9h`），條件是 `o[1] <> 0FFh`、`<> 7Fh` 且 `bank0^[1CCh] <> 0`（格 `4BE6h`）；★★★ **`7EC9h` 腳本讀得到**：全 corpus 34 處存取，腳本一路寫 `FFh`，而唯一的讀取端 `ECL2/0x03:00F6h` 是 `COMPARE 7EC9 FF / IF = / SAVE 00 7ED5`，清掉它的**只有這個 handler** ⇒ remake 不寫的話，腳本設過 `FFh` 之後就永遠停在那個分支上，這是控制流分歧不是表現層問題；★ `4BE6h` 的 22 處寫入全部落在 ECL1 的三個世界地圖 block（`0x50`／`0x51`／`0x52`）在 0／1 之間切，配上 handler 兩條互斥條件（非零載 3D 地圖、為零載大圖），形狀是「現在是不是第一人稱畫面」——只宣稱它決定走哪一路；兩格都走 `recordStore` 所以會進 `SaveWrites`（只改 map 不記錄的話存檔重載會不一致）；`RunResult` 多帶 `LoadFilesLoaded3DMap` 讓上層知道原作走哪一路；⚠ 不宣稱 `DS:728Ah`（比對 `'P'`）是什麼、`LOADBIGPIC` 的 `'y'` 是哪一張、`4BC5h` 在 corpus 裡沒有任何讀取端所以不知道誰讀）
- [1180 攻擊次數以「半次」為單位，而整數次數靠**回合奇偶**換算](./1180-attack-blows-half-units-and-round-parity.md)（`READY`：★★★ `ADJUSTBLOWS`（DOS `overlay-13:0F12h`，PC-98 同名、**位元組完全相同**）＝ `(半次值 + (ROUND and 1)) div 2` —— 加的是**回合數最低位**，所以奇數半次值在回合之間交替（`3` → 2,1,2,1，平均 1.5 ＝ AD&D 的每兩回合三次），**寫成 `/2` 會每回合都少半次而且沒有任何徵兆**；★★ 名字全部是原作自己的 Borland 符號：`BASEATTBLOWS[0..1]`（`+11Ch`）／`ATTBLOWS`（DOS `+19Ch`）／`FIGBLOWS`／`ADJUSTBLOWS`／**`DS:758Dh` ＝ `ROUND`**、`758Eh` ＝ `LASTROUND`、`75E6h` ＝ `SHOWDUDEMOVE`（用 spec 1019 量出的 PC-98 ＝ DOS ＋ `3292h` 對位，`758Dh+3292h = A81Fh` 正是符號表裡的 `ROUND`）；★★★ **remake 先前讀 `+0A1h` 當攻擊次數**——那一格落在 `SPELLSKNOWN`（`+079h`，100 bytes）**裡面**、68 種怪物**全部是 0**、整包 overlay **沒有任何一處讀它**（同一套掃描對 `+0DEh`／`+11Ch`／`+19Ch`／`+0DDh`／`+151h` 分別找到 10／11／10／9／14 處，所以不是掃描方法的問題）⇒ 每一隻怪物都只打一下；改讀 `BASEATTBLOWS[0]` 後分布是 0×2／2×51／4×13／8×2，**15 種怪物**先前每回合少打一到三下；★ 槽的選擇也對上了：`GIANT SPIDER`／`PHASE SPIDER` 的 `{0, 2}` 正是 spec 1010 那句 `if 打手^[11Ch] = 0 then 槽 := 2`，`19Bh+j` 就是 `ATTBLOWS[j−1]`；加速／緩速改成乘除**半次值再換算**（先換算再加倍會先丟掉那半次）；★ 隊伍側也接上了：寫入端全在 `overlay-25`，門檻是**戰士 7 級／聖武士 7 級（規則書是 8 級，原作不同）／遊俠 8 級**，同一組門檻在現職業迴圈、前職業迴圈、直接查 `PREVIOUSLEVEL[2..4]` 三處一致；★★ **寫進 `+11Ch` 的立即數只有 `2` 與 `3`、沒有任何一處寫 `4`** ⇒ 這一款的隊員**永遠到不了一回合兩次**，AD&D 13 級那段沒有實作（有測試擋住「照規則書補一個 4」）；遠程的彈藥上限也照抄了，含「**數量 0 不會壓成 1**」那個像疏漏的組合；⚠ 不宣稱：`PREVIOUSLEVEL`（`+111h`）remake 還沒有模型（雙職角色靠前職業拿到的一次半算不出來）、spec 771 的 `overlay-13:1CE2h` 加成沒接、`15BCh+1`／`15C6h+1` 的內部沒讀、`LASTROUND` 限制什麼沒讀完）
- [1179 戰鬥員名字：68 種全部有繁中，而且由測試守著](./1179-combatant-name-localization.md)（`READY`：原版六章 `MON*CHA.DAX` 共 **68 種**相異名字，本輪之前 pack 只宣告 23 種——其餘 45 種在戰鬥選單、命中訊息、目標清單一律是大寫英文（`THRI-KREEN`／`NEO-OTYUGH`／`MARGOYLE`）；★ **漏一種不會有任何徵兆**：`LocalizeCombatantName` 查不到就原樣回傳、`found` 回 false 而呼叫端只在 true 時覆寫 ⇒ 沒有錯誤、沒有 log、測試不紅，只有玩家看得到，所以要用正對照回答；★★ 三條閘門各擋一個方向——原版有而 pack 沒有、宣告的 `source` 拼錯（規則永遠不生效）、兩種生物撞同一個譯名；第一條走 pack 的**執行時查詢**不是比對 JSON 鍵（宣告與翻譯各自對得起來但查詢路徑斷掉的話，比 JSON 一樣會過）；突變驗過（`THRI-KREEN` → `THRI KREEN` 兩條同時紅）；★ 譯名不是自己決定的：`coab-glossary.md` 明文規定戰鬥怪物名的唯一來源是 `combatant_name_rules`、稽核時併入，所以 `glossary-audit` 一口氣抓到 3 條與敘述用譯名衝突（`DISPLACER BEAST` 移位獸、`OTYUGH` 奧提尤格、`THRI-KREEN` 斯瑞克林 —— 手札裡早就有）；★★ 另外三條 `duplicate_rendering` **不是重複**是**原文別名**——`MON*CHA` 的名字欄有長度上限，`AKABAR BEL AKAS`／`FIRE KNIFE`／`WORG` 是 `Akabar Bel Akash`／`Fire Knives`／`warg` 的截短或單複數寫法；譯名表因此加了「原文另作 `X`」的別名折疊，折疊後這三對不再誤報而且**多一道一致性檢查**；⚠ 五個測試在斷言**顯示名**（`fighter.Name == "FIRE KNIFE"`）——顯示名會被翻譯，身分要看 `SourceName`，已全部改掉；順帶對齊 `item_name_E8`（火巨靈 → 伊弗利特）；不宣稱不上場的生物是否都在譯名表裡、戰鬥員名字有沒有第二個來源）
- [1178 物品名稱是三個編號組出來的：`DS:1040h` 那張 255 筆的詞表](./1178-item-name-word-table.md)（`READY`：第 N 筆在 `DS:1040h + N×15h`，Pascal `string[20]`，編號 `01h`..`FFh` 共 255 筆——指令在 `overlay-24:063Bh`（`mul 15h` ＋ `add 1040h`，基底是 0 那一格所以編號直接當索引）；資料側正對照：253 件物品照這規則組回去 **208 件與紀錄裡存的名字逐字元相同**，基底位移一格或兩格都是 **0 件**；★ **名字完全不含物品類別**，remake 先前拿 `item_type_XX` 當基底再貼成分，靠一條「和基底相同就不加」的守衛才沒穿幫；★★ **`+32h`（Plus）與 `+36h`（Cursed）不進名字**——加值走名稱編號 `A2h`..`A6h` 那幾個**可以被藏起來**的成分，原版 72 件帶 Plus 的物品有 54 件從頭到尾不顯示加值；⚠ `+35h` 不是布林是三個 bit，`+39h` 數量連 `1` 都印（`1 Oil`）；⚠ 紀錄的 `+00h..+29h` 是組名的**輸出欄位不是來源**——`sub_467` 一進來就 `mov byte ptr es:[di], 0` 清掉再重寫，出廠帶的 `Deck`／`Beaker` 是過期快取（那些字根本不在詞表裡），**不需要翻譯**；語系檔的 `item_name_XX` 由 54 筆補成 252 筆，並改正第 260 輪照 reference 填錯的編號（`9Fh` 之後整段偏 3 ⇒ 加值顯示錯數字）；翻譯層加一條「三成分而中間是連接詞就前後對調」讓 `Wand of Fireballs` 讀成「火球之魔杖」；不宣稱 `*` 標記那個 `DS:650Ah` 鏈在問什麼、那份過期快取是哪一版留下的、PC-98 的詞表位址）
- [1177 「站上去沒反應」自動分得出來：守衛的 `COMPARE` 拆出前置狀態](./1177-cell-sweep-guard-preconditions.md)（`READY`：逐格實測的「沒演出來」先前是一個混在一起的桶子——**remake 沒接**與**缺前置劇情**處置相反，而分辨的成本是一格一格反組譯；改法是把守衛的 `COMPARE <格子> <值> / IF <op> / EXIT` 拆成結構，把格子設成**避開 `EXIT`** 的值再站一次，演得出來就是「有接、只是缺前置」；★★ **不是「設成比對的值」就好**：`COMPARE 4C01 01 / IF >= / EXIT` 設成 1 反而保證離開，要往另一邊走（`<>` 設比對值、`=` 設 ＋1、`>=` 設 −1…），補上運算子之後從 3 格變成 5 格；全遊戲 250 個索引：中文 235 → **240**、沒演出來 15 → **10**、落回原文仍是 0；★ 剩下那 10 格逐格看過都是**盤點的限制**不是缺口，四種形狀（守衛比移動前快照、旗標被該段前導覆蓋、擷取到的守衛跨過真正的處理常式、處理常式本來就不講話）寫進報表抬頭）
- [1176 SAVGAM 的第四塊是**位元組定址**的程式碼視窗，先前匯出時整塊是 0](./1176-savgam-code-window.md)（`READY`：匯出原版格式時前三塊都從 session 重編，第四塊卻只是把匯入來源那一份原封抄過去 ⇒ **新開的遊戲沒有匯入來源，那一塊整塊是 0**；匯入之後換過段再存檔，寫出去的也是匯入當時那一段的碼；★ 這一塊**不能沿用前三塊的編碼**——`EncodeECLBank` 刻意跳過值為 0 的格子（原作分不出「寫過 0」與「沒寫過」），而位元組碼的 `00h` 是真的指令（`EXIT`），跳過會把程式打洞，所以第四塊有自己一對`DecodeECLCode`／`EncodeECLCode`、**0 照收**；★★ 量測：`cmd/ecl-cell-refs -range 8000-9DFF` 掃出 **3,012 處而 opcode 只有三種**（`GOTO` 1,767／`GOSUB` 1,087／`GETTABLE` 148），**沒有任何一處把值寫進區 3** ⇒「自我修改的碼會遺失」在這一款不成立，真正修掉的是「新開的遊戲匯出的存檔那一塊整塊是 0」；⚠ 那是**這一款**的量測不是引擎的性質，所以 remake 照通用做法存回去，不靠「反正沒人寫」這個前提）
- [1175 大型目標換另一組傷害骰：`+0DEh` 的 **bit 7 單獨成立**](./1175-large-target-damage-switch.md)（`READY`：`overlay-13:15EFh` 在攻擊結算當下切——**只有攻擊者有槽 0 武器才切**（天生攻擊不分大小）、切的是類別表 `+02h`..`+04h`（大型三連）、**加值是換掉不是相加**；★★ 六章 43 種怪物的 `+0DEh` 只有七個值，而 **`81h and 7 = 1`** ⇒ 只看低 3 位會把 **`BEHOLDER`／`BUGBEAR`／兩種巨蛛判成小型**——bit 7 是獨立的「傷害算大型」旗標，低 3 位是佔格大小，同一個位元組裝兩件事；remake 先前只留 `data[0xDE] & 7`，那個位元整個消失，現在 `Record.RawSize` 保留完整位元組；★ `Fighter` 帶兩組傷害三連加 `HasSlotZeroWeapon`，切換集中在 `WeaponDamageAgainst(目標)` 一處；★★ **怪物側接上了**（`monster.ProjectMonsterWeapon`，開戰掛完 `MON*ITM` 就投影槽 0 武器）——那是 spec 1120 留的「規則那一側還沒接」；`ReplaceFighterEquipment` 也一起帶新欄位，少了它換裝之後打大型目標用的還是上一把武器的骰；★ 整個戰鬥 overlay **只有這一個切換點**（另外兩處讀 `+151h` 的是背刺與射程），所以切換與近戰／遠程無關；⚠ 原作的 `+1A2h` 是**調整不是指派**（`− 小加值 ＋ 大加值`），連續兩次大型目標攻擊之間沒有重算的話加值會漂——沒有找到復原的地方，也沒有證據說重算是每次攻擊都跑，remake 這一側兩組都存著、只讀不寫所以不會漂；★★ 那個漏欄位的 bug 現在**擋得住整類**：`TestReplaceFighterEquipmentCarriesEveryEquipmentDerivedField` 拿同一個角色帶武器與不帶武器各投影一次，有差的欄位就是裝備衍生的——名單由行為算出來、不必手寫，突變驗過拿掉任何一行搬運都會紅）
- [1174 換武器之後傷害怎麼算：**槽 0 的武器無條件蓋掉記錄的骰子**](./1174-derived-values-weapon-overrides-record-dice.md)（`READY`：關掉 spec 1120 留的那個問題——派生值重算 `overlay-24:0C28h` 先把記錄的基準骰整組抄進現值，再叫 `0025h` 做武器投影：**`武器 := 角色^[151h]`（槽 0），為 NIL 就整支返回**，否則 `角色^[19Eh]／[1A0h]／[1A2h] := 類別表的 +09h／+0Ah／+0Bh`（小型傷害三連）——**沒有任何條件**，不看記錄的骰是不是 0；⇒ spec 1120 量到「兩者都非 0 但不一樣」那 13 隻不是矛盾，`BUGBEAR` 的 `2d4` 是**放下武器時**的天生攻擊；★ 大／小體型在**戰鬥當下**才切（`overlay-13:1608h` 目標體型 `and 7 > 1` 時改用類別表 `+02h`..`+04h` 並把加值換掉）——兩處都要有，只做一邊會讓所有攻擊都用小型骰；★ remake 這一輪把 `FighterWithEquipment` 從「第一件有傷害骰的裝備中物品」改成**槽 0**（舊規則會被排在鏈前面的弓或彈藥搶走），⚠ 沒有槽 0 武器時沿用舊規則——原作是什麼都不做，但 remake 還沒有徒手模型，照抄會讓只帶弓的角色沒有攻擊，寫成可以拿掉的條件；不宣稱類別表 `+0Eh` 兩個位元的語意、槽 9 雙持怎麼選第二把，大／小切換**還沒接進 remake**）
- [1173 `DS:7603h` 是**下一批怪物的圖示槽**，`CLEARMONSTERS` 設成 8 而不是清成 0](./1173-icon-slot-counter.md)（`READY`：關掉 spec 1145／1146 留的同一個未定項——`+143h` 在 PC-98 符號表裡叫 `ICONINDEX`，`overlay-33 entry#5` 拿它與字串 `CPIC` 組戰鬥圖檔名；★ `inc` 在放置迴圈**結束之後**才做一次 ⇒ **同一批 `SETUP MONSTER` 的怪物共用一個圖示槽**，下一批才換；起始值 8 是因為 **0..7 是隊伍成員的圖示**，所以重設值是 8 不是 0；⇒ 它是**快取索引不是遊戲狀態**——remake 的每個戰鬥員自己帶 `SpriteBlock`，可觀察行為相同，`1Ch` 那一步在 remake 沒有對應動作也不該有，`1Ch CLEARMONSTERS` 因此轉 `done`）
- [1172 `2E10h` 的髒旗標建進 VM：重畫不再靠「回頭掃寫入」猜](./1172-view-mirror-dirty-flags.md)（`READY`：spec 1150 讀完的模型終於接上——`STOREVALUE` 寫 `C04B`／`C04C`／`C04D` 時**當場**鏡射到 `720Fh`／`7210h`／`7211h` 並立 `8B68h`，`C059`／`C05F` 立 `8B67h`、`4BFD`／`4BFE` 立 `8B6Ah`、`0Eh PICTURE` 開圖立 `8B62h`（`0FFh` 關圖清 `8B62h`／`8B65h`）、`31h SPRITE OFF` 清 `8B65h`，`2Dh CALL 2E10h` 取快照後把五個旗標清掉；先前那個「回頭掃同一 block、執行序在前的 `SaveWrites`」啟發式整支拿掉；★★ **`CALL C01Eh` 要在 VM 裡走一步**——原作的 `MoveForward` 當場改地圖暫存器，remake 把 `CALL` 的副作用留到執行結束後才套用，鏡射不跟著走的話中間那些 `2E10h` 的快照會停在走之前，盜賊公會的走位動畫會少掉第一步（停在 `(8,3)` 而不是 `(9,3)`）；⚠ **兩個 remake 這一側的補丁**（原作沒有，都寫成可以拿掉的條件）：鏡射多記「三格是哪個 block 的腳本寫的」並要求與目前 block 相同（少了它火刀據點入口會落在 `(6,0)` 而不是 `(6,1)`——原作的進場放置是引擎做的、會覆蓋三格，remake 那一半是 game pack 宣告的 spawn），以及 `Written` 遮罩每次頂層執行重新起算；⚠ 第 660 輪把五條漏掉的同步點補上（轉向、原版存檔匯入、remake 讀檔、`finishNewGameEntry`、`syncDungeonStateFromECLRegisters`）並移除 `GO WITH GUARDS` 那個硬寫死的座標補丁；★★ 第 661 輪把**投影的時機**對上原作——`RunResult` 多一格 `FinalView`（頂層執行結束當下的鏡射），執行結束時座標還髒著就收尾投影一次，於是「寫了座標卻沒有 `2E10h`」的執行也會搬隊伍，`Written` 遮罩**整格移除**（欄位、三個位元常數、每次執行開頭的歸零全刪）；⇒ 修好一個玩家看得見的 bug：`ECL3/0x10:0C7Eh`「指揮官帶你走側門」連寫 `C04B=2`／`C04C=5`／`C04D=1` 之後直接 `GOTO` 回主分派器（隔壁那條搶劫結局 `0D24h` 才有重畫），remake 先前**完全沒搬**，玩家讀完那句話還站在辦公室裡、得自己撞那扇鎖住的側門；⚠ `Block` 那一格仍在，而它的拿掉條件也改寫了——`applyDeclaredDungeonSpawn` 一直都有回寫暫存器，卡的是**順序**（進場放置排在投影之前，而投影拿的是執行期間的快照），拿掉它下水道入口會落在 `(0,0)`、火刀據點落在 `(6,0)`；★ 回歸擋板跑的是**真的 bytecode**（寫座標 → `CALL` → 再寫一輪 → 再 `CALL`），手寫 `RunResult` 會把「快照在什麼時候取」這個答案直接寫進輸入裡；不宣稱 `8B65h` 的生產者（`ECL2` entry#8）——它只影響要不要重畫，而 remake 收到 `2E10h` 一律重畫）
- [1171 卷軸的 `USE`：判別看**裝備槽**、名字編號就是剩餘支數](./1171-scroll-use-slot-predicate-and-consumption.md)（`READY`：★★ 原作判「是不是卷軸」看的是**類別表的第一格（裝備槽）**落在 `0Bh`..`0Dh`，**不是物品類別**——`3Ch` 的名字叫 `Scroll` 但槽是 `0Ah`，原作把它當充能物品（效果 `5Fh`／`60h`），所以正確的數字是**卷軸 12／充能 18**不是 15／15，spec 1169 說「`5Fh`／`60h` 沒有持有者」也因此有答案；★★ **`+35h` 一個位元組管兩件事**：它是隱藏名稱旗標，非 0 時連卷軸上有哪些法術都列不出來，而清掉它同時把名字從 `Magic User Scroll` 變成 `... With 3 Spells`——辨識與命名是同一個欄位（第 2／5 章的卷軸 `+35h = 06`、第 3／4 章 `= 00`）；清除條件是法術辨識（效果 `10h`）或牧師讀槽 `0Ch` 的卷軸；★★ **名字編號就是剩餘支數**：`+30h` 六章一律 `0D4h`，每唸一支減一，`< 0D2h` 才丟掉 ⇒ **唸完第三支才消失**，`0D2h` 就是 `0D4h − 2`；★★★ 失敗判定正是 AD&D 1e：牧師槽或法師槽有等級 → 一定成功、**盜賊 10 級以上 → 1d100 ≤ 75**、其餘一定失敗只印 `oops!`（失敗不消耗卷軸）；★ 選單本體 `overlay-19:3474h` 是通用法術挑選器，七個標題字串（`in Memory`／`in Grimoire`／`on Scroll`…）把用途講死了，USE 傳 2；remake 這一側唸卷軸與記憶施法**選目標的方式一樣**（宣告過的 73 支走既有那條）；⚠ 條件 B 的 `+111h > +0E6h` 沒讀出來，只實作「牧師槽有等級」那一半）
- [1170 充能物品的 `USE` 接進 remake：18 件全部活過來](./1170-charged-item-use-in-the-remake.md)（`READY`：戰鬥選單的 `使用` 先前按下去沒有動作，六章 18 件充能物品在戰鬥裡全是死的——18/18 全部走得通；★ 分類**用欄位不用類別**（`+3Dh > 0` 且 `+3Eh < 80h`），照類別分會漏掉粉塵與飛彈項鍊（類別都是 `46h`）；★ 等級一律 **6**（`DS:7563h` 非 0 時原作不查使用者），所以火球魔杖永遠 6d6；★ 目標形狀取自主表 `+6`、敵我借用 `+7`（1 自己／4 我方／0 敵方，九件的兩欄組合完全一致）——⚠ **借用是推論不是讀出來的**，所以沒見過的組合一律回錯誤；⚠ 圓心有兩種：飛彈項鍊用玩家瞄的點、比克魔杖的 handler 自己從使用者算；⚠ 扣除順序是「數量 > 1 就只扣數量」——**一疊的最後一個才消耗充能**，而 `結果 = 0`（速度藥水對已被緩速的目標）整段跳過、不扣充能；沒接的兩件事寫在正文：巨人力量到期換回去（`92h` 的到期處理沒讀，舊值已存進記錄）、選單裡的物品清單（目前 `U` 用、`Alt+U` 換））
- [1169 法術主表那 13 筆無名列不是占位，是**物品效果列**](./1169-item-effect-rows-in-the-spell-table.md)（`READY`：★★ 原作自己說了——火球的 handler 第二條就是 `cmp ds:6F97h, 40h`，為那一列分了 `(1d3×2＋1)d6` 的路，**如果玩家取不到，這段程式沒有存在的理由**；取得管道就是 USE，`40h` 的持有者是飛彈項鍊；九支 handler 逐條讀完並由四句訊息釘死語意（`is Speedy`／`is stronger`／`is paralyzed`／`is invisible`）——速度藥水（`27h` 加速，先查 `2Ah` 緩速有就不做事）、巨人力量（`entry#19` 把力量提到 **21**，時間 `(1d4＋4)×10` 掛成 `92h` 記錄並存下舊值）、麻痺魔杖（`34h`）、粉塵（`47h`，4 個目標）、飛彈項鍊、魔法飛彈魔杖（`2d4＋2`）、隱形藥水（`19h`）、比克魔杖、治療藥水（`2d4＋2`）；★ **比克魔杖只打 `RACETYPE = 12h`**——資料裡那四種全是 Moander 的植物眷屬（`BIT O' MOANDER`／兩種 vegepygmy／`SHAMBLING MOUND`），非植物一律當豁免成功而它的 `+8` 是 1 ⇒ 傷害 0；順帶讀完 `overlay-23 entry#20` 的四個參數（目標／傷害／豁免種類／豁免成功），`= 3` 沒有分支不要補；⚠⚠ **這個缺口是自己的過濾器造成的**：`spell_damage_table.py` 的 `or spell["placeholder"]` 讓四列從來沒被讀過，於是「沒有骰子」看起來像資料裡沒有——**排除條件也是斷言，要跟結論一樣需要證據**；推翻 spec 815 的「占位／玩家取不到」）
- [1168 戰鬥的 `USE` 分成哪幾類：253 件物品逐件盤點](./1168-combat-use-item-gap.md)（`READY`：這一篇把「USE 按下去會走哪一條」變成數字：原版六章 **253 件物品** ＝ 卷軸 15／充能 15／其他 223，充能那 15 件是治療藥水、極效治療、速度、隱形、魔法飛彈魔杖、粉塵、項鍊——**沒有 USE 這些在戰鬥裡全是死的**（充能那 15 件已於 spec 1170 接完）；★★ 充能物品要的東西**全在原版資料裡**：效果編號就是法術主表的列，目標模式（`+6`）、豁免（`+8`）、效果碼（`+0Ah`）都在表上，骰子在各自的 handler（spec 1169）；⚠ 卷軸的判準（類別 `3Ch`..`3Eh`）是從資料推的，`cmd/item-use-audit` 逐件檢查「名字含 Scroll」⟺「類別在這三個裡」，253 件全部成立、不成立就整支失敗；★ 選單逐項串接（照 spec 905 的形狀），`使用` 身上沒東西就不顯示；`移動`／`施法` 兩項還沒照條件收放，理由（`MOVELEFT` 沒有模型、`CANCAST` 與全域旗標沒有對應狀態）寫在正文）
...
- [第一千二百四十六輪原版外層角色檔與隊伍組裝流程](./1246-original-outer-party-assembly.md)（`READY`：本作新建角色先獨立保存，再由 Curse 清單加入隊伍；Pool／Hillsfar 舊格式匯入 fail closed）
