...
- [1168 戰鬥的 `USE` 還缺什麼：15 件充能物品、9 個沒宣告的效果](./1168-combat-use-item-gap.md)（`READY`：戰鬥選單有 `使用` 但 remake 沒有 USE 的動作本身——這一篇把「還缺多少」變成數字：原版六章 **253 件物品** ＝ 卷軸 15／充能 15／其他 223，充能那 15 件是治療藥水、極效治療、速度、隱形、魔法飛彈魔杖、粉塵、項鍊——**沒有 USE 這些在戰鬥裡全是死的**；★★ 擋住的是**資料不是程式**：充能物品的效果編號就是法術主表的列，而 remake 已經有資料驅動的效果施法路，差的是**目標模式**（那一格在 game pack 而不在原版表裡），15 件用到的效果有 **9 個還沒宣告**；⚠ 卷軸的判準（類別 `3Ch`..`3Eh`）是從資料推的，`cmd/item-use-audit` 逐件檢查「名字含 Scroll」⟺「類別在這三個裡」，253 件全部成立、不成立就整支失敗；★ 本輪把選單改成逐項串接（照 spec 905 的形狀）並讓 `使用` 誠實——身上沒東西就不顯示；`移動`／`施法` 兩項還沒照條件收放，理由（`MOVELEFT` 沒有模型、`CANCAST` 與全域旗標沒有對應狀態）寫在正文）
