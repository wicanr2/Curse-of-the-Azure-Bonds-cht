...
- [1170 充能物品的 `USE` 接進 remake：15 件全部活過來](./1170-charged-item-use-in-the-remake.md)（`READY`：戰鬥選單的 `使用` 先前按下去沒有動作，六章 15 件充能物品在戰鬥裡全是死的——本輪 15/15 全部走得通；★ 分類**用欄位不用類別**（`+3Dh > 0` 且 `+3Eh < 80h`），照類別分會漏掉粉塵與飛彈項鍊（類別都是 `46h`）；★ 等級一律 **6**（`DS:7563h` 非 0 時原作不查使用者），所以火球魔杖永遠 6d6；★ 目標形狀取自主表 `+6`、敵我借用 `+7`（1 自己／4 我方／0 敵方，九件的兩欄組合完全一致）——⚠ **借用是推論不是讀出來的**，所以沒見過的組合一律回錯誤；⚠ 圓心有兩種：飛彈項鍊用玩家瞄的點、比克魔杖的 handler 自己從使用者算；⚠ 扣除順序是「數量 > 1 就只扣數量」——**一疊的最後一個才消耗充能**，而 `結果 = 0`（速度藥水對已被緩速的目標）整段跳過、不扣充能；沒接的三件事寫在正文：卷軸（`sub_3474h` 沒讀，按下去回錯誤但**不消耗**）、巨人力量到期換回去（`92h` 的到期處理沒讀，舊值已存進記錄）、選單裡的物品清單（目前 `U` 用、`Alt+U` 換））
