...
- [1157 那 23 處逐處分類：全部是真實移動；以及兩處被 IDA 切錯的共用 handler](./1157-restore-previous-cell-sites-and-shared-handlers.md)（`READY`：spec 1155 那 23 處只有兩種——8 處明確移動、15 處 `SAVE 4BF0 C04B; SAVE 4BF1 C04C`「退回上一格」，**兩類都是真實的隊伍移動**，所以 `C04D` 條件擋掉的是玩家看得見的位移；★ 15 處的守衛**各不相同**，只有一處用 `7EC7h`——「等 `7EC7h` 有 producer」不是共同前提；★★ `ECL2/0x04:160Ch` 是從**六個**地方跳進去的共用退格出口（`02D0h`／`0ABFh`／`0AD9h`／`0CFCh`／`0E6Eh`／`15FDh`），先前寫「只有戰後走得到」是只看了一個入口；★★★ 實跑探針指出火刃巢穴三條路線測試會紅是因為**那些路線是照著被 `C04D` 壓掉之後的行為寫的**——原作在那些格子會把隊伍推回去，要換忠實模型得連同重新推導路線一起做；★★ `2Fh`／`30h` 也是共用一支（`0DD8h` 重讀 `DS:75FFh`），真正範圍是 `0DA4h`..`0E12h`（43 條，IDA 拆成 11＋32），audit 表原本連 `2Fh` 那一列都沒有；★★ `AND`／`OR` 之後的六個比較格子是 `compare_variables(0, 結果)`——**`0` 是左運算元**（`0DF3h` 先 push 0），remake 程式碼本來就對、錯的是註解，⚠ 照那行錯註解「修正」會壞掉而且**測不出來**（全 corpus 174 處 `AND`／`OR` 沒有一處接排序型 `IF`））
