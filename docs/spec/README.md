...
- [1178 物品名稱是三個編號組出來的：`DS:1040h` 那張 255 筆的詞表](./1178-item-name-word-table.md)（`READY`：第 N 筆在 `DS:1040h + N×15h`，Pascal `string[20]`，編號 `01h`..`FFh` 共 255 筆——指令在 `overlay-24:063Bh`（`mul 15h` ＋ `add 1040h`，基底是 0 那一格所以編號直接當索引）；資料側正對照：253 件物品照這規則組回去 **208 件與紀錄裡存的名字逐字元相同**，基底位移一格或兩格都是 **0 件**；★ **名字完全不含物品類別**，remake 先前拿 `item_type_XX` 當基底再貼成分，靠一條「和基底相同就不加」的守衛才沒穿幫；★★ **`+32h`（Plus）與 `+36h`（Cursed）不進名字**——加值走名稱編號 `A2h`..`A6h` 那幾個**可以被藏起來**的成分，原版 72 件帶 Plus 的物品有 54 件從頭到尾不顯示加值；⚠ `+35h` 不是布林是三個 bit，`+39h` 數量連 `1` 都印（`1 Oil`）；⚠ 紀錄的 `+00h..+29h` 是**未鑑定名的快取**（`Deck`／`Beaker`），組名時會被覆寫，不是權威資料；語系檔的 `item_name_XX` 由 54 筆補成 252 筆，並改正第 260 輪照 reference 填錯的編號（`9Fh` 之後整段偏 3 ⇒ 加值顯示錯數字）；翻譯層加一條「三成分而中間是連接詞就前後對調」讓 `Wand of Fireballs` 讀成「火球之魔杖」；不宣稱 `*` 標記那個 `DS:650Ah` 鏈在問什麼、未鑑定替身名的來源、PC-98 的詞表位址）
