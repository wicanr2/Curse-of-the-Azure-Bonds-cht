...
- 未施法時原本的 enemy target cursor 保持不變；移動模式不會誤觸攻擊／施法。

## 明確 boundary

本輪尚未猜測地形／戰場邊界、負重／護甲 movement allowance、進入敵方格的攻擊、離開鄰接的 free rear attack、facing、離場 flee 與完整 movement animation；這些需 CombatMap／equipment／rules adapter 證據後接入。
