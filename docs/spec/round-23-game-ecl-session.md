...
`BlockSession` 保存 current block 與 selection offset，並在 `NEWECL` signal 時切換；目前 memory、call stack、party/map state 尚未完整跨 block 持久化，因此仍是 bounded integration，不是完整 VM。
