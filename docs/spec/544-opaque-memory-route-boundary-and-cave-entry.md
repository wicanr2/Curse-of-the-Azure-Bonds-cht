...
- `TestRealNewGameContinuesFromHapToBeholderCaveEntrance` 已由同一新遊戲
  session 正常抵達 Dexam 洞穴入口 `(4,5,N)`。測試在此停下，不以猜測的 GEO
  邊連線取代洞穴內的傳送／隨機遭遇／出口。
...
| Dexam 洞穴入口 `(4,5,N)` | `exact`（remake path contract）／`strong inference`（原版座標對照） | `zhentil-keep.beholder-cave` 的 JSON spawn、ECL block 0x22／GEO4 block 0x25 handoff、同一 session 測試 | 只能宣稱已抵達入口，不能宣稱洞內全路徑 |
...
暗神殿 → Dimswart → hooded woman → ECL block 0x22／GEO4 block 0x25 → (4,5,N)`。
