...
> ★★★ **`.sav` 檔裡放的是現行長度的角色記錄**（本支寫 `1A6h` ＝ 422，
> 進門先 `DS:7584h := 0`）。spec 1076 在 DOS 的存檔載入端、
> spec 1072／1073 在 PC-98 兩側，都看到同一件事。
> ⇒ **決定格式的是 `DS:7584h`，不是副檔名。**
> spec 1043 那張「副檔名 → 記錄長度」的表只適用於
> `DS:0A816h` 那條「匯入舊角色」的掃描路徑，已在該 spec 標註。
>
> ⚠ 名字為空 → `.guy`、有名字 → `.sav` 這個分岔的判準本身
> （為什麼隊伍成員用 `.sav`、獨立角色用 `.guy`），本規格不宣稱。
...
- 沒有宣稱名字為空時用 `.guy`、有名字時用 `.sav` 的判準是什麼。
