...
同一套認法用在 `MSCPLAY` 上得到相反的答案。PC-98 的 `MSCPLAY`（`0893:0114h`）
有 7 處呼叫點、跨 5 個 overlay：

```
overlay-01×1、overlay-05×1、overlay-10×2、overlay-17×2、overlay-18×1
```

拿這組指紋去比 DOS：最接近的候選只對上 `overlay-17×2`（差 5），而且**有一整排
並列**——沒有任何一個突出來。對照 `SOUNDFX` 的 1 比 29，這是「找不到」而不是
「找到一個比較像的」。

三件事互相印證：

1. DOS 的音效段 `0713h` **只有兩個入口**（`SOUNDFX`、`INITSOUND`），
   PC-98 的 `SOUNDX` 有五個（多 `MSCPLAY`、`MSCSTOP`、`BGMPLAY`）。
2. DOS 遊戲 image 裡**沒有任何音樂資料檔**——全部是 `.DAX`、`.EXE`、
   `.OVR`、`.BAT`。
3. game pack 早就把那 12 首標成 `source_platform: pc-9801`。

⇒ **DOS 版沒有背景音樂**。「DOS 側的音樂那一半」這個待辦的答案是「沒有那一半」，
不是「還沒做」。
