...
> ★ `DS:4FC7h` 是**離開旗標**：spec 1045 的地城主迴圈 `until DS:4FC7h <> 0`、
> spec 1095 的跨迴圈全域停止、spec 1152 的 `2Eh DAMAGE` 在全隊倒下時把它設 1。
> 所以 `PROGRAM 3` ＝ 宣告隊伍全滅、停掉迴圈。
> ⚠ 訓練所的三個「這次不收費」旗標是 `DS:7587h`／**`DS:4FC8h`**／`DS:8B6Fh`
> （spec 1084）——`4FC8h` 不是 `4FC7h`。
