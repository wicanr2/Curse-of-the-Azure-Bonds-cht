...
- ~~`DS:BDF4h`／`BDF5h` 如何決定樣式參數 `var_3C`~~ —— **spec 1039 由 DOS 對側關掉**：
  `var_3C := ord((旗標1 <> 0) and (旗標2 <> 0))`，兩個都非零才是 1。
- 收尾的 `0418:14C7(18h, 27h, 18h, 0)` 是清除哪一塊（DOS 對側 `0542:0B4Ah` 四個參數相同）。

> ★★★ **spec 1039（DOS 對側）**：DOS **沒有**熱鍵表，改成把選項行
> 逐個串成 `'~' ＋ 文字 ＋ ' '`（最後一個不接空白，整行上限 50 bytes）——
> **波浪號就是熱鍵標記**，`~` 後面第一個字元即熱鍵。
> PC-98 這裡的 `i ＋ 40h` 熱鍵表正是它的替代品。
> ⚠ 中文化要跟 PC-98，`~` 後面接 Big5 首位元組會把熱鍵判成亂碼。
