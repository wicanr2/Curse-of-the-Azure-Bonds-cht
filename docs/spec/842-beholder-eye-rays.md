...
- ~~沒有宣稱豁免類別碼 `0`／`1`／`3` 各對應 AD&D 的哪一欄。~~ → spec 988：
  加上恐懼用的 `4` 之後，四格中三格與 AD&D 1e 的五欄順序（麻痺/毒/死亡、
  石化/變形、法杖、吐息、法術）對得上，**順序已確定**。但引擎有兩處自己的
  歸類：麻痺凝視用 `1`（AD&D 屬第一欄）、分解光線用 `3`（AD&D 屬法術欄）。
