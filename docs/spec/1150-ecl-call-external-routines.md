...
當下回頭掃同 block 的 `SaveWrites`、而且要有新寫的 `C04D`」的啟發式（spec 276／
402／403 的正常玩家路徑回歸鎖著）。原作的模型是 `STOREVALUE` 當場寫、`CALL`
只看髒旗標——兩者在「對話把 `C04B`／`C04C` 當暫存」時給出不同結果，要換成原作
模型必須先解釋那些回歸為什麼過得去，不能只因為讀完了就改。
