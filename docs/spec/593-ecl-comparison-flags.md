...
`0062:008E`（PC-98 `overlay-07:1928h`）的結構與數值比較**完全對稱**：先把兩個
字串各複製一份到區域緩衝區，`FillChar(DS:A88Ah, 6, 0)`，然後**呼叫六次**
RTL 的字串比較（`0A65:0734`），用 `jnz`／`jz`／`jnb`／`jbe`／`ja`／`jb` 這組
完全相同的判斷分別設定六個旗標。

所以 `16h`～`1Bh` 對字串與對數值的語意一致，`18h` 在字串上就是「字典序小於」。

原版把字串比較跑了六次（而不是比一次再分派），與 `06h` 把 `div` 執行兩次
（[spec 592](592-ecl-arithmetic-and-compare.md)）是同一種風格。remake 只比
一次再分派，結果相同——這裡沒有副作用。

## 明確不宣稱

- RTL `0A65:0734` 的比較規則（區分大小寫？長度不同時怎麼排？）。
