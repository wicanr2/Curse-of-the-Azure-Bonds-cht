...
⚠ **「沒有壓縮」只在 PC-98 成立。** DOS 的對應 `overlay-07:012AEh`（167 條，
本輪讀的 PC-98 版只有 57 條）做的是 **6-bit 解包**，每 3 個位元組出 4 個字元，
字元集就是字型的 64 格。PC-98 為了裝得下 Shift-JIS 才把整個壓縮層拿掉。
詳見 spec 992——**中文化必須跟 PC-98 的做法**。
