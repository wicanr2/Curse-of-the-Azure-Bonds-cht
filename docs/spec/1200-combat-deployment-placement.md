...
- **地城牆面調整**（只在隊 0、朝向為偶數、組 0、第一次失敗時做一次）：以隊伍
  的地城座標 `A2A9h`／`A2AAh` 為基準，用 `6CEh` 表（4 組 × 4 個方向碼，8＝停）
  逐方向叫 `378h`（牆面從兩側各查一次）；牆擋住就把 `teamX`／`teamY` 往
  `489Eh`／`48A7h` 的方向位移，重新開掃。`7F27h = 3`（非地城模式）跳過牆檢。
