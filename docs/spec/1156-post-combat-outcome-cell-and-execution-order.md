...
⚠ **2026-08-27 勘誤／補充（spec 1229）**：這句只證明 `81h` 落在「未勝」
集合，不能反推所有敗戰都是 `81h`。`ECL4/0x20` 的兩場守衛戰只有
`7EC7h == 80h` 才走敗戰押送法庭，而正常逃離戰場得到 `81h`。remake 因此採
勝利 `0`、敵方獲勝 `80h`、隊伍逃跑 `81h`；全滅同為 `80h` 但直接終止。
...
- 本輪原本沒有宣稱 remake 該怎麼產生 `7EC7h`；後續已由 spec 1229 補證並接線。
