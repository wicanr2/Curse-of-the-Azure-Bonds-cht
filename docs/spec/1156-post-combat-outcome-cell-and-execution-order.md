...
`80h` 的一個生產者已定位：POSTCOM 的**全滅路**在印全滅訊息前寫
`+58Eh := 80h`（PC-98 `overlay-05:18F8h..1903h`，判準與現場見 spec 1204）；
POSTCOM 進場時先清 0。⚠ 仍不宣稱 `81h` 具體是逃跑還是全滅，也不宣稱
`80h` 沒有其他寫入者——三處呼叫端只分「> 80h」與「<= 80h」兩邊。
