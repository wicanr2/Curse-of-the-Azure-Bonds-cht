...
通過那條結局（`0C7Eh`）連寫 `C04B=2`／`C04C=5`／`C04D=1` 之後直接 `GOTO`
回主分派器，**沒有** `CALL 2E10h`；隔壁那條搶劫結局（`0D24h`）寫完同樣三格
才有重畫。原作的 `STOREVALUE` 一寫就當場改 `720Fh`，所以兩條都會搬隊伍，
差別只在有沒有立刻重畫畫面。

remake 先前把投影掛在 `2E10h` 上，於是**這一條完全不搬**：玩家讀完「指揮官
帶你走側門」之後還站在辦公室裡，得自己去撞那扇鎖住的側門。第 661 輪補上
收尾投影之後才對上（spec 1172）。
