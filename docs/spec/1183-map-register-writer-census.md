...
逐張地圖的盤點見 spec 1184 與
[`docs/audit/map-spawn-sources.md`](../audit/map-spawn-sources.md)：18 個
`first_person` 定義裡 11 個完全靠腳本、4 個宣告了 `spawn` 而且與腳本寫的座標
**對不上**、3 個沒有 script block（只當同 area 的退回值）。**沒有任何一個宣告值
等於腳本寫過的座標。**
...
- 沒有宣稱 4 個對不上的宣告值與腳本座標誰對——那要跑原版（見 spec 1184）。
