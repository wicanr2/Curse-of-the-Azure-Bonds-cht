...
確定的**；少掉的那一格是 `MONSTERTYPE`，由原版怪物資料定案（spec 1166）。

## 三處與既有規格的讀法不一致

新名字沒有覆蓋既有的 `decoded` 欄位，只並列。三處曾經需要另外對，現在只剩一處：

| 位移 | 既有讀法 | PC-98 符號 |
|---|---|---|
| `+0E6h` | 多職角色的現行等級（spec 185）| `HIGHESTPREVLEVEL`（前一個最高等級）|
| `+11Ch` | **已對完**：`BASEATTBLOWS[0]`（半次單位的基準攻擊次數），spec 1010 那句 `= 0 就用槽 2` 是它的一個用法而不是另一種讀法（spec 1180）| `BASEATTBLOWS[0]` |
| `+192h` | ECL 旗標，投影位址 `7CE4h`（spec 1098）| `DUM1`（保留欄）|

`+192h` 兩者相容——ECL 投影到一個 Pascal 端的保留欄沒有矛盾。`+11Ch` 已由
spec 1180 對完。**只剩 `+0E6h`** 要再讀一次 DOS 側的用法。
