...
**spec 731 讀完 `sub_F06h` 之後可以判斷了**：那句訊息只有在
`DS:37DAh` 表格該法術項目的 `+0Ah`（要套用的效果編號）大於 0 時才會被傳給
`PUTEFFECT`，而 `4DA0h` 傳進去的傷害量是 0。所以只要那一欄是 0，這句 `is clumsy`
根本不會顯示——「忘了改」不會有可見後果。要蓋棺論定得查表格內容，
而表在 resident 資料段。
