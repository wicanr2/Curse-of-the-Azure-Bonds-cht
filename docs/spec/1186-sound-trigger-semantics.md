...
## 戰鬥音樂：`MUSICNO` 之外的第二種換曲形狀

`MSCPLAY` 的引數就是曲號。多數換曲點的形狀是「先寫 `MUSICNO`，再把它推進去」，
但 `INITCOMBAT`（`COMPREP` ＝ overlay-10）**完全不碰 `MUSICNO`**：

```
mov byte [7F27h], 5
cmp byte [LOADMONNUM], 47h     ; 8BE2h
jnz  →  mov al, 07h            ; 曲目 7 ＝ 戰鬥
        mov al, 0Bh            ; 曲目 11 ＝ 地城二
push ax
call MSCPLAY
```

⇒ **戰鬥開始換哪一首，取決於載入了哪一組怪物。**

★ 這一段解掉了一個掛很久的空格：`docs/audit/pc98-music-triggers.md` 一直寫著
「兩首（戰鬥、地城二）沒有立即數換曲點選到，**不能讀成「用不到」**」。
那句警告是對的，但只能停在那裡——因為掃描器只認 `mov byte [MUSICNO], imm`
一種形狀。**掃描面窄一格，兩首曲子就整個消失，而報表看起來完全正常。**
補上第二種形狀之後，宣告的 12 首**全部**有落點。
...
⚠ **2026-08-30 勘誤（spec 1249）**：這裡漏讀了 spec 1000 已有的 exact producer。
`overlay-24:00C28h` 直接以物品類別 `49h`（Arrow）／`1Ch`（Quarrel）建立
`+17Dh`／`+181h`，不需要也不應由槽 10／11／12 反推；正式投影已改正。
