...
`TestRealNewGameContinuesFromHapToMythDrannor`：由第 542 輪艾森布拉城外沿正常
...
`TestRealNewGameContinuesFromHapToMythDrannor`，只驗證到 Dexam 洞穴入口
...
宣告的已驗證位置交易。`TestRealNewGameContinuesFromHapToMythDrannor`
...
`TestRealNewGameContinuesFromHapToMythDrannor` 在 Docker 通過。第 547
...
2026-08-14 第五百六十／五百六十一輪解出 `INTERPET` 的兩張分派表。ECL opcode
dispatcher（DOS `overlay-02:3377h` 讀 `ds:75FFh`、PC-98 `373Eh` 讀 `ds:0A891h`）
在兩平台各有 52 個 handler，涵蓋 `00h..40h` 中除 `1Fh` 外的 64 個 opcode，
與 `internal/ecl/operand.go` 的 `KnownCommands` 範圍一致；`1Fh` 也從未出現在
ECL corpus。ECL `CALL`（opcode `2Dh`）handler 認得 7 個 external routine，
selector 與 operand 的關係是 `operand = (selector + 7FFFh) mod 10000h`，該 bias
由 `sub ax, 7FFFh` 直接讀出，並獨立驗證第 519 輪的 `2E10h → AE11h`；CoAB
corpus 只靜態使用其中 `2E10h`（12 次）與 `6803h`（11 次），合計 23 個 CALL。
方法是對候選值逐一符號執行比較鏈，分支目標取 IDA code ref。
過程中修掉兩個會產生假表的工具 bug：正規表示式吃掉立即數的 `h` 後綴（使
`cmp al, 10h` 被當成十進位），以及用「selector 最多的分支＝default」猜預設值
（少報了 selector `0001h`）。第一個 bug 曾寫進 spec 560 第一版並推送，已在該
規格的勘誤節記錄。台帳現為 2,825 個函式中 4 個已解讀。權威規格為
`docs/spec/560-ecl-opcode-dispatch-table.md` 與
`docs/spec/561-ecl-external-call-registry.md`。

## 2026-08-15：建角 HP 與選單資格；spec 1101 初版整份作廢

`overlay-17:00782h`（全台帳最大的函式）先前只讀到屬性夾值就停了。這一輪把
`21C1h`..`2290h` 的收尾讀完，補上 spec 1101（建角 HP）與 spec 1102（三張選單
各自能選什麼）。

**已被推翻的斷言**：

- spec 1101 初版（`1101-hit-point-cap-and-constitution-bonus.md`，已刪）把
  `overlay-17:046ECh` 記成「HP 上限」，並宣告 `+109h + i` 的語意有衝突。
  兩點都錯：`046ECh` 是**有效等級**，spec 850 早已逐條讀完並記在函式台帳裡；
  `+109h + i` 是**職業槽等級**，spec 1034／850／869／857／1055 各獨立確認過一次。
  該版的 `045E9h` 內容也只是 spec 869 的較不完整重述（漏了戰士系
  `+75h in [2,3,4]` 的完整加值表與遊俠一級乘 2）。**整份重寫成
  `1101-creation-hit-points.md`。**
- spec 1093 §七 把種族天賦表寫成 0＝人類起算。實際編號是 1..7（人類是 7），
  表裡的 `0（人類）` 對應的是 `case` 的 `else` 分支——人類與半獸人共用，
  兩者都不掛任何效果。已改寫，並補上 `+144h` 是體型（1 小／2 中）。
- spec 1093 的「四張清單的選項文字不在這一支裡」不成立：四組字串都是本支
  自己從資料段取的（spec 1100 已解出內容）。已改寫。

**新結論**：建角種族選單只有六項（半獸人的顯示分支不存在，`+74h` 永遠不會是
6）；陣營選單依職業組合查 `DS:41D8h` 過濾，聖騎士只有守序善、遊俠只有三個善、
盜賊系少掉兩個善；`DS:45B1h`／`45B2h` 是 AD&D 起始金錢表，八格全中，但擲出來
的結果整支函式再也沒被讀過，改寫死 `+103h := 12Ch`。

Remake 端跟著補齊：`gamepack/rules/character-tables.json` 多了生命骰、體質加值、
可選陣營、`selectable`／`size`；`party.RollCreationHitPoints` 是機制，資料全在
JSON；建角流程加了 `AcceptGuidedAbilities` 收尾（年齡 ＋ HP ＋ 基準值同步）。

## 2026-08-16：建角四段流程收尾，兩個順序先前擺錯

讀完 `overlay-17:00782h` 剩下的尾巴（`15CAh` 迴圈頂點到 `2514h` 的 `retf`），
把名字輸入與 `Save <名字>? ` 接進 remake，角色也真的進得了隊伍名冊。

**已被推翻的斷言**（都是前一輪自己寫的）：

- HP 被擺在「屬性接受之後」算一次。實際上 `21C1h` 在 `15CAh` 的**重擲迴圈裡**
  ——屬性重擲一次，體質變了，HP 就跟著重算。已移進 `RollGuidedAbilities`。
- 基準值複製被擺在屬性接受那一步。實際位置是 `244Bh`，**在名字輸入之後**、
  `Save` 之前。重擲迴圈每輪都把目前值清成 0，基準值那一組全程不動，
  直到名字收下才寫入一次。已移進 `CommitGuidedName`。

**新讀到的**：`Save <名字>? ` 只比對 `'N'`（`24BBh`），答 `N` 就 `FreeMem` 離開、
角色不留，其餘任何鍵都存檔，兩條路都直接結束函式；名字複製長度是 `0Fh`
＝ 15 字元（remake 維持自己的 20）；空名字會直接重問（`2441h`）。
屬性擲點在 `1625h`，是 `ROLLDICE(3, 6)` 後接 `inc ax` 再包一層迴圈——
`ROLLDICE` 的回傳基準沒確認，**公式仍不宣稱**；同一段的種族調整已可看出
矮人體質 ＋1／魅力 −1、精靈敏捷 ＋1／體質 −1，但七個種族沒有逐條讀完。

畫面用 `-guided-creation-step=name|save` 擷取確認（`docs/screenshots/`）。
擷取用的名字改成旗標傳入，避免在 `cmd/` 留中文字面值（Han baseline）。

## 2026-08-16（續）：`ROLLDICE` 定案，擲點與種族調整補齊

使用者指示「擲點直接依 D&D 定調為 3d6，種族屬性差異也確認並記錄」。
查下去發現不必用推定——`ROLLDICE` 本體（`overlay-23` entry #9 → 局部 `137Fh`）
只有 20 條，逐條讀完就有答案：參數是（骰數, 面數），每顆 `Random(面數) + 1`，
所以 **`ROLLDICE(3, 6)` 就是標準 3d6**，與使用者的判斷一致。
建角在上面加了兩層：每次 **＋1**、每個屬性 **擲六次取最大**（`161Ch`..`16CBh`）。
寫成 spec 1103。

**已被推翻的斷言**：spec 1087 把 `ROLLDICE` 的參數順序寫反
（`ROLLDICE(6, 1)`／`ROLLDICE(100, 1)`）。實際位元組是先推 1 再推 6
（`overlay-02:01730h`）⇒ `ROLLDICE(1, 6)`。該規格的文字描述（「各擲一顆 d6」）
本來就是對的，只有引數轉寫錯；五處已改正。spec 571 的順序原本就正確。

**種族屬性調整**七個全部取出，逐格與 AD&D 一版相符：矮人體質 ＋1／魅力 −1、
精靈敏捷 ＋1／體質 −1、半身人力量 −1／敏捷 ＋1、半獸人力量 ＋1／體質 ＋1／
魅力 −2，地精／半精靈／人類走 `else` 無調整。半獸人有完整調整但建角選不到
（spec 1102），與屬性上下限、可選職業、起始年齡同一個現象。

remake：擲點參數與種族調整都進 `character-tables.json`，
`party.RollCreationAbilities` 只留機制；gamepack 測試把七列調整與 3d6+1／六次
釘死，避免重新產生 JSON 時無聲改掉。

## 2026-08-16（第 564 輪）：ECL 有序副作用改成逐 opcode 閉合

`RE-01` 的 29 個未審候選如果一個一個當獨立問題處理，會重複讀同一批 handler。
把它換個問法：33 個候選只由 **20 個不同的 opcode** 組成，而次序是 dispatcher 與
各 handler 的性質，不是某段直線區域的性質。所以本輪讀的是 handler 不是候選——
DOS 23 支 ＋ operand 解碼器 ＋ lifecycle 驅動器，寫成 spec 1104。

三條通則：

- **PC 一律在效果之前推進**（帶 operand 的走 `overlay-07 entry#2` 一次吃掉 opcode
  與全部 operand，不帶的用單一 `inc`）⇒ 停下再續跑不會重播同一條指令，
  這是原生的 exactly-once，不需要額外的「已處理」旗標。
- **畫面的提交點只有一個**：五個髒旗標的全部清除點都在 `2Dh CALL` 的 `2E10h`
  分支。`0Eh PICTURE` 只設旗標，玩家看到換圖是在後面那個 CALL。
- **`20h NEWECL` 是終止指令**：載入新 block、PC 重設為 `8000h`、設兩個停止旗標，
  驅動器據此從 lifecycle 頂端重跑。

**已被推翻的斷言**：

- 清冊把 `NEWECL` 當 fallthrough。修正 `endsStraightLine` 後候選 33→32，
  指令數 1,355 不變（corpus 只有兩個 `NEWECL`，後面的指令本來就另有入口）。
- round-38 規格說「`CLEARMONSTERS` 會清除目前 encounter prefix」，remake 據此在
  `1Ch` 連 `MonsterSetup` 一起清掉。原作 `120Eh` 只釋放怪物鏈與計數，
  `SETUP MONSTER` 寫的 `ds:7601h`／`7602h` 一格都沒碰。corpus 有**四個**站點
  （ECL3 `0x11`／`0x12`、ECL4 `0x21`、ECL5 `0x32`）先 SETUP 再 CLEAR 才開打，
  敵人因此拿不到圖。已修並以四站點測試釘住（拿掉修正會四站點全紅）。
- 我自己中途把 far call `006Bh:002Ah` 讀成 overlay 本地的 `0x6DA`——那是 IDA
  對未重定位 addend 的算法，落點其實在 resident。正確換算寫成
  `scripts/resolve_far_calls.py`，靠它才解出 `overlay-07 entry#1/#2` 是操作元
  解碼器、`CALL` 的七路 switch 與各 handler 的真實呼叫圖。

**順帶關掉 `RE-03` 的靜態層**：`2Dh` 把 operand 減 `7FFFh` 後走七路 switch，
23 個靜態可達 CALL 只用到 `2E10h`（12 次）與 `6803h`（11 次，逐格動畫），
另五路 corpus 從未使用，不在 switch 內的目標**靜默 no-op**。

**負面結果值得記著**：`resume_only` 這一類是空的。所有「等這一輪跑完才發生」的
效果都在 lifecycle 驅動器（`3691h`）與 block 轉換函式（`3772h`）裡，
不在任何 opcode handler 內——去 handler 裡找 resume 語意會白費。

產物：spec 1104、`docs/audit/ecl-opcode-effect-phases.{md,json}`（46 列，
已讀 25 支／未讀 21 支，fail-closed 覆蓋檢查）、review ledger 32 筆
（31 `covered` ＋ 1 `partial`）、`scripts/resolve_far_calls.py`、
`tools/ida/dump_functions_batch.py`。全套 `./tools/go.sh test ./...` 31 個套件全綠。
