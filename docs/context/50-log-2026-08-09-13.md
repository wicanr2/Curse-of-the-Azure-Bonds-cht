...
`TestRealNewGameRunsToTheEnding`：由第 542 輪艾森布拉城外沿正常
...
`TestRealNewGameRunsToTheEnding`，只驗證到 Dexam 洞穴入口
...
宣告的已驗證位置交易。`TestRealNewGameRunsToTheEnding`
...
`TestRealNewGameRunsToTheEnding` 在 Docker 通過。第 547
