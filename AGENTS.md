...
|---|---|---|---|
| 第一人稱全量比對（`tools/fp-oracle-compare.py` 全 1,474 張）| 3 小時以上 | **只重驗 42 張已知樣本**（約 10 分鐘）：13 張曾經有差的 ＋ 29 張曾經被修法弄壞的 | 換掉牆面算圖的**幾何**（`drawWallFar`／`drawWallMid`／視錐），或要對外宣稱新的 fidelity 數字時 |
| 按鍵重放的幀數上限掃描（1,200 → 40,000 逐級）| 每輪 10 分鐘 | 只在**改走法**時量一次，把表寫進 `keyDrivenDefaultFrames` 的註解 | 同上 |

⚠ **降級不是取消，它換掉的是「什麼時候跑」不是「跑不跑」。** 42 張樣本能擋住的
是**已知**的兩類回歸；它擋不住「改動弄壞了從來沒差過的畫面」。所以：

- 引用的全量數字要**標明是哪一次全量跑出來的**，不能拿樣本數字冒充全量。
  現在可引用的是第二次那一輪：1,474 張、1,461 張逐格相同、389 格不同。
- 樣本全綠**不構成**「全部正確」。實例：把「內側」取錯邊的那一版，13 張樣本
  全部 0 格，而全量跑到 913 張時是**變好 7、變壞 29**。
- 走到「要宣稱完成」那一步之前，全量要再跑一次完整的。
