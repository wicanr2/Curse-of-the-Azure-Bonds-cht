...
- **oracle 擷取「驗不了的那一維」會安靜地變成 remake 的錯。** 原版擷取一律要
  把狀態從畫面讀回來核對，但**有些畫面根本不顯示那個狀態**——CoAB 有幾張地圖
  不顯示座標，擷取腳本只核對得到朝向。那些圖的擷取一旦站錯格，逐格比對會把它
  記成「remake 畫錯」，而且數字很大、看起來很像真的 bug。判讀規則：
  **驗不了的那一維上，「差異很大且類別是整面牆有無」要先當成擷取錯，
  小差異（相鄰灰階、幾十格）才是渲染問題。**
  ⚠ 想補一個「不靠畫面文字」的驗證時先做**正對照**：第 750 輪那個
  「用 GEO 推深度 1 有沒有牆、再看畫面那一帶是不是純色」的判準**兩個方向都有
  偽陽性**（側牆不蓋到中間、遠牆正好蓋在中間），其中幾筆在逐格比對裡是完全
  相同的 ⇒ 判準錯，不是資料錯。**驗證器要先能通過已知正確的樣本。**
