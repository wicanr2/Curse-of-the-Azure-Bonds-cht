...
第一百六十三輪待提交：依 reference `Draw3dWorldFar/Mid/Near` 將 party direction 與 GEO wall fields 接成 ordered `WallLayoutCall` traversal，dungeon preview 改用 Far／Mid／Near 全段 wall stamps；補上 depth／座標 regression、READY 規格與共用 graphics knowledge。GEO wrap、sky／roof、door、遮擋與 camera 仍保留 boundary。
