...
第一百七十七輪功能 commit 待補：依 reference `seg001.Init` 將 State、save／preview fallback 與 startup dungeon default 從 `(8,8,0)` 修正為 `(7,13,0)`，並清理已過時的 README／knowledge assertions；新增 READY 規格。`InitAgain` direction 2、完整 SAVGAM context 與劇情 integration 仍保留 boundary。
