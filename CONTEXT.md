...
規則要求推翻紀錄集中一處。**逐筆內容寫在當月分冊**（正文一律直接改成正確答案，
不在正文敘述當初怎麼錯的）；本節只留索引，方便查「這個結論是不是被推翻過」。

| 日期 | 被推翻的斷言 | 正確答案 | 紀錄 |
|---|---|---|---|
| 2026-08-15 | `overlay-17:046ECh` 是「HP 上限」；`+109h + i` 的語意有衝突 | `046ECh` 是**有效等級**（spec 850 早已讀完）；`+109h + i` 是**職業槽等級**，五份規格獨立確認過 | [`50-log`](docs/context/50-log-2026-08-09-13.md) |
| 2026-08-15 | spec 1093 §七 的種族天賦表以 0 ＝ 人類起算 | 種族編號是 1..7，人類是 7；表裡的 `0` 其實是 `case` 的 `else`（人類與半獸人共用，都不掛效果） | 同上 |
| 2026-08-15 | 建角四張清單的選項文字「不在這一支裡」 | 四組字串都是建角函式自己從資料段取的（spec 1100） | 同上 |
| 2026-08-16 | HP 在屬性接受後算一次；基準值複製在屬性接受那一步 | HP 在**重擲迴圈裡**（`21C1h`）；基準值複製在**名字輸入之後**（`244Bh`） | 同上 |
