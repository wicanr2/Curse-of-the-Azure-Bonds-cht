...
第一百零二輪：DOS player `.SAV/.GUY` + `.FX/.SWG` sidecar bundle importer。
...
第一百零二輪功能／文件 commit：待本輪提交。新增 `ParseDOSPlayerFiles` sidecar bundle importer，並將 gold/gems/jewelry 保存到 `Character`；`SAVGAM?.DAT` party／area container 仍未解析。
