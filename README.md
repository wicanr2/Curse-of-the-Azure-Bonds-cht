...
- ECL `LOAD PIECES` 現在會保存三個 map-piece selectors 並繼續執行；實際地城／牆面／碰撞副作用仍待 file loader 反組譯。
- game state 會保留 `LOAD PIECES` request 一次並提供給後續 map-piece adapter；目前不猜測檔案對應或地城副作用。
