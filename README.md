...
| 規格 | 1,177 份 `docs/spec/*.md`，各自標證據等級與「明確不宣稱」邊界 |
| 程式 | 465 個 `.go`／130,744 行；`internal/game` 沒有區域或劇情專屬檔 |
| ECL 文字 | 控制流可達 1,022 頁，**未接上 0** |
| ECL 副作用 | 可達 14,177 條指令，**只讀掉運算元 0**、部分完成 **487**（opcode：`done` 58／`partial` 3）|
| 戰鬥法術 | 可宣告 **73／73** 全部有 handler、視覺與音效 |
| 命中判定 | 與原作同一式：FIRE KNIFE 自己打自己，原作與 remake 都要 **d20 ≥ 18** |
| 第一人稱畫面 | 17 張地圖、**1,474 張**與原版逐格比對：1,461 張完全相同；剩下 13 張的成因已找到並修好，逐張重驗 0 格 |
| 存檔 | 角色記錄 422 bytes：decoded 307／documented 115／**unknown 0** |
| UI 詞條 | game pack `zh-TW` 1,526 條、`assets/locale/zh-TW.json` 1,143 條 |
...
| 段內支線 | 全遊戲對照表 **299 個場景對到格子**（`cmd/ecl-cell-events`）；逐格實測 **250 個分派索引**（`cmd/cell-sweep`），**235 格中文、落回原文 0 格**，15 格沒演出來全部說得出守衛（12 格是 `4C00` 區的跨地圖污染，spec 1142）。另有一次**帶著主線進度**的取樣：23 份段界快照上 9 段、**123 格演得出來、落回原文 0 格** |
...
- **ECL 的 9 個部分完成 opcode**，共 1,024 條指令：`PICTURE`、`COMBAT`、`CALL`、
  ...
  `PICTURE`（199 條）也讀完了（spec 1148）：`0FFh` 是關閉，先前 remake 什麼都不做。
...
- **表現層與音訊**：畫面逐張對照、每個場景與戰鬥 phase 的音效綁定。
  第一人稱已有全作逐格對照（1,474 張，spec 1185）；仍未驗的是**天空色**——
  提爾佛頓那兩個欄位是從原版存檔量到的，其餘 16 張 `first_person` 地圖仍是
  宣告值，多半是 0（黑天花板）。SPRIT 畫布相對於戰場格的錨點仍未量（現在只在沒有
  CPIC 時才會走到那條路）。
