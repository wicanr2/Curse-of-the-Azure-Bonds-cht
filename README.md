...
- `CAMP → MAGIC` 現在提供原版已證實的 `CAST／MEMORIZE／SCRIBE／DISPLAY／REST／EXIT` command menu；`DISPLAY` 可查看各角色已記憶法術繁中名稱並保留 DOS known-spell flags 數量，`REST` 回用 CAMP 休息服務，施法／記憶／抄錄規則仍待接入。
