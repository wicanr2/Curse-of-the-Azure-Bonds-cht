...
| 段內支線 | 全遊戲對照表 **299 個場景對到格子**（`cmd/ecl-cell-events`）；逐格實測 **250 個分派索引**（`cmd/cell-sweep`），**235 格中文、落回原文 0 格**，15 格沒演出來全部說得出守衛（12 格是 `4C00` 區的跨地圖污染，spec 1142）|
| 世界地圖 | 路段編號 `4C9D ＝ 出發地 × 4 ＋ 方向`（spec 1143）；走得到的分支逐條驗語系，**落回原文 0 條**（`cmd/route-sweep`）|
...
- **主線的段內支線**：開場到結局同一條 session 已經跑得完
  ...
  下水道口都還沒接。驗證方式是**分段**：一個 ECL block 一段、一條 `NEWECL` 邊
  一段交接，計畫見
