...
| 規格 | 1,120 份 `docs/spec/*.md`，各自標證據等級與「明確不宣稱」邊界 |
...
| 存檔 | 角色記錄 422 bytes：decoded 296／documented 101／unknown 25 |
...
- **戰鬥回合生命週期**：回合開始那一段已收完（spec 1135／1136）——先攻算式、
  ...
  剩下的：原作 `+09h` 的幾何意義**刻意留白**（兩個寫入端互相矛盾）；
  機會攻擊前面四道未解讀的閘沒有對應物；`v`（bank 1 依隊號選的固定命中修正）
  沒有對應物；隊員的 AC 與命中加值仍是本作近似，不是原作的職業／等級表。
  另外 ECL 的 `24h COMBAT` 那 199 處仍只做了分派。
