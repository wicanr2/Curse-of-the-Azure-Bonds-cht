...
- `State` 現在會一次性保存／消費 ECL `DAMAGE` pending requests，確保事件／選單 pause 不會遺失 script effect；未解出的 random target／`CanHitTarget`／死亡 continuation 仍待 adapter。
- DOS player `saveVerse` `0xDF–0xE3` 已保存到角色；ECL `DAMAGE` 的 selected／whole-party／random-target branches 可透過注入骰點與 `CanHitTarget` 寫回 roster／fighter HP，AC／affect projection 與死亡 continuation 仍保留邊界。
