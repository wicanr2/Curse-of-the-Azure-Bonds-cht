...
| ECL 副作用 | 可達 14,177 條指令，**只讀掉運算元 0**、部分完成 **367**（opcode：`done` 59／`partial` 2）|
...
- **ECL 剩 2 個部分完成 opcode**，共 367 條指令（`PICTURE` 199、`CALL` 168）。
...
- **快照走訪有 5 份推不動**（114 份裡的 `inside-block-42-*`，同一個成因）：
  走到 `ECL6/0x45:0557h` 的 `TREASURE ... 7F80`，而 `7F80` 應該由
  `0x047D ADD 7B2C, 80h → 7F80` 寫成 `≥ 80h`（隨機那一路），remake 那一刻讀到
  的是 **1** ⇒ 走進固定區塊查表而 `ITEM6.DAX` 沒有區塊 1。形狀是**控制流到得了
  `0557h` 卻沒經過 `047Dh`**。
