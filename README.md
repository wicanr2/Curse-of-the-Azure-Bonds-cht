...
- dungeon preview 會顯示目前 facing 的 reference `WallDoorFlags`／GEO `x3 detail` evidence；開門、解鎖、撬門、撞門與 door symbol overlay 仍待完成。
- dungeon preview movement 已辨識 GEO detail `1` 的 unlocked doorway；detail `2/3` 仍阻擋，完整開門／解鎖選單與技能／骰點規則仍待完成。
...
目前地城 preview 已提供 P 撬鎖、K Knock 與 B 撞門：保留 DOS open-locks、Strength／exceptional strength、隊伍順序與骰點判定，成功後對 GEO 門雙側解鎖；完整 door menu 與從劇情抵達門的流程仍未完成。
