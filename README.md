...
再睜開眼睛時，人在提爾佛頓的一間屋子裡，裝備全沒了，右臂下多出五枚青色的印記。
掌心生口的手掌是摩安德，圓圈裡的 Z 字是散塔林會，三道橫槓穿過半月屬於紅袍法師
德拉坎德羅斯，燃燒的利爪屬於烈焰之主提朗瑟克斯，還有一枚來自火刀。這五方彼此
談不上信任，結盟只為擴張共同的力量——而枷印會發光，會在你們自己都不知情的時候
接管你們。

賢者迪姆斯沃特看出了唯一的縫隙：五方各有各的盤算，誰都想搶先把你們用在自己那一步
上，不惜損害盟友。於是你們順著提爾佛頓的下水道往下查，一路走到散提爾堡、哈普圖斯
與尤拉什，取回洛山達護符、龍盔與摩安德護手，最後回頭面對掌握著光芒之池的
提朗瑟克斯。五道枷印，要一道一道拆掉。

這個故事來自 Kate Novak 與 Jeff Grubb 1988 年的小說《Azure Bonds》，SSI 隔年把它
做成《Pool of Radiance》的續作。小說的主角、同樣背負枷印的女劍士愛麗雅絲，在遊戲
中會帶著同伴龍餌與你們碰面；她認得那些印記，自己也有過相同的遭遇。
...
本輪達標依據是原版建角／建隊垂直鏈完成、六部位雙色同時支援忠實與手繪 theme，
以及 spec 1249 關閉「RE 已解但彈藥 producer 漏接」的玩家／AI／QUICK 規則缺陷；
既有三平台封包、Linux／Wine smoke 與同 session 推廣片收據仍有效。保留的 1 分是
Windows／macOS 真機、macOS 簽署／公證、音訊人耳與對外素材權利風險；它們必須
持續揭露，但依 2026-08-30 使用者決定不再阻擋第一個正式版本發布。
...
| **發行工程** | `1.0.4-20260904` 已以獨立 engine `8cc96f8650ef` 重生 Linux x86_64 AppImage、Windows x86_64 ZIP、macOS x86_64／arm64 ZIP，patch／full-local 各四件且八件雜湊全數重算通過；Linux 與 Windows（Wine）full-local 已實際啟動，兩張開場截圖逐位元組相同。公開 patch 不含原版 ZIP 與 PC-98 OGG，full-local 四平台目錄均含本機遊戲資料。GitHub Release 已由 API 讀回驗證：非草稿、非預覽，五個附件皆 `uploaded`，size 與 SHA-256 digest 與本機 patch manifest 相同。Windows／macOS 尚待真機啟動，macOS 尚未簽署／公證；內部推廣片的音樂與原版圖像權利仍未釐清（見 [1.0.4 發行收據](docs/audit/release-1.0.4-20260904.md)） |
...
- **三平台發行包與推廣片**：執行 `tools/package-release.sh <版本>` 與
  ...
  公開下載為 [v1.0.4-20260904 最新 patch 版](https://github.com/wicanr2/Curse-of-the-Azure-Bonds-cht/releases/tag/v1.0.4-20260904)：
