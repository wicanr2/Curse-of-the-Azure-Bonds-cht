...
- ECL `PARTYSTRENGTH (0x1D)` 與 `PARTY SURPRISE (0x22)` 已依 reference 消費 word destinations，跨 `NEWECL` 保存 renderer/state-neutral requests 並繼續事件；實際 party HP／AC／職業等級計算仍待 State adapter 接入。
