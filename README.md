...
| 規格 | 1,121 份 `docs/spec/*.md`，各自標證據等級與「明確不宣稱」邊界 |
| 程式 | 316 個 `.go`／96,244 行；`internal/game` 沒有區域或劇情專屬檔 |
...
| 主線分段 | 25 段全部可直入（`-segment ECL{成員}/0x{block}`），逐段一條測試；47 條 `NEWECL` 邊逐條交接 |
| 分段接線 | 入口有中文劇情 **22**／入口不出文字 3／**落回原文 0** |
| 段測試 | 整條主線跑拆成 **18 個段 subtest**，每段結束的快照都存得下去讀得回來 |
...
猶拉什 → 希爾斯法 → 立石群的灰袍男子 → **密斯卓諾遺跡入口（第六章）**。
...
- **開場到結局的主線串接**是最大的一塊：目前走到眼魔洞穴與散提爾堡世界選單，
  ...
  25 段現在都能用 `-segment <id>` 直接進入，逐段一條測試；段的邊界狀態存得下去
  讀得回來，47 條 `NEWECL` 邊也逐條有交接測試。接線的現況量過了：22 段的入口有
  中文劇情、3 段的入口不出文字（是被別段帶進來的段落）、**沒有一段落回原文**。
  整條主線跑（哈普村 → 密斯卓諾）已拆成 18 個段 subtest，覆蓋的步數一步沒少，
  每段結束的快照都存得下去讀得回來。第六章已經接到墓園的紅網與黛米爾公主的祝福；剩下的工作在
  **段內**——密斯卓諾的盜墓者、外城與內城遺跡、最終戰。
