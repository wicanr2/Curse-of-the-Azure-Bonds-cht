...
截至 2026-08-09 的完整「已完成／未完成／驗證方式」盤點見
 [`docs/project-status.md`](docs/project-status.md)。目前 GitHub `main` 為第 517
輪反組譯盤點 milestone，獨立 engine 已包含前一輪資料驅動轉場變更；實際
最新版本以 GitHub `main`／本文件所在 commit 為準。這是可執行的多垂直切片 prototype，
尚未宣稱完整可通關。

第 517 輪整理出仍未閉合的 11 個行為逆向主題與 4 個 fidelity／發行主題，並移除
