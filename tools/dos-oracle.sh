...
#      答 `2` `3` `Return` 就進 EGA。答完會寫出 `CURSE.CFG`，內容是四行純文字
#      （`E`／`S`／`C:\SAVE\`／`F`，見 spec 1134）——**留著它三題整個跳過，
#      直接以 EGA 進標題畫面**。取 oracle 一律 `KEEP_CFG=1`：少三個按鍵就少
#      三個錯位的機會。本工具預設刪掉它，只為了重現「第一次啟動」。
...
# 而一個錯位的鍵可能剛好按到 `EXIT TO DOS`。要走完整條選單路徑請改用
# `tools/dos-oracle-session.sh`——容器常駐、逐步送鍵、每一步都把畫面文字讀回來
# （`tools/dos_screen.py`）再決定下一鍵。本工具留給「一句話拍一張」的場合。
set -euo pipefail
